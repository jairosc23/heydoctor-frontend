...
const SECTION = join(
  DIR,
  "../../app/panel/consultas/[id]/_components/chart/ClinicalReasoningSection.tsx",
);
const CHART = join(
  DIR,
...
const FORBIDDEN = [
  "composeClinicalReasoning",
  "clinical-reasoning.aggregate",
  "evaluateClinicalReasoningGate",
  "resolveClinicalReasoningCapability",
  "adaptClinicalReasoningComposerInput",
  "adaptAssembledUnderstanding",
  "adaptClinicalFoundationContext",
  "clinical-reasoning.composer",
...
test("clinical reasoning client does not import Composer, Aggregate, Gate, adapters, Understanding or Foundation", () => {
...
test("Encounter clinical reasoning section consumes only the HTTP client", () => {
  const source = readFileSync(SECTION, "utf8");
  assert.ok(source.includes("@/lib/clinical-reasoning"));
  assert.ok(source.includes("listEnabledClinicalReasoningTypes"));
  assert.equal(source.includes("href="), false);
  assert.equal(source.includes("/write"), false);
  assertClean("ClinicalReasoningSection.tsx", source);
});

test("Encounter chart wires the section without engine domain imports", () => {
  const source = readFileSync(CHART, "utf8");
  assert.ok(source.includes("ClinicalReasoningSection"));
