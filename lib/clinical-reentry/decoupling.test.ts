...
const SECTION = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalReentrySection.tsx");
const CHART = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalEncounterChart.tsx");

const FORBIDDEN = [
  "composeClinicalReentry",
  "clinical-reentry.aggregate",
  "evaluateClinicalReentryGate",
  "resolveClinicalReentryCapability",
  "adaptClinicalReentryComposerInput",
  "adaptConstitutedLearning",
  "adaptClinicalFoundationContext",
  "clinical-reentry.composer",
  "clinical-reentry.gate",
  "clinical-reentry.capability",
...
test("clinical reentry client does not import Composer, Aggregate, Gate, adapters, Learning or Foundation", () => {
...
test("Encounter clinical reentry section consumes only the HTTP client", () => {
  const source = readFileSync(SECTION, "utf8");
  assert.ok(source.includes("@/lib/clinical-reentry"));
  assert.ok(source.includes("listEnabledClinicalReentryTypes"));
