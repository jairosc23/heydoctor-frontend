...
const SECTION = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalKnowledgeSection.tsx");
const CHART = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalEncounterChart.tsx");

const FORBIDDEN = [
  "composeClinicalKnowledge",
  "clinical-knowledge.aggregate",
  "evaluateClinicalKnowledgeGate",
  "resolveClinicalKnowledgeCapability",
  "adaptClinicalKnowledgeComposerInput",
  "adaptKnowledgeCitation",
  "adaptClinicalFoundationClinic",
  "clinical-knowledge.composer",
  "clinical-knowledge.gate",
  "clinical-knowledge.capability",
...
test("clinical knowledge client does not import Composer, Aggregate, Gate, adapters, Reentry or Foundation", () => {
...
test("Encounter clinical knowledge section consumes only the HTTP client", () => {
  const source = readFileSync(SECTION, "utf8");
  assert.ok(source.includes("@/lib/clinical-knowledge"));
  assert.ok(source.includes("listEnabledClinicalKnowledgeTypes"));
