import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedHeartScoreCalculationEngineEntryView, type GovernedHeartScoreCalculationEngineResult } from "./governed-heart-score-calculation-engine";
function mapEntry(raw: unknown): GovernedHeartScoreCalculationEngineEntryView | null {
...
export function mapGovernedHeartScoreCalculationEngineEnvelope(payload: unknown): GovernedHeartScoreCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedHeartScoreCalculationEngineEntryView => e !== null);
