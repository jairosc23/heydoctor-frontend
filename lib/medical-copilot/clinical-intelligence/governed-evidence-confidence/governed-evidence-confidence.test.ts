...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-evidence-confidence";
import { mapGovernedEvidenceConfidenceEnvelope } from "./governed-evidence-confidence-mapper";

describe("GovernedEvidenceConfidence mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedEvidenceConfidenceEnvelope({
    ...
    assert.equal(mapGovernedEvidenceConfidenceEnvelope(null), null);
  });
});
