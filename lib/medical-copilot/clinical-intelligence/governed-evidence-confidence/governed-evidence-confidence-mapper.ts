import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedEvidenceConfidenceComponentKey, type GovernedEvidenceConfidenceComponentPresence, type GovernedEvidenceConfidenceResult } from "./governed-evidence-confidence";

const COMPONENT_DEFS: Array<{ key: GovernedEvidenceConfidenceComponentKey; label: string }> = [
...
export function mapGovernedEvidenceConfidenceEnvelope(payload: unknown): GovernedEvidenceConfidenceResult | null {
  ...
  const components: GovernedEvidenceConfidenceComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "confidence") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
