import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedGlasgowCalculationEngineEntryView, type GovernedGlasgowCalculationEngineResult } from "./governed-glasgow-calculation-engine";
function mapEntry(raw: unknown): GovernedGlasgowCalculationEngineEntryView | null {
...
export function mapGovernedGlasgowCalculationEngineEnvelope(payload: unknown): GovernedGlasgowCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGlasgowCalculationEngineEntryView => e !== null);
