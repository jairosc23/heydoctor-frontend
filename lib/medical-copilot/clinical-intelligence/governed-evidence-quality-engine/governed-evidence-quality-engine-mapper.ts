import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceQualityEngineEntryView, type GovernedEvidenceQualityEngineResult } from "./governed-evidence-quality-engine";

function mapEntry(raw: unknown): GovernedEvidenceQualityEngineEntryView | null {
...
export function mapGovernedEvidenceQualityEngineEnvelope(payload: unknown): GovernedEvidenceQualityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceQualityEngineEntryView => e !== null);
