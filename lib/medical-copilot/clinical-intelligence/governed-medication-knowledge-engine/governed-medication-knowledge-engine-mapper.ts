import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedMedicationKnowledgeEngineEntryView, type GovernedMedicationKnowledgeEngineResult } from "./governed-medication-knowledge-engine";

function mapEntry(raw: unknown): GovernedMedicationKnowledgeEngineEntryView | null {
...
export function mapGovernedMedicationKnowledgeEngineEnvelope(payload: unknown): GovernedMedicationKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMedicationKnowledgeEngineEntryView => e !== null);
