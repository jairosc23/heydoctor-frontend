import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedReadmissionRiskPopulationEngineEntryView, type GovernedReadmissionRiskPopulationEngineResult } from "./governed-readmission-risk-population-engine";
function mapEntry(raw: unknown): GovernedReadmissionRiskPopulationEngineEntryView | null {
...
export function mapGovernedReadmissionRiskPopulationEngineEnvelope(payload: unknown): GovernedReadmissionRiskPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedReadmissionRiskPopulationEngineEntryView => e !== null);
