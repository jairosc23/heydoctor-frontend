import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDecisionQualityEngineEntryView, type GovernedDecisionQualityEngineResult } from "./governed-decision-quality-engine";
function mapEntry(raw: unknown): GovernedDecisionQualityEngineEntryView | null {
...
export function mapGovernedDecisionQualityEngineEnvelope(payload: unknown): GovernedDecisionQualityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDecisionQualityEngineEntryView => e !== null);
