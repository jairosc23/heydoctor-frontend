import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGoldGuidelineEngineEntryView, type GovernedGoldGuidelineEngineResult } from "./governed-gold-guideline-engine";

function mapEntry(raw: unknown): GovernedGoldGuidelineEngineEntryView | null {
...
export function mapGovernedGoldGuidelineEngineEnvelope(payload: unknown): GovernedGoldGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGoldGuidelineEngineEntryView => e !== null);
