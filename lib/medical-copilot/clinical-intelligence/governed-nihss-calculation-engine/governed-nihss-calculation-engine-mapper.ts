import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedNihssCalculationEngineEntryView, type GovernedNihssCalculationEngineResult } from "./governed-nihss-calculation-engine";
function mapEntry(raw: unknown): GovernedNihssCalculationEngineEntryView | null {
...
export function mapGovernedNihssCalculationEngineEnvelope(payload: unknown): GovernedNihssCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedNihssCalculationEngineEntryView => e !== null);
