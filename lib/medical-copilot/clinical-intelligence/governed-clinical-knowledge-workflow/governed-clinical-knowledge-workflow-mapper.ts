import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalKnowledgeWorkflowRefView, type GovernedClinicalKnowledgeWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-knowledge-workflow";
function mapRef(raw: unknown): GovernedClinicalKnowledgeWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalKnowledgeWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalKnowledgeWorkflowEnvelope(payload: unknown): GovernedClinicalKnowledgeWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalKnowledgeWorkflowRefView => x !== null)
