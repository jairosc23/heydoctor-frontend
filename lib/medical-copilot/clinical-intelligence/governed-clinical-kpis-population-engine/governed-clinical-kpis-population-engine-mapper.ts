import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedClinicalKpisPopulationEngineEntryView, type GovernedClinicalKpisPopulationEngineResult } from "./governed-clinical-kpis-population-engine";
function mapEntry(raw: unknown): GovernedClinicalKpisPopulationEngineEntryView | null {
...
export function mapGovernedClinicalKpisPopulationEngineEnvelope(payload: unknown): GovernedClinicalKpisPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalKpisPopulationEngineEntryView => e !== null);
