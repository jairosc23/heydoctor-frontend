import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCalculationValidationEngineEntryView, type GovernedCalculationValidationEngineResult } from "./governed-calculation-validation-engine";
function mapEntry(raw: unknown): GovernedCalculationValidationEngineEntryView | null {
...
export function mapGovernedCalculationValidationEngineEnvelope(payload: unknown): GovernedCalculationValidationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCalculationValidationEngineEntryView => e !== null);
