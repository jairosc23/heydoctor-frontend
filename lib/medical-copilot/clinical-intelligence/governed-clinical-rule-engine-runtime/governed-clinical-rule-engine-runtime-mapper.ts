import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedClinicalRuleEngineRuntimeEvaluationView, type GovernedClinicalRuleEngineRuntimeResult } from "./governed-clinical-rule-engine-runtime";

function mapEvaluation(raw: unknown): GovernedClinicalRuleEngineRuntimeEvaluationView | null {
...
export function mapGovernedClinicalRuleEngineRuntimeEnvelope(payload: unknown): GovernedClinicalRuleEngineRuntimeResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedClinicalRuleEngineRuntimeEvaluationView => e !== null);
