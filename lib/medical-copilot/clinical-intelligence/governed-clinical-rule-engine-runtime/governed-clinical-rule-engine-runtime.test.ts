...
describe("GovernedClinicalRuleEngineRuntime mapper", () => {
  it("maps deterministic rule evaluations with explainability fields", () => {
    const mapped = mapGovernedClinicalRuleEngineRuntimeEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Clinical Rule Engine Runtime",
      triggeredCount: 1,
      evaluations: [{
    ...
    assert.equal(mapGovernedClinicalRuleEngineRuntimeEnvelope(null), null);
  });
});
