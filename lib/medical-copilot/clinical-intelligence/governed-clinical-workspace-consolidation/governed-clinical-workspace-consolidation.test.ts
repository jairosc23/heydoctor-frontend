...
import { GOVERNED_CLINICAL_WORKSPACE_CONSOLIDATION_GOVERNANCE } from "./governed-clinical-workspace-consolidation";
import { mapGovernedClinicalWorkspaceConsolidationEnvelope } from "./governed-clinical-workspace-consolidation-mapper";

describe("Phase 32 GovernedClinicalWorkspaceConsolidation mapper", () => {
  it("maps composition presence and preserves HITL", () => {
    const mapped = mapGovernedClinicalWorkspaceConsolidationEnvelope({
      ...
      reason: "governed_clinical_workspace_consolidation_composed_for_physician_review",
    });
    assert.ok(mapped);
    ...
    assert.equal(mapGovernedClinicalWorkspaceConsolidationEnvelope(null), null);
  });
});
