import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedImagingKnowledgeEngineEntryView, type GovernedImagingKnowledgeEngineResult } from "./governed-imaging-knowledge-engine";

function mapEntry(raw: unknown): GovernedImagingKnowledgeEngineEntryView | null {
...
export function mapGovernedImagingKnowledgeEngineEnvelope(payload: unknown): GovernedImagingKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedImagingKnowledgeEngineEntryView => e !== null);
