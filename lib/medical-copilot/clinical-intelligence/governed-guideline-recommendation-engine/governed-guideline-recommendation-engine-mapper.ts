import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGuidelineRecommendationEngineEntryView, type GovernedGuidelineRecommendationEngineResult } from "./governed-guideline-recommendation-engine";

function mapEntry(raw: unknown): GovernedGuidelineRecommendationEngineEntryView | null {
...
export function mapGovernedGuidelineRecommendationEngineEnvelope(payload: unknown): GovernedGuidelineRecommendationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGuidelineRecommendationEngineEntryView => e !== null);
