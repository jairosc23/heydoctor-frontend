import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceConfidenceEngineEntryView, type GovernedEvidenceConfidenceEngineResult } from "./governed-evidence-confidence-engine";

function mapEntry(raw: unknown): GovernedEvidenceConfidenceEngineEntryView | null {
...
export function mapGovernedEvidenceConfidenceEngineEnvelope(payload: unknown): GovernedEvidenceConfidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceConfidenceEngineEntryView => e !== null);
