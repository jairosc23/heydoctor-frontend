import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedSyndromicRecognitionDiagnosticIntelEngineEntryView, type GovernedSyndromicRecognitionDiagnosticIntelEngineResult } from "./governed-syndromic-recognition-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedSyndromicRecognitionDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedSyndromicRecognitionDiagnosticIntelEngineEnvelope(payload: unknown): GovernedSyndromicRecognitionDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedSyndromicRecognitionDiagnosticIntelEngineEntryView => e !== null);
