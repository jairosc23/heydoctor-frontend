import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalConflictDetectionEngineEntryView, type GovernedClinicalConflictDetectionEngineResult } from "./governed-clinical-conflict-detection-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalConflictDetectionEngineEntryView | null {
...
export function mapGovernedClinicalConflictDetectionEngineEnvelope(payload: unknown): GovernedClinicalConflictDetectionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalConflictDetectionEngineEntryView => e !== null);
