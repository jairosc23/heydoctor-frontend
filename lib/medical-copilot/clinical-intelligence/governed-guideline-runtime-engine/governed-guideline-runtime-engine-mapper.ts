import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGuidelineRuntimeEngineEntryView, type GovernedGuidelineRuntimeEngineResult } from "./governed-guideline-runtime-engine";

function mapEntry(raw: unknown): GovernedGuidelineRuntimeEngineEntryView | null {
...
export function mapGovernedGuidelineRuntimeEngineEnvelope(payload: unknown): GovernedGuidelineRuntimeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGuidelineRuntimeEngineEntryView => e !== null);
