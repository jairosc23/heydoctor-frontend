import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDecisionSafetyEngineEntryView, type GovernedDecisionSafetyEngineResult } from "./governed-decision-safety-engine";
function mapEntry(raw: unknown): GovernedDecisionSafetyEngineEntryView | null {
...
export function mapGovernedDecisionSafetyEngineEnvelope(payload: unknown): GovernedDecisionSafetyEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDecisionSafetyEngineEntryView => e !== null);
