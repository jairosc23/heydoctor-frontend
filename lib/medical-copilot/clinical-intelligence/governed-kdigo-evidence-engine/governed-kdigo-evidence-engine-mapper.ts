import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedKdigoEvidenceEngineEntryView, type GovernedKdigoEvidenceEngineResult } from "./governed-kdigo-evidence-engine";

function mapEntry(raw: unknown): GovernedKdigoEvidenceEngineEntryView | null {
...
export function mapGovernedKdigoEvidenceEngineEnvelope(payload: unknown): GovernedKdigoEvidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedKdigoEvidenceEngineEntryView => e !== null);
