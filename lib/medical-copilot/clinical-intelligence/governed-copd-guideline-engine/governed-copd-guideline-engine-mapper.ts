import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedCopdGuidelineEngineEntryView, type GovernedCopdGuidelineEngineResult } from "./governed-copd-guideline-engine";

function mapEntry(raw: unknown): GovernedCopdGuidelineEngineEntryView | null {
...
export function mapGovernedCopdGuidelineEngineEnvelope(payload: unknown): GovernedCopdGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCopdGuidelineEngineEntryView => e !== null);
