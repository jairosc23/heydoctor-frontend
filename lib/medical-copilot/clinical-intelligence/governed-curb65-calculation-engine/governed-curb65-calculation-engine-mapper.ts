import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCurb65CalculationEngineEntryView, type GovernedCurb65CalculationEngineResult } from "./governed-curb65-calculation-engine";
function mapEntry(raw: unknown): GovernedCurb65CalculationEngineEntryView | null {
...
export function mapGovernedCurb65CalculationEngineEnvelope(payload: unknown): GovernedCurb65CalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCurb65CalculationEngineEntryView => e !== null);
