import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedClinicalDecisionPackageComponentKey, type GovernedClinicalDecisionPackageComponentPresence, type GovernedClinicalDecisionPackageResult } from "./governed-clinical-decision-package";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalDecisionPackageComponentKey; label: string }> = [
...
export function mapGovernedClinicalDecisionPackageEnvelope(payload: unknown): GovernedClinicalDecisionPackageResult | null {
  ...
  const components: GovernedClinicalDecisionPackageComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "package") present = data.runtime != null || data.surfacesPresent != null || data.ready === true || data.packageId != null;
