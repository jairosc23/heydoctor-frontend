...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-clinical-decision-package";
import { mapGovernedClinicalDecisionPackageEnvelope } from "./governed-clinical-decision-package-mapper";

describe("GovernedClinicalDecisionPackage mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedClinicalDecisionPackageEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Clinical Decision Package",
      kind: "evidence",
    ...
    assert.equal(mapGovernedClinicalDecisionPackageEnvelope(null), null);
  });
});
