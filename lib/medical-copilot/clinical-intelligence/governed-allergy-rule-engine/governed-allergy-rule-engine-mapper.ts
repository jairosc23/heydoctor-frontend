import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedAllergyRuleEngineEvaluationView, type GovernedAllergyRuleEngineResult } from "./governed-allergy-rule-engine";

function mapEvaluation(raw: unknown): GovernedAllergyRuleEngineEvaluationView | null {
...
export function mapGovernedAllergyRuleEngineEnvelope(payload: unknown): GovernedAllergyRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedAllergyRuleEngineEvaluationView => e !== null);
