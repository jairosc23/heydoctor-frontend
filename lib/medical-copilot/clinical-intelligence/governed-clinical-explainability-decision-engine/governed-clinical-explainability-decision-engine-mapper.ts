import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalExplainabilityEngineEntryView, type GovernedClinicalExplainabilityEngineResult } from "./governed-clinical-explainability-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalExplainabilityEngineEntryView | null {
...
export function mapGovernedClinicalExplainabilityEngineEnvelope(payload: unknown): GovernedClinicalExplainabilityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalExplainabilityEngineEntryView => e !== null);
