import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticAlertsDiagnosticIntelEngineEntryView, type GovernedDiagnosticAlertsDiagnosticIntelEngineResult } from "./governed-diagnostic-alerts-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticAlertsDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticAlertsDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticAlertsDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticAlertsDiagnosticIntelEngineEntryView => e !== null);
