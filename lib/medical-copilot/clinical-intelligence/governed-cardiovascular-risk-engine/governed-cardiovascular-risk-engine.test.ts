...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-cardiovascular-risk-engine";
import { mapGovernedCardiovascularRiskEngineEnvelope } from "./governed-cardiovascular-risk-engine-mapper";
describe("GovernedCardiovascularRiskEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedCardiovascularRiskEngineEnvelope({
    ...
    assert.equal(mapGovernedCardiovascularRiskEngineEnvelope(null), null);
  });
});
