import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedQsofaCalculationEngineEntryView, type GovernedQsofaCalculationEngineResult } from "./governed-qsofa-calculation-engine";
function mapEntry(raw: unknown): GovernedQsofaCalculationEngineEntryView | null {
...
export function mapGovernedQsofaCalculationEngineEnvelope(payload: unknown): GovernedQsofaCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedQsofaCalculationEngineEntryView => e !== null);
