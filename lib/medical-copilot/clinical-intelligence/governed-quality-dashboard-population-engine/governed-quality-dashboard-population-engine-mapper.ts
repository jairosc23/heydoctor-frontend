import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedQualityDashboardPopulationEngineEntryView, type GovernedQualityDashboardPopulationEngineResult } from "./governed-quality-dashboard-population-engine";
function mapEntry(raw: unknown): GovernedQualityDashboardPopulationEngineEntryView | null {
...
export function mapGovernedQualityDashboardPopulationEngineEnvelope(payload: unknown): GovernedQualityDashboardPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedQualityDashboardPopulationEngineEntryView => e !== null);
