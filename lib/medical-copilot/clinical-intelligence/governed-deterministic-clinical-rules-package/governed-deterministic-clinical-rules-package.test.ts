...
describe("GovernedDeterministicClinicalRulesPackage mapper", () => {
  it("maps deterministic rule evaluations with explainability fields", () => {
    const mapped = mapGovernedDeterministicClinicalRulesPackageEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Deterministic Clinical Rules Package",
      triggeredCount: 1,
      evaluations: [{
    ...
    assert.equal(mapGovernedDeterministicClinicalRulesPackageEnvelope(null), null);
  });
});
