import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedDiabetesGuidelineEngineEntryView, type GovernedDiabetesGuidelineEngineResult } from "./governed-diabetes-guideline-engine";

function mapEntry(raw: unknown): GovernedDiabetesGuidelineEngineEntryView | null {
...
export function mapGovernedDiabetesGuidelineEngineEnvelope(payload: unknown): GovernedDiabetesGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiabetesGuidelineEngineEntryView => e !== null);
