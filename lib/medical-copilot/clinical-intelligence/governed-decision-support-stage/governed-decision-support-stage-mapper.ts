import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedDecisionSupportStageResult, type GovernedDecisionSupportStageStageView, type GovernedDecisionSupportStageSurfaceRefView } from "./governed-decision-support-stage";

function mapRef(raw: unknown): GovernedDecisionSupportStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedDecisionSupportStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedDecisionSupportStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedDecisionSupportStageEnvelope(payload: unknown): GovernedDecisionSupportStageResult | null {
  ...
  let stages: GovernedDecisionSupportStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedDecisionSupportStageStageView => x !== null);
