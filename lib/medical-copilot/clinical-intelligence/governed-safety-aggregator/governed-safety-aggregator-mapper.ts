import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedSafetyAggregatorRefView, type GovernedSafetyAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-safety-aggregator";
function mapRef(raw: unknown): GovernedSafetyAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedSafetyAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedSafetyAggregatorEnvelope(payload: unknown): GovernedSafetyAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedSafetyAggregatorRefView => x !== null)
