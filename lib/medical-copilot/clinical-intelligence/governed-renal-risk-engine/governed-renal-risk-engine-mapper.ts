import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedRenalRiskEngineComponentKey, type GovernedRenalRiskEngineComponentPresence, type GovernedRenalRiskEngineResult } from "./governed-renal-risk-engine";
const COMPONENT_DEFS: Array<{ key: GovernedRenalRiskEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedRenalRiskEngineEnvelope(payload: unknown): GovernedRenalRiskEngineResult | null {
  ...
  const components: GovernedRenalRiskEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
