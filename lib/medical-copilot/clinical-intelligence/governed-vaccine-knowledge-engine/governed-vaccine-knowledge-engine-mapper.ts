import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedVaccineKnowledgeEngineEntryView, type GovernedVaccineKnowledgeEngineResult } from "./governed-vaccine-knowledge-engine";

function mapEntry(raw: unknown): GovernedVaccineKnowledgeEngineEntryView | null {
...
export function mapGovernedVaccineKnowledgeEngineEnvelope(payload: unknown): GovernedVaccineKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedVaccineKnowledgeEngineEntryView => e !== null);
