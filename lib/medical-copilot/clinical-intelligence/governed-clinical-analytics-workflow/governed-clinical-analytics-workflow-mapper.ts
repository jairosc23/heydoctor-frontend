import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalAnalyticsWorkflowRefView, type GovernedClinicalAnalyticsWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-analytics-workflow";
function mapRef(raw: unknown): GovernedClinicalAnalyticsWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalAnalyticsWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalAnalyticsWorkflowEnvelope(payload: unknown): GovernedClinicalAnalyticsWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalAnalyticsWorkflowRefView => x !== null)
