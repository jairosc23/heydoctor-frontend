import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedClinicalAlertRuleEngineEvaluationView, type GovernedClinicalAlertRuleEngineResult } from "./governed-clinical-alert-rule-engine";

function mapEvaluation(raw: unknown): GovernedClinicalAlertRuleEngineEvaluationView | null {
...
export function mapGovernedClinicalAlertRuleEngineEnvelope(payload: unknown): GovernedClinicalAlertRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedClinicalAlertRuleEngineEvaluationView => e !== null);
