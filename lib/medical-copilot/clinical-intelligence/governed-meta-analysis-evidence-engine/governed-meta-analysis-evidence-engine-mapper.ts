import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedMetaAnalysisEvidenceEngineEntryView, type GovernedMetaAnalysisEvidenceEngineResult } from "./governed-meta-analysis-evidence-engine";

function mapEntry(raw: unknown): GovernedMetaAnalysisEvidenceEngineEntryView | null {
...
export function mapGovernedMetaAnalysisEvidenceEngineEnvelope(payload: unknown): GovernedMetaAnalysisEvidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMetaAnalysisEvidenceEngineEntryView => e !== null);
