import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedRecommendationValidationComponentKey, type GovernedRecommendationValidationComponentPresence, type GovernedRecommendationValidationResult } from "./governed-recommendation-validation";

const COMPONENT_DEFS: Array<{ key: GovernedRecommendationValidationComponentKey; label: string }> = [
...
export function mapGovernedRecommendationValidationEnvelope(payload: unknown): GovernedRecommendationValidationResult | null {
  ...
  const components: GovernedRecommendationValidationComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "validation") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
