...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-recommendation-validation";
import { mapGovernedRecommendationValidationEnvelope } from "./governed-recommendation-validation-mapper";

describe("GovernedRecommendationValidation mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedRecommendationValidationEnvelope({
    ...
    assert.equal(mapGovernedRecommendationValidationEnvelope(null), null);
  });
});
