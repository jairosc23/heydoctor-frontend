import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedChronicDiseaseRuleEngineEvaluationView, type GovernedChronicDiseaseRuleEngineResult } from "./governed-chronic-disease-rule-engine";

function mapEvaluation(raw: unknown): GovernedChronicDiseaseRuleEngineEvaluationView | null {
...
export function mapGovernedChronicDiseaseRuleEngineEnvelope(payload: unknown): GovernedChronicDiseaseRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedChronicDiseaseRuleEngineEvaluationView => e !== null);
