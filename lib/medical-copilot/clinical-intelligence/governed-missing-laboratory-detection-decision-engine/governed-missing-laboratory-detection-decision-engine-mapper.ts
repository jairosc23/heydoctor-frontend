import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedMissingLaboratoryDetectionEngineEntryView, type GovernedMissingLaboratoryDetectionEngineResult } from "./governed-missing-laboratory-detection-decision-engine";
function mapEntry(raw: unknown): GovernedMissingLaboratoryDetectionEngineEntryView | null {
...
export function mapGovernedMissingLaboratoryDetectionEngineEnvelope(payload: unknown): GovernedMissingLaboratoryDetectionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMissingLaboratoryDetectionEngineEntryView => e !== null);
