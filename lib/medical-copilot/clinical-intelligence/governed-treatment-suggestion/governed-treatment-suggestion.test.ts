...
import { GOVERNED_CLINICAL_SUGGESTIONS_UI_GOVERNANCE } from "./governed-treatment-suggestion";
import { mapGovernedTreatmentSuggestionEnvelope } from "./governed-treatment-suggestion-mapper";

describe("GovernedTreatmentSuggestion mapper", () => {
  it("maps HITL suggestion surface without write flags", () => {
    const mapped = mapGovernedTreatmentSuggestionEnvelope({
    ...
    assert.equal(mapGovernedTreatmentSuggestionEnvelope(null), null);
  });
});
