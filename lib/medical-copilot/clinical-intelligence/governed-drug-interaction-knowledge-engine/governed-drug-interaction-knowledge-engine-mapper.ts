import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedDrugInteractionKnowledgeEngineEntryView, type GovernedDrugInteractionKnowledgeEngineResult } from "./governed-drug-interaction-knowledge-engine";

function mapEntry(raw: unknown): GovernedDrugInteractionKnowledgeEngineEntryView | null {
...
export function mapGovernedDrugInteractionKnowledgeEngineEnvelope(payload: unknown): GovernedDrugInteractionKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDrugInteractionKnowledgeEngineEntryView => e !== null);
