...
import { GOVERNED_CONSULTATION_PERSISTENCE_BRIDGE_GOVERNANCE } from "./governed-consultation-persistence-bridge";
import { mapGovernedConsultationPersistenceBridgeEnvelope } from "./governed-consultation-persistence-bridge-mapper";

describe("Governed Consultation Persistence Bridge mapper", () => {
  it("maps READY_TO_CONNECT surface without write flags", () => {
    const mapped = mapGovernedConsultationPersistenceBridgeEnvelope({
      status: "READY_TO_CONNECT",
      runtime: {
        infrastructure: true,
        bridge: { status: "READY_TO_CONNECT", domain: "consultations" },
        binding: { connected: false, repositoryInvoked: false },
        validator: { pathComplete: true, stoppedBeforeWrite: true },
        preview: { nextStep: "connect_consultations_adapter" },
        execution: { planned: true, executed: false },
        readiness: { readyToConnect: true, readyForPersistence: false },
      },
      governance: { ...GOVERNED_CONSULTATION_PERSISTENCE_BRIDGE_GOVERNANCE },
      reason: "governed_consultation_persistence_bridge_ready_to_connect_pre_write",
    });
    assert.ok(mapped);
    ...
    assert.equal(mapGovernedConsultationPersistenceBridgeEnvelope(null), null);
  });
});
