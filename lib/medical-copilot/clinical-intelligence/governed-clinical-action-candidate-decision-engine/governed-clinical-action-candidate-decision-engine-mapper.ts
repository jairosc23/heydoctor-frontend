import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalActionCandidateEngineEntryView, type GovernedClinicalActionCandidateEngineResult } from "./governed-clinical-action-candidate-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalActionCandidateEngineEntryView | null {
...
export function mapGovernedClinicalActionCandidateEngineEnvelope(payload: unknown): GovernedClinicalActionCandidateEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalActionCandidateEngineEntryView => e !== null);
