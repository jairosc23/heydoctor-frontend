import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedPopulationHealthPackageEntryView, type GovernedPopulationHealthPackageResult } from "./governed-population-health-package";
function mapEntry(raw: unknown): GovernedPopulationHealthPackageEntryView | null {
...
export function mapGovernedPopulationHealthPackageEnvelope(payload: unknown): GovernedPopulationHealthPackageResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPopulationHealthPackageEntryView => e !== null);
