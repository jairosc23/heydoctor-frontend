import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedAcogGuidelineEngineEntryView, type GovernedAcogGuidelineEngineResult } from "./governed-acog-guideline-engine";

function mapEntry(raw: unknown): GovernedAcogGuidelineEngineEntryView | null {
...
export function mapGovernedAcogGuidelineEngineEnvelope(payload: unknown): GovernedAcogGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedAcogGuidelineEngineEntryView => e !== null);
