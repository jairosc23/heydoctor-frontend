import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedKdigoGuidelineEngineEntryView, type GovernedKdigoGuidelineEngineResult } from "./governed-kdigo-guideline-engine";

function mapEntry(raw: unknown): GovernedKdigoGuidelineEngineEntryView | null {
...
export function mapGovernedKdigoGuidelineEngineEnvelope(payload: unknown): GovernedKdigoGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedKdigoGuidelineEngineEntryView => e !== null);
