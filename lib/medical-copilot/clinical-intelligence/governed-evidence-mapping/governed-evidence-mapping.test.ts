...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-evidence-mapping";
import { mapGovernedEvidenceMappingEnvelope } from "./governed-evidence-mapping-mapper";

describe("GovernedEvidenceMapping mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedEvidenceMappingEnvelope({
    ...
    assert.equal(mapGovernedEvidenceMappingEnvelope(null), null);
  });
});
