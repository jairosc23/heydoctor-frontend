import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedEvidenceMappingComponentKey, type GovernedEvidenceMappingComponentPresence, type GovernedEvidenceMappingResult } from "./governed-evidence-mapping";

const COMPONENT_DEFS: Array<{ key: GovernedEvidenceMappingComponentKey; label: string }> = [
...
export function mapGovernedEvidenceMappingEnvelope(payload: unknown): GovernedEvidenceMappingResult | null {
  ...
  const components: GovernedEvidenceMappingComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "mapping") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
