import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDifferentialPrioritizationEngineEntryView, type GovernedDifferentialPrioritizationEngineResult } from "./governed-differential-prioritization-decision-engine";
function mapEntry(raw: unknown): GovernedDifferentialPrioritizationEngineEntryView | null {
...
export function mapGovernedDifferentialPrioritizationEngineEnvelope(payload: unknown): GovernedDifferentialPrioritizationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDifferentialPrioritizationEngineEntryView => e !== null);
