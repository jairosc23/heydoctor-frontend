import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedDrugInteractionRuleEngineEvaluationView, type GovernedDrugInteractionRuleEngineResult } from "./governed-drug-interaction-rule-engine";

function mapEvaluation(raw: unknown): GovernedDrugInteractionRuleEngineEvaluationView | null {
...
export function mapGovernedDrugInteractionRuleEngineEnvelope(payload: unknown): GovernedDrugInteractionRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedDrugInteractionRuleEngineEvaluationView => e !== null);
