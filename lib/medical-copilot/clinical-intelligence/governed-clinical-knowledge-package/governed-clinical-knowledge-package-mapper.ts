import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedClinicalKnowledgePackageEntryView, type GovernedClinicalKnowledgePackageResult } from "./governed-clinical-knowledge-package";

function mapEntry(raw: unknown): GovernedClinicalKnowledgePackageEntryView | null {
...
export function mapGovernedClinicalKnowledgePackageEnvelope(payload: unknown): GovernedClinicalKnowledgePackageResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalKnowledgePackageEntryView => e !== null);
