import assert from "node:assert/strict";
import { describe, it } from "node:test";
import { mapGovernedClinicalKnowledgePackageEnvelope } from "./governed-clinical-knowledge-package-mapper";

describe("Clinical Knowledge Package", () => {
  it("maps envelope with knowledge entries and HITL seals", () => {
    const mapped = mapGovernedClinicalKnowledgePackageEnvelope({
      status: "ok",
      data: {
        status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
        title: "Clinical Knowledge Package",
        applicableCount: 1,
        entries: [{
          entryId: "e1",
          entryTitle: "Entry",
          domain: "clinical_knowledge_package",
          topic: "t",
          summary: "s",
          explanation: "x",
          evidenceRefs: ["medical_copilot_session"],
          applicability: "APPLICABLE",
          confidence: "medium",
        }],
        enginesPresent: ["disease_knowledge_engine"],
        governance: { requiresPhysicianReview: true, usesLlm: false },
      },
    });
    assert.ok(mapped);
    assert.equal(mapped!.usesLlm, false);
    assert.equal(mapped!.writesEmr, false);
    assert.equal(mapped!.entries.length, 1);
    assert.equal(mapped!.applicableCount, 1);
  });
});
