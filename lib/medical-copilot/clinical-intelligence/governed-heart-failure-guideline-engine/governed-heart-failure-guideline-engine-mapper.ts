import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedHeartFailureGuidelineEngineEntryView, type GovernedHeartFailureGuidelineEngineResult } from "./governed-heart-failure-guideline-engine";

function mapEntry(raw: unknown): GovernedHeartFailureGuidelineEngineEntryView | null {
...
export function mapGovernedHeartFailureGuidelineEngineEnvelope(payload: unknown): GovernedHeartFailureGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedHeartFailureGuidelineEngineEntryView => e !== null);
