import { GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiseaseProgressionEngineLongitudinalEngineEntryView, type GovernedDiseaseProgressionEngineLongitudinalEngineResult } from "./governed-disease-progression-engine-longitudinal-engine";
function mapEntry(raw: unknown): GovernedDiseaseProgressionEngineLongitudinalEngineEntryView | null {
  ...
  return {
    ...
    timelineRole: typeof e.timelineRole === "string" ? (e.timelineRole as string) : "",
...
export function mapGovernedDiseaseProgressionEngineLongitudinalEngineEnvelope(payload: unknown): GovernedDiseaseProgressionEngineLongitudinalEngineResult | null {
  if (!payload || typeof payload !== "object") return null;
  const root = payload as Record<string, unknown>;
  const data = root.governance !== undefined || root.entries !== undefined || root.enginesPresent !== undefined || root.patientTimelineEngineLongitudinalEngine !== undefined
  ...
  else if (data.enginesPresent || data.patientTimelineEngineLongitudinalEngine) {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiseaseProgressionEngineLongitudinalEngineEntryView => e !== null);
  ...
  return {
    ...
    governance: { ...GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE }, reason: typeof data.reason === "string" ? data.reason : null,
