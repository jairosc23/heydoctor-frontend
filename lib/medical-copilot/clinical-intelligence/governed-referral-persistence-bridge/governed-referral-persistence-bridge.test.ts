...
import { GOVERNED_REFERRAL_PERSISTENCE_BRIDGE_GOVERNANCE } from "./governed-referral-persistence-bridge";
import { mapGovernedReferralPersistenceBridgeEnvelope } from "./governed-referral-persistence-bridge-mapper";

describe("Governed Referral Persistence Bridge mapper", () => {
  it("maps READY_TO_CONNECT surface without write flags", () => {
    const mapped = mapGovernedReferralPersistenceBridgeEnvelope({
      status: "READY_TO_CONNECT",
      runtime: {
        infrastructure: true,
        bridge: { status: "READY_TO_CONNECT", domain: "referrals" },
        binding: { connected: false, repositoryInvoked: false },
        validator: { pathComplete: true, stoppedBeforeWrite: true },
        preview: { nextStep: "connect_referrals_adapter" },
      ...
      reason: "governed_referral_persistence_bridge_ready_to_connect_pre_write",
    });
    assert.ok(mapped);
    ...
    assert.equal(mapGovernedReferralPersistenceBridgeEnvelope(null), null);
  });
});
