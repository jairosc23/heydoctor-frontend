import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalIntelligenceWorkflowRefView, type GovernedClinicalIntelligenceWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-intelligence-workflow";
function mapRef(raw: unknown): GovernedClinicalIntelligenceWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalIntelligenceWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalIntelligenceWorkflowEnvelope(payload: unknown): GovernedClinicalIntelligenceWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalIntelligenceWorkflowRefView => x !== null)
