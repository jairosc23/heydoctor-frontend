import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedHasBledCalculationEngineEntryView, type GovernedHasBledCalculationEngineResult } from "./governed-has-bled-calculation-engine";
function mapEntry(raw: unknown): GovernedHasBledCalculationEngineEntryView | null {
...
export function mapGovernedHasBledCalculationEngineEnvelope(payload: unknown): GovernedHasBledCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedHasBledCalculationEngineEntryView => e !== null);
