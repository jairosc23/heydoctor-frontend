import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceTraceabilityEngineEntryView, type GovernedEvidenceTraceabilityEngineResult } from "./governed-evidence-traceability-engine";

function mapEntry(raw: unknown): GovernedEvidenceTraceabilityEngineEntryView | null {
...
export function mapGovernedEvidenceTraceabilityEngineEnvelope(payload: unknown): GovernedEvidenceTraceabilityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceTraceabilityEngineEntryView => e !== null);
