import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGuidelineTraceabilityEngineEntryView, type GovernedGuidelineTraceabilityEngineResult } from "./governed-guideline-traceability-engine";

function mapEntry(raw: unknown): GovernedGuidelineTraceabilityEngineEntryView | null {
...
export function mapGovernedGuidelineTraceabilityEngineEnvelope(payload: unknown): GovernedGuidelineTraceabilityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGuidelineTraceabilityEngineEntryView => e !== null);
