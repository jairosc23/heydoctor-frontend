import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedPublicHealthKnowledgeEngineEntryView, type GovernedPublicHealthKnowledgeEngineResult } from "./governed-public-health-knowledge-engine";

function mapEntry(raw: unknown): GovernedPublicHealthKnowledgeEngineEntryView | null {
...
export function mapGovernedPublicHealthKnowledgeEngineEnvelope(payload: unknown): GovernedPublicHealthKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPublicHealthKnowledgeEngineEntryView => e !== null);
