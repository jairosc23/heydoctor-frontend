import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticValidationDiagnosticIntelEngineEntryView, type GovernedDiagnosticValidationDiagnosticIntelEngineResult } from "./governed-diagnostic-validation-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticValidationDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticValidationDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticValidationDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticValidationDiagnosticIntelEngineEntryView => e !== null);
