import { GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedChronicDiseaseTimelineLongitudinalEngineEntryView, type GovernedChronicDiseaseTimelineLongitudinalEngineResult } from "./governed-chronic-disease-timeline-longitudinal-engine";
function mapEntry(raw: unknown): GovernedChronicDiseaseTimelineLongitudinalEngineEntryView | null {
...
export function mapGovernedChronicDiseaseTimelineLongitudinalEngineEnvelope(payload: unknown): GovernedChronicDiseaseTimelineLongitudinalEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedChronicDiseaseTimelineLongitudinalEngineEntryView => e !== null);
