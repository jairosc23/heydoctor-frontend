import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedAscoGuidelineEngineEntryView, type GovernedAscoGuidelineEngineResult } from "./governed-asco-guideline-engine";

function mapEntry(raw: unknown): GovernedAscoGuidelineEngineEntryView | null {
...
export function mapGovernedAscoGuidelineEngineEnvelope(payload: unknown): GovernedAscoGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedAscoGuidelineEngineEntryView => e !== null);
