import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedFollowUpKnowledgeEngineEntryView, type GovernedFollowUpKnowledgeEngineResult } from "./governed-follow-up-knowledge-engine";

function mapEntry(raw: unknown): GovernedFollowUpKnowledgeEngineEntryView | null {
...
export function mapGovernedFollowUpKnowledgeEngineEnvelope(payload: unknown): GovernedFollowUpKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedFollowUpKnowledgeEngineEntryView => e !== null);
