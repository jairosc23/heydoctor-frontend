import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalHypothesisEngineEntryView, type GovernedClinicalHypothesisEngineResult } from "./governed-clinical-hypothesis-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalHypothesisEngineEntryView | null {
...
export function mapGovernedClinicalHypothesisEngineEnvelope(payload: unknown): GovernedClinicalHypothesisEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalHypothesisEngineEntryView => e !== null);
