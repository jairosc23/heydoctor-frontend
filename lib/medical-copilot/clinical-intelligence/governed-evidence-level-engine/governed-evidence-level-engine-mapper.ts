import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceLevelEngineEntryView, type GovernedEvidenceLevelEngineResult } from "./governed-evidence-level-engine";

function mapEntry(raw: unknown): GovernedEvidenceLevelEngineEntryView | null {
...
export function mapGovernedEvidenceLevelEngineEnvelope(payload: unknown): GovernedEvidenceLevelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceLevelEngineEntryView => e !== null);
