import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedPreventiveGuidelineEngineEntryView, type GovernedPreventiveGuidelineEngineResult } from "./governed-preventive-guideline-engine";

function mapEntry(raw: unknown): GovernedPreventiveGuidelineEngineEntryView | null {
...
export function mapGovernedPreventiveGuidelineEngineEnvelope(payload: unknown): GovernedPreventiveGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPreventiveGuidelineEngineEntryView => e !== null);
