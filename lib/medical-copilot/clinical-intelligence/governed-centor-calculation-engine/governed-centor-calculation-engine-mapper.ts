import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCentorCalculationEngineEntryView, type GovernedCentorCalculationEngineResult } from "./governed-centor-calculation-engine";
function mapEntry(raw: unknown): GovernedCentorCalculationEngineEntryView | null {
...
export function mapGovernedCentorCalculationEngineEnvelope(payload: unknown): GovernedCentorCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCentorCalculationEngineEntryView => e !== null);
