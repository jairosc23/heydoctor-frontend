...
import { GOVERNED_CLINICAL_SUGGESTIONS_UI_GOVERNANCE } from "./governed-clinical-suggestion-runtime";
import { mapGovernedClinicalSuggestionRuntimeEnvelope } from "./governed-clinical-suggestion-runtime-mapper";

describe("GovernedClinicalSuggestionRuntime mapper", () => {
  it("maps HITL suggestion surface without write flags", () => {
    const mapped = mapGovernedClinicalSuggestionRuntimeEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Clinical Suggestion Runtime",
      kind: "runtime",
    ...
    assert.equal(mapGovernedClinicalSuggestionRuntimeEnvelope(null), null);
  });
});
