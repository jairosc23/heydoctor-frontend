import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalPhysicianReviewWorkflowRefView, type GovernedClinicalPhysicianReviewWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-physician-review-workflow";
function mapRef(raw: unknown): GovernedClinicalPhysicianReviewWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPhysicianReviewWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalPhysicianReviewWorkflowEnvelope(payload: unknown): GovernedClinicalPhysicianReviewWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPhysicianReviewWorkflowRefView => x !== null)
