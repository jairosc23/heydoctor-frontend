import { GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedOutcomeTrackingLongitudinalEngineEntryView, type GovernedOutcomeTrackingLongitudinalEngineResult } from "./governed-outcome-tracking-longitudinal-engine";
function mapEntry(raw: unknown): GovernedOutcomeTrackingLongitudinalEngineEntryView | null {
...
export function mapGovernedOutcomeTrackingLongitudinalEngineEnvelope(payload: unknown): GovernedOutcomeTrackingLongitudinalEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedOutcomeTrackingLongitudinalEngineEntryView => e !== null);
