import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedRedFlagKnowledgeEngineEntryView, type GovernedRedFlagKnowledgeEngineResult } from "./governed-red-flag-knowledge-engine";

function mapEntry(raw: unknown): GovernedRedFlagKnowledgeEngineEntryView | null {
...
export function mapGovernedRedFlagKnowledgeEngineEnvelope(payload: unknown): GovernedRedFlagKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedRedFlagKnowledgeEngineEntryView => e !== null);
