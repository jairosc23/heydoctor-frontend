import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedMeldCalculationEngineEntryView, type GovernedMeldCalculationEngineResult } from "./governed-meld-calculation-engine";
function mapEntry(raw: unknown): GovernedMeldCalculationEngineEntryView | null {
...
export function mapGovernedMeldCalculationEngineEnvelope(payload: unknown): GovernedMeldCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMeldCalculationEngineEntryView => e !== null);
