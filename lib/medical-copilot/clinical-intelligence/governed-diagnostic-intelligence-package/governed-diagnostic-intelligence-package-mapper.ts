import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticIntelligencePackageEntryView, type GovernedDiagnosticIntelligencePackageResult } from "./governed-diagnostic-intelligence-package";
function mapEntry(raw: unknown): GovernedDiagnosticIntelligencePackageEntryView | null {
...
export function mapGovernedDiagnosticIntelligencePackageEnvelope(payload: unknown): GovernedDiagnosticIntelligencePackageResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticIntelligencePackageEntryView => e !== null);
