import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedWomensHealthKnowledgeEngineEntryView, type GovernedWomensHealthKnowledgeEngineResult } from "./governed-womens-health-knowledge-engine";

function mapEntry(raw: unknown): GovernedWomensHealthKnowledgeEngineEntryView | null {
...
export function mapGovernedWomensHealthKnowledgeEngineEnvelope(payload: unknown): GovernedWomensHealthKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedWomensHealthKnowledgeEngineEntryView => e !== null);
