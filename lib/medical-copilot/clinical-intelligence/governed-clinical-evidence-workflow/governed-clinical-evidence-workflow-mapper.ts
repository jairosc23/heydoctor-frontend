import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalEvidenceWorkflowRefView, type GovernedClinicalEvidenceWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-evidence-workflow";
function mapRef(raw: unknown): GovernedClinicalEvidenceWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalEvidenceWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalEvidenceWorkflowEnvelope(payload: unknown): GovernedClinicalEvidenceWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalEvidenceWorkflowRefView => x !== null)
