import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCalculationRuntimeEngineEntryView, type GovernedCalculationRuntimeEngineResult } from "./governed-calculation-runtime-engine";
function mapEntry(raw: unknown): GovernedCalculationRuntimeEngineEntryView | null {
...
export function mapGovernedCalculationRuntimeEngineEnvelope(payload: unknown): GovernedCalculationRuntimeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCalculationRuntimeEngineEntryView => e !== null);
