import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceHierarchyEngineEntryView, type GovernedEvidenceHierarchyEngineResult } from "./governed-evidence-hierarchy-engine";

function mapEntry(raw: unknown): GovernedEvidenceHierarchyEngineEntryView | null {
...
export function mapGovernedEvidenceHierarchyEngineEnvelope(payload: unknown): GovernedEvidenceHierarchyEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceHierarchyEngineEntryView => e !== null);
