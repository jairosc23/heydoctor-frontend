import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCockcroftGaultCalculationEngineEntryView, type GovernedCockcroftGaultCalculationEngineResult } from "./governed-cockcroft-gault-calculation-engine";
function mapEntry(raw: unknown): GovernedCockcroftGaultCalculationEngineEntryView | null {
...
export function mapGovernedCockcroftGaultCalculationEngineEnvelope(payload: unknown): GovernedCockcroftGaultCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCockcroftGaultCalculationEngineEntryView => e !== null);
