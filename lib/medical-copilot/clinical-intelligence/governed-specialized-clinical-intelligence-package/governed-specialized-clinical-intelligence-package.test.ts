...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-specialized-clinical-intelligence-package";
...
describe("GovernedSpecializedClinicalIntelligencePackage mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedSpecializedClinicalIntelligencePackageEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Specialized Clinical Intelligence Package",
    ...
    assert.equal(mapGovernedSpecializedClinicalIntelligencePackageEnvelope(null), null);
  });
});
