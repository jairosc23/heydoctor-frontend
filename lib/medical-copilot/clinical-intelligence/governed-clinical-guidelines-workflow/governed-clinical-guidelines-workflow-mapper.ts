import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalGuidelinesWorkflowRefView, type GovernedClinicalGuidelinesWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-guidelines-workflow";
function mapRef(raw: unknown): GovernedClinicalGuidelinesWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalGuidelinesWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalGuidelinesWorkflowEnvelope(payload: unknown): GovernedClinicalGuidelinesWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalGuidelinesWorkflowRefView => x !== null)
