import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedDiseaseBurdenPopulationEngineEntryView, type GovernedDiseaseBurdenPopulationEngineResult } from "./governed-disease-burden-population-engine";
function mapEntry(raw: unknown): GovernedDiseaseBurdenPopulationEngineEntryView | null {
...
export function mapGovernedDiseaseBurdenPopulationEngineEnvelope(payload: unknown): GovernedDiseaseBurdenPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiseaseBurdenPopulationEngineEntryView => e !== null);
