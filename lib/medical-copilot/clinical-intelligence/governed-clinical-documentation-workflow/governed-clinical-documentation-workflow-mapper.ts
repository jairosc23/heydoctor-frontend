import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalDocumentationWorkflowRefView, type GovernedClinicalDocumentationWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-documentation-workflow";
function mapRef(raw: unknown): GovernedClinicalDocumentationWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDocumentationWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalDocumentationWorkflowEnvelope(payload: unknown): GovernedClinicalDocumentationWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDocumentationWorkflowRefView => x !== null)
