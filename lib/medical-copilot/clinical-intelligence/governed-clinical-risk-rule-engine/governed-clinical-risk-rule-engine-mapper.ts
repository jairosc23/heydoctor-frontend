import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedClinicalRiskRuleEngineEvaluationView, type GovernedClinicalRiskRuleEngineResult } from "./governed-clinical-risk-rule-engine";

function mapEvaluation(raw: unknown): GovernedClinicalRiskRuleEngineEvaluationView | null {
...
export function mapGovernedClinicalRiskRuleEngineEnvelope(payload: unknown): GovernedClinicalRiskRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedClinicalRiskRuleEngineEvaluationView => e !== null);
