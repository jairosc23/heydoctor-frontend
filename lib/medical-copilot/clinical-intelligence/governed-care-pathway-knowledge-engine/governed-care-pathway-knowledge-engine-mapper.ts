import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedCarePathwayKnowledgeEngineEntryView, type GovernedCarePathwayKnowledgeEngineResult } from "./governed-care-pathway-knowledge-engine";

function mapEntry(raw: unknown): GovernedCarePathwayKnowledgeEngineEntryView | null {
...
export function mapGovernedCarePathwayKnowledgeEngineEnvelope(payload: unknown): GovernedCarePathwayKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCarePathwayKnowledgeEngineEntryView => e !== null);
