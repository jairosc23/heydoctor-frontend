import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedMissingInformationDetectionEngineEntryView, type GovernedMissingInformationDetectionEngineResult } from "./governed-missing-information-detection-decision-engine";
function mapEntry(raw: unknown): GovernedMissingInformationDetectionEngineEntryView | null {
...
export function mapGovernedMissingInformationDetectionEngineEnvelope(payload: unknown): GovernedMissingInformationDetectionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMissingInformationDetectionEngineEntryView => e !== null);
