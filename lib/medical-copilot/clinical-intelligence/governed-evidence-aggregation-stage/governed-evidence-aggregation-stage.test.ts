...
describe("GovernedEvidenceAggregationStage mapper", () => {
  it("maps pipeline stage without write/LLM flags", () => {
    const mapped = mapGovernedEvidenceAggregationStageEnvelope({
    ...
    assert.equal(mapGovernedEvidenceAggregationStageEnvelope(null), null);
  });
});
