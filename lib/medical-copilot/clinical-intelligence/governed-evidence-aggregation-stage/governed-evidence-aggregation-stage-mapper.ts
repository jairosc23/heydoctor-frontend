import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedEvidenceAggregationStageResult, type GovernedEvidenceAggregationStageStageView, type GovernedEvidenceAggregationStageSurfaceRefView } from "./governed-evidence-aggregation-stage";

function mapRef(raw: unknown): GovernedEvidenceAggregationStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedEvidenceAggregationStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedEvidenceAggregationStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedEvidenceAggregationStageEnvelope(payload: unknown): GovernedEvidenceAggregationStageResult | null {
  ...
  let stages: GovernedEvidenceAggregationStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedEvidenceAggregationStageStageView => x !== null);
