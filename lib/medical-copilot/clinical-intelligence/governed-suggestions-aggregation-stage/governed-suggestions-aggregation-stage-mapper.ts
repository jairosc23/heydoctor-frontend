import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedSuggestionsAggregationStageResult, type GovernedSuggestionsAggregationStageStageView, type GovernedSuggestionsAggregationStageSurfaceRefView } from "./governed-suggestions-aggregation-stage";

function mapRef(raw: unknown): GovernedSuggestionsAggregationStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedSuggestionsAggregationStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedSuggestionsAggregationStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedSuggestionsAggregationStageEnvelope(payload: unknown): GovernedSuggestionsAggregationStageResult | null {
  ...
  let stages: GovernedSuggestionsAggregationStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedSuggestionsAggregationStageStageView => x !== null);
