import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedCkdEpiCalculationEngineEntryView, type GovernedCkdEpiCalculationEngineResult } from "./governed-ckd-epi-calculation-engine";
function mapEntry(raw: unknown): GovernedCkdEpiCalculationEngineEntryView | null {
...
export function mapGovernedCkdEpiCalculationEngineEnvelope(payload: unknown): GovernedCkdEpiCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCkdEpiCalculationEngineEntryView => e !== null);
