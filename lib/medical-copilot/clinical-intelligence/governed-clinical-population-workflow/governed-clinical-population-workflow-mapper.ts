import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalPopulationWorkflowRefView, type GovernedClinicalPopulationWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-population-workflow";
function mapRef(raw: unknown): GovernedClinicalPopulationWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPopulationWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalPopulationWorkflowEnvelope(payload: unknown): GovernedClinicalPopulationWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPopulationWorkflowRefView => x !== null)
