import { GOVERNED_THERAPEUTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedClinicalInterventionPlanningTherapeuticEngineEntryView, type GovernedClinicalInterventionPlanningTherapeuticEngineResult } from "./governed-clinical-intervention-planning-therapeutic-engine";
function mapEntry(raw: unknown): GovernedClinicalInterventionPlanningTherapeuticEngineEntryView | null {
  ...
  return {
    ...
    therapeuticRole: typeof e.therapeuticRole === "string" ? (e.therapeuticRole as string) : "",
...
export function mapGovernedClinicalInterventionPlanningTherapeuticEngineEnvelope(payload: unknown): GovernedClinicalInterventionPlanningTherapeuticEngineResult | null {
  if (!payload || typeof payload !== "object") return null;
  const root = payload as Record<string, unknown>;
  const data = root.governance !== undefined || root.entries !== undefined || root.enginesPresent !== undefined || root.medicationOptimizationTherapeuticEngine !== undefined
  ...
  else if (data.enginesPresent || data.medicationOptimizationTherapeuticEngine) {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalInterventionPlanningTherapeuticEngineEntryView => e !== null);
  ...
  return {
    ...
    governance: { ...GOVERNED_THERAPEUTIC_INTELLIGENCE_UI_GOVERNANCE }, reason: typeof data.reason === "string" ? data.reason : null,
