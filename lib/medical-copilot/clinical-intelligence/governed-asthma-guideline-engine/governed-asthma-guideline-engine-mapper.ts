import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedAsthmaGuidelineEngineEntryView, type GovernedAsthmaGuidelineEngineResult } from "./governed-asthma-guideline-engine";

function mapEntry(raw: unknown): GovernedAsthmaGuidelineEngineEntryView | null {
...
export function mapGovernedAsthmaGuidelineEngineEnvelope(payload: unknown): GovernedAsthmaGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedAsthmaGuidelineEngineEntryView => e !== null);
