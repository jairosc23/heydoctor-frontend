import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalValidationWorkflowRefView, type GovernedClinicalValidationWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-validation-workflow";
function mapRef(raw: unknown): GovernedClinicalValidationWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalValidationWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalValidationWorkflowEnvelope(payload: unknown): GovernedClinicalValidationWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalValidationWorkflowRefView => x !== null)
