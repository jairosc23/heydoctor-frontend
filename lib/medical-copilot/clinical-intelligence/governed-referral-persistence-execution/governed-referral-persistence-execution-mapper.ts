import { GOVERNED_REFERRAL_PERSISTENCE_EXECUTION_GOVERNANCE, type GovernedReferralPersistenceExecutionComponentKey, type GovernedReferralPersistenceExecutionComponentPresence, type GovernedReferralPersistenceExecutionResult } from "./governed-referral-persistence-execution";
const COMPONENT_DEFS: Array<{ key: GovernedReferralPersistenceExecutionComponentKey; label: string }> = [
...
export function mapGovernedReferralPersistenceExecutionEnvelope(payload: unknown): GovernedReferralPersistenceExecutionResult | null {
  ...
  const components: GovernedReferralPersistenceExecutionComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
  ...
  return {
    ...
    components, governance: { ...GOVERNED_REFERRAL_PERSISTENCE_EXECUTION_GOVERNANCE },
