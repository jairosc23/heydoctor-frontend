import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedOttawaKneeRulesCalculationEngineEntryView, type GovernedOttawaKneeRulesCalculationEngineResult } from "./governed-ottawa-knee-rules-calculation-engine";
function mapEntry(raw: unknown): GovernedOttawaKneeRulesCalculationEngineEntryView | null {
...
export function mapGovernedOttawaKneeRulesCalculationEngineEnvelope(payload: unknown): GovernedOttawaKneeRulesCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedOttawaKneeRulesCalculationEngineEntryView => e !== null);
