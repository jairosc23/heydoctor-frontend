import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedClinicalAiOrchestratorPackageRefView, type GovernedClinicalAiOrchestratorPackageResult, type GovernedOrchestratorAggregatorView } from "./governed-clinical-ai-orchestrator-package";
function mapRef(raw: unknown): GovernedClinicalAiOrchestratorPackageRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalAiOrchestratorPackageRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalAiOrchestratorPackageEnvelope(payload: unknown): GovernedClinicalAiOrchestratorPackageResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalAiOrchestratorPackageRefView => x !== null)
