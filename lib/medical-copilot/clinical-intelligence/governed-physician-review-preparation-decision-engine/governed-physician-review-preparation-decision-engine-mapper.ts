import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedPhysicianReviewPreparationEngineEntryView, type GovernedPhysicianReviewPreparationEngineResult } from "./governed-physician-review-preparation-decision-engine";
function mapEntry(raw: unknown): GovernedPhysicianReviewPreparationEngineEntryView | null {
...
export function mapGovernedPhysicianReviewPreparationEngineEnvelope(payload: unknown): GovernedPhysicianReviewPreparationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPhysicianReviewPreparationEngineEntryView => e !== null);
