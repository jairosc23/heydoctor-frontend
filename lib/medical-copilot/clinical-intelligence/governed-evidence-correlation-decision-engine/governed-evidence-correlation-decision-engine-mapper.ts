import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceCorrelationEngineEntryView, type GovernedEvidenceCorrelationEngineResult } from "./governed-evidence-correlation-decision-engine";
function mapEntry(raw: unknown): GovernedEvidenceCorrelationEngineEntryView | null {
...
export function mapGovernedEvidenceCorrelationEngineEnvelope(payload: unknown): GovernedEvidenceCorrelationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceCorrelationEngineEntryView => e !== null);
