import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalCoherenceEngineEntryView, type GovernedClinicalCoherenceEngineResult } from "./governed-clinical-coherence-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalCoherenceEngineEntryView | null {
...
export function mapGovernedClinicalCoherenceEngineEnvelope(payload: unknown): GovernedClinicalCoherenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalCoherenceEngineEntryView => e !== null);
