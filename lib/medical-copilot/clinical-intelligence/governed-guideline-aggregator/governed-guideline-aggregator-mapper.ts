import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedGuidelineAggregatorRefView, type GovernedGuidelineAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-guideline-aggregator";
function mapRef(raw: unknown): GovernedGuidelineAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedGuidelineAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedGuidelineAggregatorEnvelope(payload: unknown): GovernedGuidelineAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedGuidelineAggregatorRefView => x !== null)
