import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedSuggestionAggregatorRefView, type GovernedSuggestionAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-suggestion-aggregator";
function mapRef(raw: unknown): GovernedSuggestionAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedSuggestionAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedSuggestionAggregatorEnvelope(payload: unknown): GovernedSuggestionAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedSuggestionAggregatorRefView => x !== null)
