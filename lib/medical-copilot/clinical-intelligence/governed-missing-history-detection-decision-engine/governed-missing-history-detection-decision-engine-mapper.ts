import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedMissingHistoryDetectionEngineEntryView, type GovernedMissingHistoryDetectionEngineResult } from "./governed-missing-history-detection-decision-engine";
function mapEntry(raw: unknown): GovernedMissingHistoryDetectionEngineEntryView | null {
...
export function mapGovernedMissingHistoryDetectionEngineEnvelope(payload: unknown): GovernedMissingHistoryDetectionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMissingHistoryDetectionEngineEntryView => e !== null);
