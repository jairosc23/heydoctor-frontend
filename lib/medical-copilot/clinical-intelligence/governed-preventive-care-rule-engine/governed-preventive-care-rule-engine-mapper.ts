import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedPreventiveCareRuleEngineEvaluationView, type GovernedPreventiveCareRuleEngineResult } from "./governed-preventive-care-rule-engine";

function mapEvaluation(raw: unknown): GovernedPreventiveCareRuleEngineEvaluationView | null {
...
export function mapGovernedPreventiveCareRuleEngineEnvelope(payload: unknown): GovernedPreventiveCareRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedPreventiveCareRuleEngineEvaluationView => e !== null);
