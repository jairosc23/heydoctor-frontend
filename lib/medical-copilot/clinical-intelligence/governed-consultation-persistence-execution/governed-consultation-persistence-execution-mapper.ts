import {
  ...
  type GovernedConsultationPersistenceExecutionComponentKey,
  type GovernedConsultationPersistenceExecutionComponentPresence,
  type GovernedConsultationPersistenceExecutionResult,
} from "./governed-consultation-persistence-execution";

const COMPONENT_DEFS: Array<{ key: GovernedConsultationPersistenceExecutionComponentKey; label: string }> = [
...
export function mapGovernedConsultationPersistenceExecutionEnvelope(payload: unknown): GovernedConsultationPersistenceExecutionResult | null {
  ...
  const components: GovernedConsultationPersistenceExecutionComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
  ...
  return {
    ...
    readOnly: true,
    persisted: false,
    writesEmr: data.writesEmr === true,
    writeAttempted: data.writeAttempted === true,
    writeExecuted: data.writeExecuted === true,
    repositoryInvoked: data.repositoryInvoked === true,
    rollbackExecuted: data.rollbackExecuted === true,
    entityPersisted: data.entityPersisted === true,
  };
}
