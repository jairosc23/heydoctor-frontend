import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedChildPughCalculationEngineEntryView, type GovernedChildPughCalculationEngineResult } from "./governed-child-pugh-calculation-engine";
function mapEntry(raw: unknown): GovernedChildPughCalculationEngineEntryView | null {
...
export function mapGovernedChildPughCalculationEngineEnvelope(payload: unknown): GovernedChildPughCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedChildPughCalculationEngineEntryView => e !== null);
