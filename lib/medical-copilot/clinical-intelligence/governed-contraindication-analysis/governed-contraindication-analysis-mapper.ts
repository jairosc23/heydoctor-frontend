import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedContraindicationAnalysisComponentKey, type GovernedContraindicationAnalysisComponentPresence, type GovernedContraindicationAnalysisResult } from "./governed-contraindication-analysis";

const COMPONENT_DEFS: Array<{ key: GovernedContraindicationAnalysisComponentKey; label: string }> = [
...
export function mapGovernedContraindicationAnalysisEnvelope(payload: unknown): GovernedContraindicationAnalysisResult | null {
  ...
  const components: GovernedContraindicationAnalysisComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "analysis") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
