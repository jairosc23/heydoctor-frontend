import { GOVERNED_DETERMINISTIC_CLINICAL_RULES_UI_GOVERNANCE, type GovernedContraindicationRuleEngineEvaluationView, type GovernedContraindicationRuleEngineResult } from "./governed-contraindication-rule-engine";

function mapEvaluation(raw: unknown): GovernedContraindicationRuleEngineEvaluationView | null {
...
export function mapGovernedContraindicationRuleEngineEnvelope(payload: unknown): GovernedContraindicationRuleEngineResult | null {
  ...
  const evaluations = evaluationsRaw.map(mapEvaluation).filter((e): e is GovernedContraindicationRuleEngineEvaluationView => e !== null);
