import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiabetesCareEngineComponentKey, type GovernedDiabetesCareEngineComponentPresence, type GovernedDiabetesCareEngineResult } from "./governed-diabetes-care-engine";
const COMPONENT_DEFS: Array<{ key: GovernedDiabetesCareEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedDiabetesCareEngineEnvelope(payload: unknown): GovernedDiabetesCareEngineResult | null {
  ...
  const components: GovernedDiabetesCareEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
