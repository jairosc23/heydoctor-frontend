...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-diabetes-care-engine";
import { mapGovernedDiabetesCareEngineEnvelope } from "./governed-diabetes-care-engine-mapper";
describe("GovernedDiabetesCareEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedDiabetesCareEngineEnvelope({
    ...
    assert.equal(mapGovernedDiabetesCareEngineEnvelope(null), null);
  });
});
