import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedNutritionKnowledgeEngineEntryView, type GovernedNutritionKnowledgeEngineResult } from "./governed-nutrition-knowledge-engine";

function mapEntry(raw: unknown): GovernedNutritionKnowledgeEngineEntryView | null {
...
export function mapGovernedNutritionKnowledgeEngineEnvelope(payload: unknown): GovernedNutritionKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedNutritionKnowledgeEngineEntryView => e !== null);
