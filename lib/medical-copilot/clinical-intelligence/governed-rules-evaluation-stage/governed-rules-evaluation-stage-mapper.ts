import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedRulesEvaluationStageResult, type GovernedRulesEvaluationStageStageView, type GovernedRulesEvaluationStageSurfaceRefView } from "./governed-rules-evaluation-stage";

function mapRef(raw: unknown): GovernedRulesEvaluationStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedRulesEvaluationStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedRulesEvaluationStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedRulesEvaluationStageEnvelope(payload: unknown): GovernedRulesEvaluationStageResult | null {
  ...
  let stages: GovernedRulesEvaluationStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedRulesEvaluationStageStageView => x !== null);
