import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticTimelineDiagnosticIntelEngineEntryView, type GovernedDiagnosticTimelineDiagnosticIntelEngineResult } from "./governed-diagnostic-timeline-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticTimelineDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticTimelineDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticTimelineDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticTimelineDiagnosticIntelEngineEntryView => e !== null);
