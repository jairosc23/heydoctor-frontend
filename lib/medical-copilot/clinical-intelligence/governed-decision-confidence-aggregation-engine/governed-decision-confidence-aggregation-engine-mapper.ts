import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDecisionConfidenceAggregationEngineEntryView, type GovernedDecisionConfidenceAggregationEngineResult } from "./governed-decision-confidence-aggregation-engine";
function mapEntry(raw: unknown): GovernedDecisionConfidenceAggregationEngineEntryView | null {
...
export function mapGovernedDecisionConfidenceAggregationEngineEnvelope(payload: unknown): GovernedDecisionConfidenceAggregationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDecisionConfidenceAggregationEngineEntryView => e !== null);
