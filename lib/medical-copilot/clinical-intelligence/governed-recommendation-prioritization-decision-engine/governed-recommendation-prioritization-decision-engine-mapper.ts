import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedRecommendationPrioritizationEngineEntryView, type GovernedRecommendationPrioritizationEngineResult } from "./governed-recommendation-prioritization-decision-engine";
function mapEntry(raw: unknown): GovernedRecommendationPrioritizationEngineEntryView | null {
...
export function mapGovernedRecommendationPrioritizationEngineEnvelope(payload: unknown): GovernedRecommendationPrioritizationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedRecommendationPrioritizationEngineEntryView => e !== null);
