import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedBmiCalculationEngineEntryView, type GovernedBmiCalculationEngineResult } from "./governed-bmi-calculation-engine";
function mapEntry(raw: unknown): GovernedBmiCalculationEngineEntryView | null {
...
export function mapGovernedBmiCalculationEngineEnvelope(payload: unknown): GovernedBmiCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedBmiCalculationEngineEntryView => e !== null);
