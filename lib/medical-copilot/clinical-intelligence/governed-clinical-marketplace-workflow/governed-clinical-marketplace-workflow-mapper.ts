import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalMarketplaceWorkflowRefView, type GovernedClinicalMarketplaceWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-marketplace-workflow";
function mapRef(raw: unknown): GovernedClinicalMarketplaceWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalMarketplaceWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalMarketplaceWorkflowEnvelope(payload: unknown): GovernedClinicalMarketplaceWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalMarketplaceWorkflowRefView => x !== null)
