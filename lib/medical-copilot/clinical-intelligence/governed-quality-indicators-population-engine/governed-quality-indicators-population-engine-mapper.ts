import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedQualityIndicatorsPopulationEngineEntryView, type GovernedQualityIndicatorsPopulationEngineResult } from "./governed-quality-indicators-population-engine";
function mapEntry(raw: unknown): GovernedQualityIndicatorsPopulationEngineEntryView | null {
...
export function mapGovernedQualityIndicatorsPopulationEngineEnvelope(payload: unknown): GovernedQualityIndicatorsPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedQualityIndicatorsPopulationEngineEntryView => e !== null);
