import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalTraceabilityEngineEntryView, type GovernedClinicalTraceabilityEngineResult } from "./governed-clinical-traceability-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalTraceabilityEngineEntryView | null {
...
export function mapGovernedClinicalTraceabilityEngineEnvelope(payload: unknown): GovernedClinicalTraceabilityEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalTraceabilityEngineEntryView => e !== null);
