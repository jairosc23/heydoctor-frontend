import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedAllergyKnowledgeEngineEntryView, type GovernedAllergyKnowledgeEngineResult } from "./governed-allergy-knowledge-engine";

function mapEntry(raw: unknown): GovernedAllergyKnowledgeEngineEntryView | null {
...
export function mapGovernedAllergyKnowledgeEngineEnvelope(payload: unknown): GovernedAllergyKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedAllergyKnowledgeEngineEntryView => e !== null);
