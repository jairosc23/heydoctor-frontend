import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedRuleAggregatorRefView, type GovernedRuleAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-rule-aggregator";
function mapRef(raw: unknown): GovernedRuleAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedRuleAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedRuleAggregatorEnvelope(payload: unknown): GovernedRuleAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedRuleAggregatorRefView => x !== null)
