import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGuidelineVersionEngineEntryView, type GovernedGuidelineVersionEngineResult } from "./governed-guideline-version-engine";

function mapEntry(raw: unknown): GovernedGuidelineVersionEngineEntryView | null {
...
export function mapGovernedGuidelineVersionEngineEnvelope(payload: unknown): GovernedGuidelineVersionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGuidelineVersionEngineEntryView => e !== null);
