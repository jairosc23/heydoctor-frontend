import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedVaccinationGuidelineEngineEntryView, type GovernedVaccinationGuidelineEngineResult } from "./governed-vaccination-guideline-engine";

function mapEntry(raw: unknown): GovernedVaccinationGuidelineEngineEntryView | null {
...
export function mapGovernedVaccinationGuidelineEngineEnvelope(payload: unknown): GovernedVaccinationGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedVaccinationGuidelineEngineEntryView => e !== null);
