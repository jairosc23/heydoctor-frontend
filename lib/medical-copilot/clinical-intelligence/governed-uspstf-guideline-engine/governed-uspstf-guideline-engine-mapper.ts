import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedUspstfGuidelineEngineEntryView, type GovernedUspstfGuidelineEngineResult } from "./governed-uspstf-guideline-engine";

function mapEntry(raw: unknown): GovernedUspstfGuidelineEngineEntryView | null {
...
export function mapGovernedUspstfGuidelineEngineEnvelope(payload: unknown): GovernedUspstfGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedUspstfGuidelineEngineEntryView => e !== null);
