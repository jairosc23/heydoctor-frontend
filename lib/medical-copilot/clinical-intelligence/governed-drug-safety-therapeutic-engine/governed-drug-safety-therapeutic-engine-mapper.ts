import { GOVERNED_THERAPEUTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDrugSafetyTherapeuticEngineEntryView, type GovernedDrugSafetyTherapeuticEngineResult } from "./governed-drug-safety-therapeutic-engine";
function mapEntry(raw: unknown): GovernedDrugSafetyTherapeuticEngineEntryView | null {
...
export function mapGovernedDrugSafetyTherapeuticEngineEnvelope(payload: unknown): GovernedDrugSafetyTherapeuticEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDrugSafetyTherapeuticEngineEntryView => e !== null);
