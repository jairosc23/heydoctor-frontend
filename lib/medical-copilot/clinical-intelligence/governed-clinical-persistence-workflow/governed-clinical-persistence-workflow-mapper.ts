import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalPersistenceWorkflowRefView, type GovernedClinicalPersistenceWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-persistence-workflow";
function mapRef(raw: unknown): GovernedClinicalPersistenceWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPersistenceWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalPersistenceWorkflowEnvelope(payload: unknown): GovernedClinicalPersistenceWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalPersistenceWorkflowRefView => x !== null)
