import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedClinicalReasoningPipelineResult, type GovernedClinicalReasoningPipelineStageView, type GovernedClinicalReasoningPipelineSurfaceRefView } from "./governed-clinical-reasoning-pipeline";

function mapRef(raw: unknown): GovernedClinicalReasoningPipelineSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedClinicalReasoningPipelineStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalReasoningPipelineSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalReasoningPipelineEnvelope(payload: unknown): GovernedClinicalReasoningPipelineResult | null {
  ...
  let stages: GovernedClinicalReasoningPipelineStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedClinicalReasoningPipelineStageView => x !== null);
