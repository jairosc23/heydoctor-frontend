import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedWellsPeCalculationEngineEntryView, type GovernedWellsPeCalculationEngineResult } from "./governed-wells-pe-calculation-engine";
function mapEntry(raw: unknown): GovernedWellsPeCalculationEngineEntryView | null {
...
export function mapGovernedWellsPeCalculationEngineEnvelope(payload: unknown): GovernedWellsPeCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedWellsPeCalculationEngineEntryView => e !== null);
