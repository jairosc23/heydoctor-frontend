import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDifferentialDiagnosisRankingEngineEntryView, type GovernedDifferentialDiagnosisRankingEngineResult } from "./governed-differential-diagnosis-ranking-decision-engine";
function mapEntry(raw: unknown): GovernedDifferentialDiagnosisRankingEngineEntryView | null {
...
export function mapGovernedDifferentialDiagnosisRankingEngineEnvelope(payload: unknown): GovernedDifferentialDiagnosisRankingEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDifferentialDiagnosisRankingEngineEntryView => e !== null);
