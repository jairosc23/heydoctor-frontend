import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticGovernanceDiagnosticIntelEngineEntryView, type GovernedDiagnosticGovernanceDiagnosticIntelEngineResult } from "./governed-diagnostic-governance-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticGovernanceDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticGovernanceDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticGovernanceDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticGovernanceDiagnosticIntelEngineEntryView => e !== null);
