...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-womens-health-review-engine";
import { mapGovernedWomensHealthReviewEngineEnvelope } from "./governed-womens-health-review-engine-mapper";
describe("GovernedWomensHealthReviewEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedWomensHealthReviewEngineEnvelope({
    ...
    assert.equal(mapGovernedWomensHealthReviewEngineEnvelope(null), null);
  });
});
