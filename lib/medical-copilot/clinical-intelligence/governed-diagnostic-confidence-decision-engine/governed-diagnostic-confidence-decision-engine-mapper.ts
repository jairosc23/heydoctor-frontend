import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDiagnosticConfidenceEngineEntryView, type GovernedDiagnosticConfidenceEngineResult } from "./governed-diagnostic-confidence-decision-engine";
function mapEntry(raw: unknown): GovernedDiagnosticConfidenceEngineEntryView | null {
...
export function mapGovernedDiagnosticConfidenceEngineEnvelope(payload: unknown): GovernedDiagnosticConfidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticConfidenceEngineEntryView => e !== null);
