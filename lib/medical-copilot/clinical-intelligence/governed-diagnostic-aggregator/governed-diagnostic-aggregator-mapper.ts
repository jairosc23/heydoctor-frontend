import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedDiagnosticAggregatorRefView, type GovernedDiagnosticAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-diagnostic-aggregator";
function mapRef(raw: unknown): GovernedDiagnosticAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedDiagnosticAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedDiagnosticAggregatorEnvelope(payload: unknown): GovernedDiagnosticAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedDiagnosticAggregatorRefView => x !== null)
