import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedNafldScoreCalculationEngineEntryView, type GovernedNafldScoreCalculationEngineResult } from "./governed-nafld-score-calculation-engine";
function mapEntry(raw: unknown): GovernedNafldScoreCalculationEngineEntryView | null {
...
export function mapGovernedNafldScoreCalculationEngineEnvelope(payload: unknown): GovernedNafldScoreCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedNafldScoreCalculationEngineEntryView => e !== null);
