import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalTransparencyEngineEntryView, type GovernedClinicalTransparencyEngineResult } from "./governed-clinical-transparency-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalTransparencyEngineEntryView | null {
...
export function mapGovernedClinicalTransparencyEngineEnvelope(payload: unknown): GovernedClinicalTransparencyEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalTransparencyEngineEntryView => e !== null);
