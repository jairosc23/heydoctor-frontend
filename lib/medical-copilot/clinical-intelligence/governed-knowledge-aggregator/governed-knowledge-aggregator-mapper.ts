import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedKnowledgeAggregatorRefView, type GovernedKnowledgeAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-knowledge-aggregator";
function mapRef(raw: unknown): GovernedKnowledgeAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedKnowledgeAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedKnowledgeAggregatorEnvelope(payload: unknown): GovernedKnowledgeAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedKnowledgeAggregatorRefView => x !== null)
