...
import { GOVERNED_CLINICAL_SUGGESTIONS_UI_GOVERNANCE } from "./governed-follow-up-suggestion";
import { mapGovernedFollowUpSuggestionEnvelope } from "./governed-follow-up-suggestion-mapper";

describe("GovernedFollowUpSuggestion mapper", () => {
  it("maps HITL suggestion surface without write flags", () => {
    const mapped = mapGovernedFollowUpSuggestionEnvelope({
    ...
    assert.equal(mapGovernedFollowUpSuggestionEnvelope(null), null);
  });
});
