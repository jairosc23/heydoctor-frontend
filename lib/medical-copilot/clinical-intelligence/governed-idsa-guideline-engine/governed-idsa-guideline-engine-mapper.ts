import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedIdsaGuidelineEngineEntryView, type GovernedIdsaGuidelineEngineResult } from "./governed-idsa-guideline-engine";

function mapEntry(raw: unknown): GovernedIdsaGuidelineEngineEntryView | null {
...
export function mapGovernedIdsaGuidelineEngineEnvelope(payload: unknown): GovernedIdsaGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedIdsaGuidelineEngineEntryView => e !== null);
