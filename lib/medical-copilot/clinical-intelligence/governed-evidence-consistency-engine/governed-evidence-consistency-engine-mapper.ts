import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceConsistencyEngineEntryView, type GovernedEvidenceConsistencyEngineResult } from "./governed-evidence-consistency-engine";

function mapEntry(raw: unknown): GovernedEvidenceConsistencyEngineEntryView | null {
...
export function mapGovernedEvidenceConsistencyEngineEnvelope(payload: unknown): GovernedEvidenceConsistencyEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceConsistencyEngineEntryView => e !== null);
