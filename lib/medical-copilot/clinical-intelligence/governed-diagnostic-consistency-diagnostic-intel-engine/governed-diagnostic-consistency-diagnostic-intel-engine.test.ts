import assert from "node:assert/strict";
import { describe, it } from "node:test";
import { mapGovernedDiagnosticConsistencyDiagnosticIntelEngineEnvelope } from "./governed-diagnostic-consistency-diagnostic-intel-engine-mapper";
describe("Diagnostic Consistency", () => {
  it("maps envelope with HITL seals", () => {
    const mapped = mapGovernedDiagnosticConsistencyDiagnosticIntelEngineEnvelope({ status: "ok", data: { status: "PROPOSED_FOR_PHYSICIAN_REVIEW", title: "Diagnostic Consistency", applicableCount: 1,
      entries: [{ entryId: "e1", entryTitle: "Entry", domain: "d", topic: "t", summary: "s", explanation: "x", evidenceRefs: [], diagnosticRole: "r", applicability: "APPLICABLE", confidence: "medium" }],
      enginesPresent: [], governance: { requiresPhysicianReview: true, usesLlm: false } } });
    assert.ok(mapped); assert.equal(mapped!.usesLlm, false); assert.equal(mapped!.entries.length, 1);
  });
});
