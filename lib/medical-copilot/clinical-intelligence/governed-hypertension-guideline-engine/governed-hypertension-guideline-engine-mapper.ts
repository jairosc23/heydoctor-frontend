import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedHypertensionGuidelineEngineEntryView, type GovernedHypertensionGuidelineEngineResult } from "./governed-hypertension-guideline-engine";

function mapEntry(raw: unknown): GovernedHypertensionGuidelineEngineEntryView | null {
...
export function mapGovernedHypertensionGuidelineEngineEnvelope(payload: unknown): GovernedHypertensionGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedHypertensionGuidelineEngineEntryView => e !== null);
