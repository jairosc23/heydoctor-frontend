import { GOVERNED_ORDERS_PERSISTENCE_EXECUTION_GOVERNANCE, type GovernedOrdersPersistenceExecutionComponentKey, type GovernedOrdersPersistenceExecutionComponentPresence, type GovernedOrdersPersistenceExecutionResult } from "./governed-orders-persistence-execution";
const COMPONENT_DEFS: Array<{ key: GovernedOrdersPersistenceExecutionComponentKey; label: string }> = [
...
export function mapGovernedOrdersPersistenceExecutionEnvelope(payload: unknown): GovernedOrdersPersistenceExecutionResult | null {
  ...
  const components: GovernedOrdersPersistenceExecutionComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
  ...
  return {
    ...
    components, governance: { ...GOVERNED_ORDERS_PERSISTENCE_EXECUTION_GOVERNANCE },
