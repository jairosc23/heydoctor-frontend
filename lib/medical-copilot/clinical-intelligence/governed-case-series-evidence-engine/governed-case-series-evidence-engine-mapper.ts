import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedCaseSeriesEvidenceEngineEntryView, type GovernedCaseSeriesEvidenceEngineResult } from "./governed-case-series-evidence-engine";

function mapEntry(raw: unknown): GovernedCaseSeriesEvidenceEngineEntryView | null {
...
export function mapGovernedCaseSeriesEvidenceEngineEnvelope(payload: unknown): GovernedCaseSeriesEvidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCaseSeriesEvidenceEngineEntryView => e !== null);
