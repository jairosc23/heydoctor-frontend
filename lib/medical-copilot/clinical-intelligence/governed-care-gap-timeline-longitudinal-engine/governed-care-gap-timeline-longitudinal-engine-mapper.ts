import { GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedCareGapTimelineLongitudinalEngineEntryView, type GovernedCareGapTimelineLongitudinalEngineResult } from "./governed-care-gap-timeline-longitudinal-engine";
function mapEntry(raw: unknown): GovernedCareGapTimelineLongitudinalEngineEntryView | null {
...
export function mapGovernedCareGapTimelineLongitudinalEngineEnvelope(payload: unknown): GovernedCareGapTimelineLongitudinalEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedCareGapTimelineLongitudinalEngineEntryView => e !== null);
