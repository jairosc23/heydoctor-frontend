import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedPreventiveScreeningSuggestionsComponentKey, type GovernedPreventiveScreeningSuggestionsComponentPresence, type GovernedPreventiveScreeningSuggestionsResult } from "./governed-preventive-screening-suggestions";

const COMPONENT_DEFS: Array<{ key: GovernedPreventiveScreeningSuggestionsComponentKey; label: string }> = [
...
export function mapGovernedPreventiveScreeningSuggestionsEnvelope(payload: unknown): GovernedPreventiveScreeningSuggestionsResult | null {
  ...
  const components: GovernedPreventiveScreeningSuggestionsComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "screening") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
