import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedPediatricSafetyEngineComponentKey, type GovernedPediatricSafetyEngineComponentPresence, type GovernedPediatricSafetyEngineResult } from "./governed-pediatric-safety-engine";
const COMPONENT_DEFS: Array<{ key: GovernedPediatricSafetyEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedPediatricSafetyEngineEnvelope(payload: unknown): GovernedPediatricSafetyEngineResult | null {
  ...
  const components: GovernedPediatricSafetyEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
