import {
  ...
  type GovernedOrdersPersistenceBridgeComponentKey,
  type GovernedOrdersPersistenceBridgeComponentPresence,
  type GovernedOrdersPersistenceBridgeResult,
...
export function mapGovernedOrdersPersistenceBridgeEnvelope(
  payload: unknown,
): GovernedOrdersPersistenceBridgeResult | null {
  ...
  const components: GovernedOrdersPersistenceBridgeComponentPresence[] = COMPONENT_DEFS.map(
    ({ key, label }) => {
      let present = false;
