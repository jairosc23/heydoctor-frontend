import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedPolypharmacyAnalysisEngineComponentKey, type GovernedPolypharmacyAnalysisEngineComponentPresence, type GovernedPolypharmacyAnalysisEngineResult } from "./governed-polypharmacy-analysis-engine";
const COMPONENT_DEFS: Array<{ key: GovernedPolypharmacyAnalysisEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedPolypharmacyAnalysisEngineEnvelope(payload: unknown): GovernedPolypharmacyAnalysisEngineResult | null {
  ...
  const components: GovernedPolypharmacyAnalysisEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
