...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-polypharmacy-analysis-engine";
import { mapGovernedPolypharmacyAnalysisEngineEnvelope } from "./governed-polypharmacy-analysis-engine-mapper";
describe("GovernedPolypharmacyAnalysisEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedPolypharmacyAnalysisEngineEnvelope({
    ...
    assert.equal(mapGovernedPolypharmacyAnalysisEngineEnvelope(null), null);
  });
});
