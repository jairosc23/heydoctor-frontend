...
describe("Clinical Consultation Workflow", () => {
  it("maps workflow envelope with stages and HITL seals", () => {
    const mapped = mapGovernedClinicalConsultationWorkflowEnvelope({ status: "ok", data: {
      status: "READY_FOR_PHYSICIAN_REVIEW",
      title: "Clinical Consultation Workflow",
      workflowId: "e2e_wf_1",
      workflowType: "clinical_consultation_workflow",
      summary: "E2E mock",
      ...
      pendingStages: [],
      workflowCount: 1,
      workflows: [{ order: 1, workflowId: "e2e_wf_1", workflowType: "clinical_consultation_workflow", title: "Clinical Consultation Workflow", summary: "E2E", sourcePackages: ["medical_copilot_session"], surfaceRefs: [{ sourcePackage: "medical_copilot_session", surfaceKind: "workflow", present: true, metricLabel: "ready", metricValue: 1 }], currentStage: "hitl_review", nextStage: null, completedStages: ["session_bind"], pendingStages: [] }],
    ...
    assert.equal(mapped.workflowType, "clinical_consultation_workflow");
