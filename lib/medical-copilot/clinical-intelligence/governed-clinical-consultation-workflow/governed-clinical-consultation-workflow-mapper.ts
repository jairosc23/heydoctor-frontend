import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalConsultationWorkflowRefView, type GovernedClinicalConsultationWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-consultation-workflow";
function mapRef(raw: unknown): GovernedClinicalConsultationWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalConsultationWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalConsultationWorkflowEnvelope(payload: unknown): GovernedClinicalConsultationWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalConsultationWorkflowRefView => x !== null)
