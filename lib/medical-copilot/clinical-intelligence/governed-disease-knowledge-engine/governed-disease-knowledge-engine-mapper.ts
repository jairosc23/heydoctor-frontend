import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedDiseaseKnowledgeEngineEntryView, type GovernedDiseaseKnowledgeEngineResult } from "./governed-disease-knowledge-engine";

function mapEntry(raw: unknown): GovernedDiseaseKnowledgeEngineEntryView | null {
...
export function mapGovernedDiseaseKnowledgeEngineEnvelope(payload: unknown): GovernedDiseaseKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiseaseKnowledgeEngineEntryView => e !== null);
