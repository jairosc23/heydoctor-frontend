import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedEgfrCalculationEngineEntryView, type GovernedEgfrCalculationEngineResult } from "./governed-egfr-calculation-engine";
function mapEntry(raw: unknown): GovernedEgfrCalculationEngineEntryView | null {
...
export function mapGovernedEgfrCalculationEngineEnvelope(payload: unknown): GovernedEgfrCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEgfrCalculationEngineEntryView => e !== null);
