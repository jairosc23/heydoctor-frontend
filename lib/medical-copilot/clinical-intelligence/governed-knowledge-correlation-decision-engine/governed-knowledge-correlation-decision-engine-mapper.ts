import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedKnowledgeCorrelationEngineEntryView, type GovernedKnowledgeCorrelationEngineResult } from "./governed-knowledge-correlation-decision-engine";
function mapEntry(raw: unknown): GovernedKnowledgeCorrelationEngineEntryView | null {
...
export function mapGovernedKnowledgeCorrelationEngineEnvelope(payload: unknown): GovernedKnowledgeCorrelationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedKnowledgeCorrelationEngineEntryView => e !== null);
