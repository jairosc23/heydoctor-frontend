...
import { GOVERNED_CLINICAL_SUGGESTIONS_UI_GOVERNANCE } from "./governed-clinical-suggestion-package";
import { mapGovernedClinicalSuggestionPackageEnvelope } from "./governed-clinical-suggestion-package-mapper";

describe("GovernedClinicalSuggestionPackage mapper", () => {
  it("maps HITL suggestion surface without write flags", () => {
    const mapped = mapGovernedClinicalSuggestionPackageEnvelope({
      status: "PROPOSED_FOR_PHYSICIAN_REVIEW",
      title: "Governed Clinical Recommendation Package",
      kind: "package",
    ...
    assert.equal(mapGovernedClinicalSuggestionPackageEnvelope(null), null);
  });
});
