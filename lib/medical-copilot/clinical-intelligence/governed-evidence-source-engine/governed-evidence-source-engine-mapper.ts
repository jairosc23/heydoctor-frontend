import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceSourceEngineEntryView, type GovernedEvidenceSourceEngineResult } from "./governed-evidence-source-engine";

function mapEntry(raw: unknown): GovernedEvidenceSourceEngineEntryView | null {
...
export function mapGovernedEvidenceSourceEngineEnvelope(payload: unknown): GovernedEvidenceSourceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceSourceEngineEntryView => e !== null);
