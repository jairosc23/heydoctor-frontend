import { GOVERNED_CLINICAL_LONGITUDINAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedProcedureTimelineLongitudinalEngineEntryView, type GovernedProcedureTimelineLongitudinalEngineResult } from "./governed-procedure-timeline-longitudinal-engine";
function mapEntry(raw: unknown): GovernedProcedureTimelineLongitudinalEngineEntryView | null {
...
export function mapGovernedProcedureTimelineLongitudinalEngineEnvelope(payload: unknown): GovernedProcedureTimelineLongitudinalEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedProcedureTimelineLongitudinalEngineEntryView => e !== null);
