...
import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-allergy-cross-check";
import { mapGovernedAllergyCrossCheckEnvelope } from "./governed-allergy-cross-check-mapper";

describe("GovernedAllergyCrossCheck mapper", () => {
  it("maps HITL intelligence surface without write flags", () => {
    const mapped = mapGovernedAllergyCrossCheckEnvelope({
    ...
    assert.equal(mapGovernedAllergyCrossCheckEnvelope(null), null);
  });
});
