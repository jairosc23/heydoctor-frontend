import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedClinicalOrchestratorRuntimeRefView, type GovernedClinicalOrchestratorRuntimeResult, type GovernedOrchestratorAggregatorView } from "./governed-clinical-orchestrator-runtime";
function mapRef(raw: unknown): GovernedClinicalOrchestratorRuntimeRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalOrchestratorRuntimeRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalOrchestratorRuntimeEnvelope(payload: unknown): GovernedClinicalOrchestratorRuntimeResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalOrchestratorRuntimeRefView => x !== null)
