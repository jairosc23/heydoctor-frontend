import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedPopulationAggregatorRefView, type GovernedPopulationAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-population-aggregator";
function mapRef(raw: unknown): GovernedPopulationAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedPopulationAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedPopulationAggregatorEnvelope(payload: unknown): GovernedPopulationAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedPopulationAggregatorRefView => x !== null)
