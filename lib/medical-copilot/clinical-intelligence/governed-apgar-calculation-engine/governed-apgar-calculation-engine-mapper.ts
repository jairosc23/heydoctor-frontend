import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedApgarCalculationEngineEntryView, type GovernedApgarCalculationEngineResult } from "./governed-apgar-calculation-engine";
function mapEntry(raw: unknown): GovernedApgarCalculationEngineEntryView | null {
...
export function mapGovernedApgarCalculationEngineEnvelope(payload: unknown): GovernedApgarCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedApgarCalculationEngineEntryView => e !== null);
