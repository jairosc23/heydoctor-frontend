import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceVersioningEngineEntryView, type GovernedEvidenceVersioningEngineResult } from "./governed-evidence-versioning-engine";

function mapEntry(raw: unknown): GovernedEvidenceVersioningEngineEntryView | null {
...
export function mapGovernedEvidenceVersioningEngineEnvelope(payload: unknown): GovernedEvidenceVersioningEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceVersioningEngineEntryView => e !== null);
