...
describe("GovernedClinicalIntakeStage mapper", () => {
  it("maps pipeline stage without write/LLM flags", () => {
    const mapped = mapGovernedClinicalIntakeStageEnvelope({
    ...
    assert.equal(mapGovernedClinicalIntakeStageEnvelope(null), null);
  });
});
