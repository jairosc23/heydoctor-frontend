import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedClinicalIntakeStageResult, type GovernedClinicalIntakeStageStageView, type GovernedClinicalIntakeStageSurfaceRefView } from "./governed-clinical-intake-stage";

function mapRef(raw: unknown): GovernedClinicalIntakeStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedClinicalIntakeStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalIntakeStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalIntakeStageEnvelope(payload: unknown): GovernedClinicalIntakeStageResult | null {
  ...
  let stages: GovernedClinicalIntakeStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedClinicalIntakeStageStageView => x !== null);
