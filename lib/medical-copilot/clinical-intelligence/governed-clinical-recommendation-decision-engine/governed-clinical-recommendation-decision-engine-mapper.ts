import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalRecommendationEngineEntryView, type GovernedClinicalRecommendationEngineResult } from "./governed-clinical-recommendation-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalRecommendationEngineEntryView | null {
...
export function mapGovernedClinicalRecommendationEngineEnvelope(payload: unknown): GovernedClinicalRecommendationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalRecommendationEngineEntryView => e !== null);
