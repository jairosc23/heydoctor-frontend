import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedClinicalAlertCenterComponentKey, type GovernedClinicalAlertCenterComponentPresence, type GovernedClinicalAlertCenterResult } from "./governed-clinical-alert-center";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalAlertCenterComponentKey; label: string }> = [
...
export function mapGovernedClinicalAlertCenterEnvelope(payload: unknown): GovernedClinicalAlertCenterResult | null {
  ...
  const components: GovernedClinicalAlertCenterComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "alerts") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
