import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedAscvdCalculationEngineEntryView, type GovernedAscvdCalculationEngineResult } from "./governed-ascvd-calculation-engine";
function mapEntry(raw: unknown): GovernedAscvdCalculationEngineEntryView | null {
...
export function mapGovernedAscvdCalculationEngineEnvelope(payload: unknown): GovernedAscvdCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedAscvdCalculationEngineEntryView => e !== null);
