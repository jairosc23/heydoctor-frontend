import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedTherapeuticAggregatorRefView, type GovernedTherapeuticAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-therapeutic-aggregator";
function mapRef(raw: unknown): GovernedTherapeuticAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedTherapeuticAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedTherapeuticAggregatorEnvelope(payload: unknown): GovernedTherapeuticAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedTherapeuticAggregatorRefView => x !== null)
