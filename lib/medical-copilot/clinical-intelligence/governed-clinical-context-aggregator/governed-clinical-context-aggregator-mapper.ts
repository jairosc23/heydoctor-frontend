import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedClinicalContextAggregatorRefView, type GovernedClinicalContextAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-clinical-context-aggregator";
function mapRef(raw: unknown): GovernedClinicalContextAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalContextAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalContextAggregatorEnvelope(payload: unknown): GovernedClinicalContextAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalContextAggregatorRefView => x !== null)
