import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedPhysicianReviewStageResult, type GovernedPhysicianReviewStageStageView, type GovernedPhysicianReviewStageSurfaceRefView } from "./governed-physician-review-stage";

function mapRef(raw: unknown): GovernedPhysicianReviewStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedPhysicianReviewStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedPhysicianReviewStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedPhysicianReviewStageEnvelope(payload: unknown): GovernedPhysicianReviewStageResult | null {
  ...
  let stages: GovernedPhysicianReviewStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedPhysicianReviewStageStageView => x !== null);
