import { GOVERNED_THERAPEUTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDrugMonitoringTherapeuticEngineEntryView, type GovernedDrugMonitoringTherapeuticEngineResult } from "./governed-drug-monitoring-therapeutic-engine";
function mapEntry(raw: unknown): GovernedDrugMonitoringTherapeuticEngineEntryView | null {
...
export function mapGovernedDrugMonitoringTherapeuticEngineEnvelope(payload: unknown): GovernedDrugMonitoringTherapeuticEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDrugMonitoringTherapeuticEngineEntryView => e !== null);
