...
describe("Missing Imaging Detection Engine", () => {
  it("maps envelope with HITL seals", () => {
    const mapped = mapGovernedMissingImagingDetectionEngineEnvelope({ status: "ok", data: { status: "PROPOSED_FOR_PHYSICIAN_REVIEW", title: "Missing Imaging Detection Engine", applicableCount: 1, entries: [{ entryId: "e1", entryTitle: "Entry", domain: "missing_imaging_detection", topic: "t", summary: "s", explanation: "x", evidenceRefs: ["medical_copilot_session"], decisionRole: "runtime", applicability: "APPLICABLE", confidence: "medium" }], enginesPresent: [], governance: { requiresPhysicianReview: true, usesLlm: false } } });
    assert.ok(mapped);
