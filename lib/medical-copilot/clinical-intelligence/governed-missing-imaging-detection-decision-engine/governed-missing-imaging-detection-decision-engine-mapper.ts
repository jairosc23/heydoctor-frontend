import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedMissingImagingDetectionEngineEntryView, type GovernedMissingImagingDetectionEngineResult } from "./governed-missing-imaging-detection-decision-engine";
function mapEntry(raw: unknown): GovernedMissingImagingDetectionEngineEntryView | null {
...
export function mapGovernedMissingImagingDetectionEngineEnvelope(payload: unknown): GovernedMissingImagingDetectionEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMissingImagingDetectionEngineEntryView => e !== null);
