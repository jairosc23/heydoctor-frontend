import {
  ...
  type GovernedPrescriptionPersistenceBridgeComponentKey,
  type GovernedPrescriptionPersistenceBridgeComponentPresence,
  type GovernedPrescriptionPersistenceBridgeResult,
...
export function mapGovernedPrescriptionPersistenceBridgeEnvelope(
  payload: unknown,
): GovernedPrescriptionPersistenceBridgeResult | null {
  ...
  const components: GovernedPrescriptionPersistenceBridgeComponentPresence[] = COMPONENT_DEFS.map(
    ({ key, label }) => {
      let present = false;
