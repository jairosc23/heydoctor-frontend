import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalSafetyWorkflowRefView, type GovernedClinicalSafetyWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-safety-workflow";
function mapRef(raw: unknown): GovernedClinicalSafetyWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalSafetyWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalSafetyWorkflowEnvelope(payload: unknown): GovernedClinicalSafetyWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalSafetyWorkflowRefView => x !== null)
