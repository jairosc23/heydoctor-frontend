...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-clinical-explainability";
import { mapGovernedClinicalExplainabilityEnvelope } from "./governed-clinical-explainability-mapper";

describe("GovernedClinicalExplainability mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedClinicalExplainabilityEnvelope({
    ...
    assert.equal(mapGovernedClinicalExplainabilityEnvelope(null), null);
  });
});
