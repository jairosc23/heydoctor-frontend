import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedClinicalExplainabilityComponentKey, type GovernedClinicalExplainabilityComponentPresence, type GovernedClinicalExplainabilityResult } from "./governed-clinical-explainability";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalExplainabilityComponentKey; label: string }> = [
...
export function mapGovernedClinicalExplainabilityEnvelope(payload: unknown): GovernedClinicalExplainabilityResult | null {
  ...
  const components: GovernedClinicalExplainabilityComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "explainability") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
