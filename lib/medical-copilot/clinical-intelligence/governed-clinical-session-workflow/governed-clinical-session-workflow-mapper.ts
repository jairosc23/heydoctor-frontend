import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalSessionWorkflowRefView, type GovernedClinicalSessionWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-session-workflow";
function mapRef(raw: unknown): GovernedClinicalSessionWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalSessionWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalSessionWorkflowEnvelope(payload: unknown): GovernedClinicalSessionWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalSessionWorkflowRefView => x !== null)
