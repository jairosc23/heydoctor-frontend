import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedBsaCalculationEngineEntryView, type GovernedBsaCalculationEngineResult } from "./governed-bsa-calculation-engine";
function mapEntry(raw: unknown): GovernedBsaCalculationEngineEntryView | null {
...
export function mapGovernedBsaCalculationEngineEnvelope(payload: unknown): GovernedBsaCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedBsaCalculationEngineEntryView => e !== null);
