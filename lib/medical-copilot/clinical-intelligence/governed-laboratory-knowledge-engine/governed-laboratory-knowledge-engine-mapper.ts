import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedLaboratoryKnowledgeEngineEntryView, type GovernedLaboratoryKnowledgeEngineResult } from "./governed-laboratory-knowledge-engine";

function mapEntry(raw: unknown): GovernedLaboratoryKnowledgeEngineEntryView | null {
...
export function mapGovernedLaboratoryKnowledgeEngineEnvelope(payload: unknown): GovernedLaboratoryKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedLaboratoryKnowledgeEngineEntryView => e !== null);
