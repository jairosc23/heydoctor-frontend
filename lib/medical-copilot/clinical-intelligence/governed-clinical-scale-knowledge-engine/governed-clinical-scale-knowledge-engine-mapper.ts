import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedClinicalScaleKnowledgeEngineEntryView, type GovernedClinicalScaleKnowledgeEngineResult } from "./governed-clinical-scale-knowledge-engine";

function mapEntry(raw: unknown): GovernedClinicalScaleKnowledgeEngineEntryView | null {
...
export function mapGovernedClinicalScaleKnowledgeEngineEnvelope(payload: unknown): GovernedClinicalScaleKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalScaleKnowledgeEngineEntryView => e !== null);
