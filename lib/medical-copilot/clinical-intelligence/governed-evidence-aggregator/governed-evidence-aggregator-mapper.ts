import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedEvidenceAggregatorRefView, type GovernedEvidenceAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-evidence-aggregator";
function mapRef(raw: unknown): GovernedEvidenceAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedEvidenceAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedEvidenceAggregatorEnvelope(payload: unknown): GovernedEvidenceAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedEvidenceAggregatorRefView => x !== null)
