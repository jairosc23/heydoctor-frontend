import {
  ...
  type GovernedSoapPersistenceBridgeComponentKey,
  type GovernedSoapPersistenceBridgeComponentPresence,
  type GovernedSoapPersistenceBridgeResult,
...
export function mapGovernedSoapPersistenceBridgeEnvelope(
  payload: unknown,
): GovernedSoapPersistenceBridgeResult | null {
  ...
  const components: GovernedSoapPersistenceBridgeComponentPresence[] = COMPONENT_DEFS.map(
    ({ key, label }) => {
      let present = false;
