import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedGinaGuidelineEngineEntryView, type GovernedGinaGuidelineEngineResult } from "./governed-gina-guideline-engine";

function mapEntry(raw: unknown): GovernedGinaGuidelineEngineEntryView | null {
...
export function mapGovernedGinaGuidelineEngineEnvelope(payload: unknown): GovernedGinaGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGinaGuidelineEngineEntryView => e !== null);
