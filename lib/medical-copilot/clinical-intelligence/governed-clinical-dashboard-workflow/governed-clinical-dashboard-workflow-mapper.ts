import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalDashboardWorkflowRefView, type GovernedClinicalDashboardWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-dashboard-workflow";
function mapRef(raw: unknown): GovernedClinicalDashboardWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDashboardWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalDashboardWorkflowEnvelope(payload: unknown): GovernedClinicalDashboardWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDashboardWorkflowRefView => x !== null)
