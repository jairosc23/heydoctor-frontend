import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedTimiCalculationEngineEntryView, type GovernedTimiCalculationEngineResult } from "./governed-timi-calculation-engine";
function mapEntry(raw: unknown): GovernedTimiCalculationEngineEntryView | null {
...
export function mapGovernedTimiCalculationEngineEnvelope(payload: unknown): GovernedTimiCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedTimiCalculationEngineEntryView => e !== null);
