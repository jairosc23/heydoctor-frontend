import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedRiskScoreKnowledgeEngineEntryView, type GovernedRiskScoreKnowledgeEngineResult } from "./governed-risk-score-knowledge-engine";

function mapEntry(raw: unknown): GovernedRiskScoreKnowledgeEngineEntryView | null {
...
export function mapGovernedRiskScoreKnowledgeEngineEnvelope(payload: unknown): GovernedRiskScoreKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedRiskScoreKnowledgeEngineEntryView => e !== null);
