import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedChronicDiseaseKnowledgeEngineEntryView, type GovernedChronicDiseaseKnowledgeEngineResult } from "./governed-chronic-disease-knowledge-engine";

function mapEntry(raw: unknown): GovernedChronicDiseaseKnowledgeEngineEntryView | null {
...
export function mapGovernedChronicDiseaseKnowledgeEngineEnvelope(payload: unknown): GovernedChronicDiseaseKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedChronicDiseaseKnowledgeEngineEntryView => e !== null);
