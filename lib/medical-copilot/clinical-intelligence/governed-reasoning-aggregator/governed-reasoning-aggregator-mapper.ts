import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedReasoningAggregatorRefView, type GovernedReasoningAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-reasoning-aggregator";
function mapRef(raw: unknown): GovernedReasoningAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedReasoningAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedReasoningAggregatorEnvelope(payload: unknown): GovernedReasoningAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedReasoningAggregatorRefView => x !== null)
