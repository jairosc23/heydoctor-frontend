import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedGovernanceAggregatorRefView, type GovernedGovernanceAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-governance-aggregator";
function mapRef(raw: unknown): GovernedGovernanceAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedGovernanceAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedGovernanceAggregatorEnvelope(payload: unknown): GovernedGovernanceAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedGovernanceAggregatorRefView => x !== null)
