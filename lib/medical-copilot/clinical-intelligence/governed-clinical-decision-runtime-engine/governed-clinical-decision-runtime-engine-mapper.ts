import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalDecisionRuntimeEngineEntryView, type GovernedClinicalDecisionRuntimeEngineResult } from "./governed-clinical-decision-runtime-engine";
function mapEntry(raw: unknown): GovernedClinicalDecisionRuntimeEngineEntryView | null {
...
export function mapGovernedClinicalDecisionRuntimeEngineEnvelope(payload: unknown): GovernedClinicalDecisionRuntimeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalDecisionRuntimeEngineEntryView => e !== null);
