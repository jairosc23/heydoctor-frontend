import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedUspstfEvidenceEngineEntryView, type GovernedUspstfEvidenceEngineResult } from "./governed-uspstf-evidence-engine";

function mapEntry(raw: unknown): GovernedUspstfEvidenceEngineEntryView | null {
...
export function mapGovernedUspstfEvidenceEngineEnvelope(payload: unknown): GovernedUspstfEvidenceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedUspstfEvidenceEngineEntryView => e !== null);
