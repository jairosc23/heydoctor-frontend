import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedLongitudinalAggregatorRefView, type GovernedLongitudinalAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-longitudinal-aggregator";
function mapRef(raw: unknown): GovernedLongitudinalAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedLongitudinalAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedLongitudinalAggregatorEnvelope(payload: unknown): GovernedLongitudinalAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedLongitudinalAggregatorRefView => x !== null)
