import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedDecisionAggregatorRefView, type GovernedDecisionAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-decision-aggregator";
function mapRef(raw: unknown): GovernedDecisionAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedDecisionAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedDecisionAggregatorEnvelope(payload: unknown): GovernedDecisionAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedDecisionAggregatorRefView => x !== null)
