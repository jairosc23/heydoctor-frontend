import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedClinicalBenchmarkPopulationEngineEntryView, type GovernedClinicalBenchmarkPopulationEngineResult } from "./governed-clinical-benchmark-population-engine";
function mapEntry(raw: unknown): GovernedClinicalBenchmarkPopulationEngineEntryView | null {
...
export function mapGovernedClinicalBenchmarkPopulationEngineEnvelope(payload: unknown): GovernedClinicalBenchmarkPopulationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalBenchmarkPopulationEngineEntryView => e !== null);
