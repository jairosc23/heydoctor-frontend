import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedPediatricsKnowledgeEngineEntryView, type GovernedPediatricsKnowledgeEngineResult } from "./governed-pediatrics-knowledge-engine";

function mapEntry(raw: unknown): GovernedPediatricsKnowledgeEngineEntryView | null {
...
export function mapGovernedPediatricsKnowledgeEngineEnvelope(payload: unknown): GovernedPediatricsKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPediatricsKnowledgeEngineEntryView => e !== null);
