import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalDecisionSystemPackageEntryView, type GovernedClinicalDecisionSystemPackageResult } from "./governed-clinical-decision-system-package";
function mapEntry(raw: unknown): GovernedClinicalDecisionSystemPackageEntryView | null {
...
export function mapGovernedClinicalDecisionSystemPackageEnvelope(payload: unknown): GovernedClinicalDecisionSystemPackageResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalDecisionSystemPackageEntryView => e !== null);
