import { GOVERNED_CLINICAL_EVIDENCE_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedEvidenceProvenanceEngineEntryView, type GovernedEvidenceProvenanceEngineResult } from "./governed-evidence-provenance-engine";

function mapEntry(raw: unknown): GovernedEvidenceProvenanceEngineEntryView | null {
...
export function mapGovernedEvidenceProvenanceEngineEnvelope(payload: unknown): GovernedEvidenceProvenanceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedEvidenceProvenanceEngineEntryView => e !== null);
