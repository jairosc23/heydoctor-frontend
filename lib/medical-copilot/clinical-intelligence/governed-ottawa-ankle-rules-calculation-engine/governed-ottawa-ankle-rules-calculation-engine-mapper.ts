import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedOttawaAnkleRulesCalculationEngineEntryView, type GovernedOttawaAnkleRulesCalculationEngineResult } from "./governed-ottawa-ankle-rules-calculation-engine";
function mapEntry(raw: unknown): GovernedOttawaAnkleRulesCalculationEngineEntryView | null {
  ...
  return {
    ...
    formulaId: typeof e.formulaId === "string" ? e.formulaId : "",
    resultValue: typeof e.resultValue === "number" ? e.resultValue : null,
    resultUnit: typeof e.resultUnit === "string" ? e.resultUnit : null,
    inputsUsed: Array.isArray(e.inputsUsed) ? e.inputsUsed.map(String) : [],
...
export function mapGovernedOttawaAnkleRulesCalculationEngineEnvelope(payload: unknown): GovernedOttawaAnkleRulesCalculationEngineResult | null {
  if (!payload || typeof payload !== "object") return null;
  const root = payload as Record<string, unknown>;
  const data = root.governance !== undefined || root.entries !== undefined || root.enginesPresent !== undefined || root.bmiCalculationEngine !== undefined
  ...
  else if (data.enginesPresent || data.bmiCalculationEngine) {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedOttawaAnkleRulesCalculationEngineEntryView => e !== null);
  ...
  return {
    ...
    applicableCount, entries, enginesPresent, governance: { ...GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE },
