import { GOVERNED_PRESCRIPTION_PERSISTENCE_EXECUTION_GOVERNANCE, type GovernedPrescriptionPersistenceExecutionComponentKey, type GovernedPrescriptionPersistenceExecutionComponentPresence, type GovernedPrescriptionPersistenceExecutionResult } from "./governed-prescription-persistence-execution";
const COMPONENT_DEFS: Array<{ key: GovernedPrescriptionPersistenceExecutionComponentKey; label: string }> = [
...
export function mapGovernedPrescriptionPersistenceExecutionEnvelope(payload: unknown): GovernedPrescriptionPersistenceExecutionResult | null {
  ...
  const components: GovernedPrescriptionPersistenceExecutionComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
  ...
  return {
    ...
    components, governance: { ...GOVERNED_PRESCRIPTION_PERSISTENCE_EXECUTION_GOVERNANCE },
