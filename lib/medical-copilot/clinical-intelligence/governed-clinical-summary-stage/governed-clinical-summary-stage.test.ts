...
describe("GovernedClinicalSummaryStage mapper", () => {
  it("maps pipeline stage without write/LLM flags", () => {
    const mapped = mapGovernedClinicalSummaryStageEnvelope({
    ...
    assert.equal(mapGovernedClinicalSummaryStageEnvelope(null), null);
  });
});
