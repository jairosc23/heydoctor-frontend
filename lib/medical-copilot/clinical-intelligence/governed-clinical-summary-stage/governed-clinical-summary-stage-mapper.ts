import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedClinicalSummaryStageResult, type GovernedClinicalSummaryStageStageView, type GovernedClinicalSummaryStageSurfaceRefView } from "./governed-clinical-summary-stage";

function mapRef(raw: unknown): GovernedClinicalSummaryStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedClinicalSummaryStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalSummaryStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalSummaryStageEnvelope(payload: unknown): GovernedClinicalSummaryStageResult | null {
  ...
  let stages: GovernedClinicalSummaryStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedClinicalSummaryStageStageView => x !== null);
