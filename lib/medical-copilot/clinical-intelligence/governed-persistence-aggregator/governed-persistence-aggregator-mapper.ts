import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedPersistenceAggregatorRefView, type GovernedPersistenceAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-persistence-aggregator";
function mapRef(raw: unknown): GovernedPersistenceAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedPersistenceAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedPersistenceAggregatorEnvelope(payload: unknown): GovernedPersistenceAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedPersistenceAggregatorRefView => x !== null)
