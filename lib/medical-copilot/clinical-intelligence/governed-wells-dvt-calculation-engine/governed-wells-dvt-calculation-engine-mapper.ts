import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedWellsDvtCalculationEngineEntryView, type GovernedWellsDvtCalculationEngineResult } from "./governed-wells-dvt-calculation-engine";
function mapEntry(raw: unknown): GovernedWellsDvtCalculationEngineEntryView | null {
...
export function mapGovernedWellsDvtCalculationEngineEnvelope(payload: unknown): GovernedWellsDvtCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedWellsDvtCalculationEngineEntryView => e !== null);
