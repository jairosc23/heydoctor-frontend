...
import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-clinical-risk-detection";
import { mapGovernedClinicalRiskDetectionEnvelope } from "./governed-clinical-risk-detection-mapper";

describe("GovernedClinicalRiskDetection mapper", () => {
  it("maps HITL intelligence surface without write flags", () => {
    const mapped = mapGovernedClinicalRiskDetectionEnvelope({
    ...
    assert.equal(mapGovernedClinicalRiskDetectionEnvelope(null), null);
  });
});
