import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedClinicalRiskDetectionComponentKey, type GovernedClinicalRiskDetectionComponentPresence, type GovernedClinicalRiskDetectionResult } from "./governed-clinical-risk-detection";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalRiskDetectionComponentKey; label: string }> = [
  { key: "risk", label: "Risk" },
...
export function mapGovernedClinicalRiskDetectionEnvelope(payload: unknown): GovernedClinicalRiskDetectionResult | null {
  ...
  const components: GovernedClinicalRiskDetectionComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "risk") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
