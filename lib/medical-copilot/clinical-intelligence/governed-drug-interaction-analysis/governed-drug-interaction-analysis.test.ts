...
import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-drug-interaction-analysis";
import { mapGovernedDrugInteractionAnalysisEnvelope } from "./governed-drug-interaction-analysis-mapper";

describe("GovernedDrugInteractionAnalysis mapper", () => {
  it("maps HITL intelligence surface without write flags", () => {
    const mapped = mapGovernedDrugInteractionAnalysisEnvelope({
    ...
    assert.equal(mapGovernedDrugInteractionAnalysisEnvelope(null), null);
  });
});
