import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedDrugInteractionAnalysisComponentKey, type GovernedDrugInteractionAnalysisComponentPresence, type GovernedDrugInteractionAnalysisResult } from "./governed-drug-interaction-analysis";

const COMPONENT_DEFS: Array<{ key: GovernedDrugInteractionAnalysisComponentKey; label: string }> = [
...
export function mapGovernedDrugInteractionAnalysisEnvelope(payload: unknown): GovernedDrugInteractionAnalysisResult | null {
  ...
  const components: GovernedDrugInteractionAnalysisComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
