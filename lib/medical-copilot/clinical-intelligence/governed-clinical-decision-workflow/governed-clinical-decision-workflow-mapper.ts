import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalDecisionWorkflowRefView, type GovernedClinicalDecisionWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-decision-workflow";
function mapRef(raw: unknown): GovernedClinicalDecisionWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDecisionWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalDecisionWorkflowEnvelope(payload: unknown): GovernedClinicalDecisionWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalDecisionWorkflowRefView => x !== null)
