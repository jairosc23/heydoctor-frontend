import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalConsistencyEngineEntryView, type GovernedClinicalConsistencyEngineResult } from "./governed-clinical-consistency-decision-engine";
function mapEntry(raw: unknown): GovernedClinicalConsistencyEngineEntryView | null {
...
export function mapGovernedClinicalConsistencyEngineEnvelope(payload: unknown): GovernedClinicalConsistencyEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalConsistencyEngineEntryView => e !== null);
