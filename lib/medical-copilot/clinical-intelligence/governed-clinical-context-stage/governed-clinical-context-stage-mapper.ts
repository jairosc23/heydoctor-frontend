import { GOVERNED_CLINICAL_REASONING_PIPELINE_UI_GOVERNANCE, type GovernedClinicalContextStageResult, type GovernedClinicalContextStageStageView, type GovernedClinicalContextStageSurfaceRefView } from "./governed-clinical-context-stage";

function mapRef(raw: unknown): GovernedClinicalContextStageSurfaceRefView | null {
...
function mapStage(raw: unknown): GovernedClinicalContextStageStageView | null {
  if (!raw || typeof raw !== "object") return null;
  const s = raw as Record<string, unknown>;
  if (typeof s.kind !== "string") return null;
  const refs = Array.isArray(s.surfaceRefs) ? s.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalContextStageSurfaceRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalContextStageEnvelope(payload: unknown): GovernedClinicalContextStageResult | null {
  ...
  let stages: GovernedClinicalContextStageStageView[] = [];
  if (Array.isArray(data.stages)) {
    stages = data.stages.map(mapStage).filter((x): x is GovernedClinicalContextStageStageView => x !== null);
