...
describe("GovernedClinicalContextStage mapper", () => {
  it("maps pipeline stage without write/LLM flags", () => {
    const mapped = mapGovernedClinicalContextStageEnvelope({
    ...
    assert.equal(mapGovernedClinicalContextStageEnvelope(null), null);
  });
});
