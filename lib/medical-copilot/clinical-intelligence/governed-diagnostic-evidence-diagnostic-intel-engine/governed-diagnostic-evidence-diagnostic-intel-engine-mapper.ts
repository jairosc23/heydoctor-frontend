import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticEvidenceDiagnosticIntelEngineEntryView, type GovernedDiagnosticEvidenceDiagnosticIntelEngineResult } from "./governed-diagnostic-evidence-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticEvidenceDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticEvidenceDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticEvidenceDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticEvidenceDiagnosticIntelEngineEntryView => e !== null);
