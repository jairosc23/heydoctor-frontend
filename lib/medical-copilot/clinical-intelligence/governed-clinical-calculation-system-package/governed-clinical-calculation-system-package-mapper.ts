import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedClinicalCalculationSystemPackageEntryView, type GovernedClinicalCalculationSystemPackageResult } from "./governed-clinical-calculation-system-package";
function mapEntry(raw: unknown): GovernedClinicalCalculationSystemPackageEntryView | null {
...
export function mapGovernedClinicalCalculationSystemPackageEnvelope(payload: unknown): GovernedClinicalCalculationSystemPackageResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalCalculationSystemPackageEntryView => e !== null);
