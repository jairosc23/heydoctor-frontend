import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedGeriatricsKnowledgeEngineEntryView, type GovernedGeriatricsKnowledgeEngineResult } from "./governed-geriatrics-knowledge-engine";

function mapEntry(raw: unknown): GovernedGeriatricsKnowledgeEngineEntryView | null {
...
export function mapGovernedGeriatricsKnowledgeEngineEnvelope(payload: unknown): GovernedGeriatricsKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGeriatricsKnowledgeEngineEntryView => e !== null);
