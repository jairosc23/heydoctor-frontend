import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticConfidenceDiagnosticIntelEngineEntryView, type GovernedDiagnosticConfidenceDiagnosticIntelEngineResult } from "./governed-diagnostic-confidence-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticConfidenceDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticConfidenceDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticConfidenceDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticConfidenceDiagnosticIntelEngineEntryView => e !== null);
