...
describe("Recommendation Ranking Engine", () => {
  it("maps envelope with HITL seals", () => {
    const mapped = mapGovernedRecommendationRankingEngineEnvelope({ status: "ok", data: { status: "PROPOSED_FOR_PHYSICIAN_REVIEW", title: "Recommendation Ranking Engine", applicableCount: 1, entries: [{ entryId: "e1", entryTitle: "Entry", domain: "recommendation_ranking", topic: "t", summary: "s", explanation: "x", evidenceRefs: ["medical_copilot_session"], decisionRole: "runtime", applicability: "APPLICABLE", confidence: "medium" }], enginesPresent: [], governance: { requiresPhysicianReview: true, usesLlm: false } } });
    assert.ok(mapped);
