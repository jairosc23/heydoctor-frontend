import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedRecommendationRankingEngineEntryView, type GovernedRecommendationRankingEngineResult } from "./governed-recommendation-ranking-decision-engine";
function mapEntry(raw: unknown): GovernedRecommendationRankingEngineEntryView | null {
...
export function mapGovernedRecommendationRankingEngineEnvelope(payload: unknown): GovernedRecommendationRankingEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedRecommendationRankingEngineEntryView => e !== null);
