import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedPreventiveCareSuggestionsComponentKey, type GovernedPreventiveCareSuggestionsComponentPresence, type GovernedPreventiveCareSuggestionsResult } from "./governed-preventive-care-suggestions";

const COMPONENT_DEFS: Array<{ key: GovernedPreventiveCareSuggestionsComponentKey; label: string }> = [
...
export function mapGovernedPreventiveCareSuggestionsEnvelope(payload: unknown): GovernedPreventiveCareSuggestionsResult | null {
  ...
  const components: GovernedPreventiveCareSuggestionsComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "preventive") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
