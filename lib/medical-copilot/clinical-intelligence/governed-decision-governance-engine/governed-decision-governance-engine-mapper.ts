import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedDecisionGovernanceEngineEntryView, type GovernedDecisionGovernanceEngineResult } from "./governed-decision-governance-engine";
function mapEntry(raw: unknown): GovernedDecisionGovernanceEngineEntryView | null {
...
export function mapGovernedDecisionGovernanceEngineEnvelope(payload: unknown): GovernedDecisionGovernanceEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDecisionGovernanceEngineEntryView => e !== null);
