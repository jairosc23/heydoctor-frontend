import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalReasoningWorkflowRefView, type GovernedClinicalReasoningWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-reasoning-workflow";
function mapRef(raw: unknown): GovernedClinicalReasoningWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalReasoningWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalReasoningWorkflowEnvelope(payload: unknown): GovernedClinicalReasoningWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalReasoningWorkflowRefView => x !== null)
