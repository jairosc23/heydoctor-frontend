import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedHypothesisValidationEngineEntryView, type GovernedHypothesisValidationEngineResult } from "./governed-hypothesis-validation-decision-engine";
function mapEntry(raw: unknown): GovernedHypothesisValidationEngineEntryView | null {
...
export function mapGovernedHypothesisValidationEngineEnvelope(payload: unknown): GovernedHypothesisValidationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedHypothesisValidationEngineEntryView => e !== null);
