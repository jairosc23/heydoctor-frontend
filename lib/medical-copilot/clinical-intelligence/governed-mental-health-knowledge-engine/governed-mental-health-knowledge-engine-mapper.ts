import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedMentalHealthKnowledgeEngineEntryView, type GovernedMentalHealthKnowledgeEngineResult } from "./governed-mental-health-knowledge-engine";

function mapEntry(raw: unknown): GovernedMentalHealthKnowledgeEngineEntryView | null {
...
export function mapGovernedMentalHealthKnowledgeEngineEnvelope(payload: unknown): GovernedMentalHealthKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedMentalHealthKnowledgeEngineEntryView => e !== null);
