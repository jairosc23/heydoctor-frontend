import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedClinicalSafetyChecksComponentKey, type GovernedClinicalSafetyChecksComponentPresence, type GovernedClinicalSafetyChecksResult } from "./governed-clinical-safety-checks";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalSafetyChecksComponentKey; label: string }> = [
...
export function mapGovernedClinicalSafetyChecksEnvelope(payload: unknown): GovernedClinicalSafetyChecksResult | null {
  ...
  const components: GovernedClinicalSafetyChecksComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "safety") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
