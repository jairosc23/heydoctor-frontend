...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-clinical-safety-checks";
import { mapGovernedClinicalSafetyChecksEnvelope } from "./governed-clinical-safety-checks-mapper";

describe("GovernedClinicalSafetyChecks mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedClinicalSafetyChecksEnvelope({
    ...
    assert.equal(mapGovernedClinicalSafetyChecksEnvelope(null), null);
  });
});
