import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedDiagnosticRuntimeDiagnosticIntelEngineEntryView, type GovernedDiagnosticRuntimeDiagnosticIntelEngineResult } from "./governed-diagnostic-runtime-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedDiagnosticRuntimeDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedDiagnosticRuntimeDiagnosticIntelEngineEnvelope(payload: unknown): GovernedDiagnosticRuntimeDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDiagnosticRuntimeDiagnosticIntelEngineEntryView => e !== null);
