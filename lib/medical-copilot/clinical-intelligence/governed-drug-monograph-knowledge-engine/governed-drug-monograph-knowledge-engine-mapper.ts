import { GOVERNED_CLINICAL_KNOWLEDGE_UI_GOVERNANCE, type GovernedDrugMonographKnowledgeEngineEntryView, type GovernedDrugMonographKnowledgeEngineResult } from "./governed-drug-monograph-knowledge-engine";

function mapEntry(raw: unknown): GovernedDrugMonographKnowledgeEngineEntryView | null {
...
export function mapGovernedDrugMonographKnowledgeEngineEnvelope(payload: unknown): GovernedDrugMonographKnowledgeEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedDrugMonographKnowledgeEngineEntryView => e !== null);
