import {
  ...
  type GovernedClinicalDocumentsPersistenceBridgeComponentKey,
  type GovernedClinicalDocumentsPersistenceBridgeComponentPresence,
  type GovernedClinicalDocumentsPersistenceBridgeResult,
...
export function mapGovernedClinicalDocumentsPersistenceBridgeEnvelope(
  payload: unknown,
): GovernedClinicalDocumentsPersistenceBridgeResult | null {
  ...
  const components: GovernedClinicalDocumentsPersistenceBridgeComponentPresence[] = COMPONENT_DEFS.map(
    ({ key, label }) => {
      let present = false;
