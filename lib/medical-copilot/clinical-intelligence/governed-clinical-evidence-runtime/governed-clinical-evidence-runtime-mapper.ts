import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedClinicalEvidenceRuntimeComponentKey, type GovernedClinicalEvidenceRuntimeComponentPresence, type GovernedClinicalEvidenceRuntimeResult } from "./governed-clinical-evidence-runtime";

const COMPONENT_DEFS: Array<{ key: GovernedClinicalEvidenceRuntimeComponentKey; label: string }> = [
...
export function mapGovernedClinicalEvidenceRuntimeEnvelope(payload: unknown): GovernedClinicalEvidenceRuntimeResult | null {
  ...
  const components: GovernedClinicalEvidenceRuntimeComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "runtime") present = data.runtime != null || data.surfacesPresent != null || data.ready === true || data.packageId != null;
