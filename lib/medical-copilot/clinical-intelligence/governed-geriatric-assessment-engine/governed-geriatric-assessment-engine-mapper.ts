import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedGeriatricAssessmentEngineComponentKey, type GovernedGeriatricAssessmentEngineComponentPresence, type GovernedGeriatricAssessmentEngineResult } from "./governed-geriatric-assessment-engine";
const COMPONENT_DEFS: Array<{ key: GovernedGeriatricAssessmentEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedGeriatricAssessmentEngineEnvelope(payload: unknown): GovernedGeriatricAssessmentEngineResult | null {
  ...
  const components: GovernedGeriatricAssessmentEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
