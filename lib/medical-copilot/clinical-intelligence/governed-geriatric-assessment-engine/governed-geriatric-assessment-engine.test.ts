...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-geriatric-assessment-engine";
import { mapGovernedGeriatricAssessmentEngineEnvelope } from "./governed-geriatric-assessment-engine-mapper";
describe("GovernedGeriatricAssessmentEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedGeriatricAssessmentEngineEnvelope({
    ...
    assert.equal(mapGovernedGeriatricAssessmentEngineEnvelope(null), null);
  });
});
