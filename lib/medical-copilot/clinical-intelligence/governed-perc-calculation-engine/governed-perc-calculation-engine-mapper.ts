import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedPercCalculationEngineEntryView, type GovernedPercCalculationEngineResult } from "./governed-perc-calculation-engine";
function mapEntry(raw: unknown): GovernedPercCalculationEngineEntryView | null {
...
export function mapGovernedPercCalculationEngineEnvelope(payload: unknown): GovernedPercCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPercCalculationEngineEntryView => e !== null);
