import { GOVERNED_DIAGNOSTIC_INTELLIGENCE_UI_GOVERNANCE, type GovernedClinicalClusteringDiagnosticIntelEngineEntryView, type GovernedClinicalClusteringDiagnosticIntelEngineResult } from "./governed-clinical-clustering-diagnostic-intel-engine";
function mapEntry(raw: unknown): GovernedClinicalClusteringDiagnosticIntelEngineEntryView | null {
...
export function mapGovernedClinicalClusteringDiagnosticIntelEngineEnvelope(payload: unknown): GovernedClinicalClusteringDiagnosticIntelEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedClinicalClusteringDiagnosticIntelEngineEntryView => e !== null);
