import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedCalculationAggregatorRefView, type GovernedCalculationAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-calculation-aggregator";
function mapRef(raw: unknown): GovernedCalculationAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedCalculationAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedCalculationAggregatorEnvelope(payload: unknown): GovernedCalculationAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedCalculationAggregatorRefView => x !== null)
