import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalWorkflowEnginePackageRefView, type GovernedClinicalWorkflowEnginePackageResult, type GovernedClinicalWorkflowView } from "./governed-clinical-workflow-engine-package";
function mapRef(raw: unknown): GovernedClinicalWorkflowEnginePackageRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalWorkflowEnginePackageRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalWorkflowEnginePackageEnvelope(payload: unknown): GovernedClinicalWorkflowEnginePackageResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalWorkflowEnginePackageRefView => x !== null)
