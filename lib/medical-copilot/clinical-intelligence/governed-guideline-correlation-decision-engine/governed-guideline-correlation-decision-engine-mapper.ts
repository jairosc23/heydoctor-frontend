import { GOVERNED_CLINICAL_DECISION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedGuidelineCorrelationEngineEntryView, type GovernedGuidelineCorrelationEngineResult } from "./governed-guideline-correlation-decision-engine";
function mapEntry(raw: unknown): GovernedGuidelineCorrelationEngineEntryView | null {
...
export function mapGovernedGuidelineCorrelationEngineEnvelope(payload: unknown): GovernedGuidelineCorrelationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedGuidelineCorrelationEngineEntryView => e !== null);
