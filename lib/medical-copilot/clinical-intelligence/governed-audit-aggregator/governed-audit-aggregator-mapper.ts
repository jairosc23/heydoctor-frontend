import { GOVERNED_CLINICAL_AI_ORCHESTRATOR_UI_GOVERNANCE, type GovernedAuditAggregatorRefView, type GovernedAuditAggregatorResult, type GovernedOrchestratorAggregatorView } from "./governed-audit-aggregator";
function mapRef(raw: unknown): GovernedAuditAggregatorRefView | null {
...
function mapAgg(raw: unknown): GovernedOrchestratorAggregatorView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedAuditAggregatorRefView => x !== null) : [];
  return {
...
export function mapGovernedAuditAggregatorEnvelope(payload: unknown): GovernedAuditAggregatorResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedAuditAggregatorRefView => x !== null)
