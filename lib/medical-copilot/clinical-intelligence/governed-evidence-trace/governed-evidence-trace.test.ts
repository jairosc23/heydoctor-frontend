...
import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE } from "./governed-evidence-trace";
import { mapGovernedEvidenceTraceEnvelope } from "./governed-evidence-trace-mapper";

describe("GovernedEvidenceTrace mapper", () => {
  it("maps HITL evidence surface without write flags", () => {
    const mapped = mapGovernedEvidenceTraceEnvelope({
    ...
    assert.equal(mapGovernedEvidenceTraceEnvelope(null), null);
  });
});
