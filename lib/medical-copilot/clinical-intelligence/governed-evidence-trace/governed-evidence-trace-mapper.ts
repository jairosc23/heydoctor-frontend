import { GOVERNED_CLINICAL_EVIDENCE_UI_GOVERNANCE, type GovernedEvidenceTraceComponentKey, type GovernedEvidenceTraceComponentPresence, type GovernedEvidenceTraceResult } from "./governed-evidence-trace";

const COMPONENT_DEFS: Array<{ key: GovernedEvidenceTraceComponentKey; label: string }> = [
...
export function mapGovernedEvidenceTraceEnvelope(payload: unknown): GovernedEvidenceTraceResult | null {
  ...
  const components: GovernedEvidenceTraceComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "trace") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.evidenceMapping != null;
