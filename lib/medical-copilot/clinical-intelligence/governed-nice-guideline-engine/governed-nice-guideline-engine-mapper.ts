import { GOVERNED_CLINICAL_GUIDELINES_ENGINE_ENTERPRISE_UI_GOVERNANCE, type GovernedNiceGuidelineEngineEntryView, type GovernedNiceGuidelineEngineResult } from "./governed-nice-guideline-engine";

function mapEntry(raw: unknown): GovernedNiceGuidelineEngineEntryView | null {
...
export function mapGovernedNiceGuidelineEngineEnvelope(payload: unknown): GovernedNiceGuidelineEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedNiceGuidelineEngineEntryView => e !== null);
