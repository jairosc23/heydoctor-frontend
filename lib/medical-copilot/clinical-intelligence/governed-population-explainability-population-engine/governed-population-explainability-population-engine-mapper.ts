import { GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE, type GovernedPopulationExplainabilityPopulationEngineEntryView, type GovernedPopulationExplainabilityPopulationEngineResult } from "./governed-population-explainability-population-engine";
function mapEntry(raw: unknown): GovernedPopulationExplainabilityPopulationEngineEntryView | null {
  ...
  return {
    ...
    populationRole: typeof e.populationRole === "string" ? (e.populationRole as string) : "",
...
export function mapGovernedPopulationExplainabilityPopulationEngineEnvelope(payload: unknown): GovernedPopulationExplainabilityPopulationEngineResult | null {
  if (!payload || typeof payload !== "object") return null;
  const root = payload as Record<string, unknown>;
  const data = root.governance !== undefined || root.entries !== undefined || root.enginesPresent !== undefined || root.populationRuntimePopulationEngine !== undefined
  ...
  else if (data.enginesPresent || data.populationRuntimePopulationEngine) {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedPopulationExplainabilityPopulationEngineEntryView => e !== null);
  ...
  return {
    ...
    governance: { ...GOVERNED_POPULATION_HEALTH_UI_GOVERNANCE }, reason: typeof data.reason === "string" ? data.reason : null,
