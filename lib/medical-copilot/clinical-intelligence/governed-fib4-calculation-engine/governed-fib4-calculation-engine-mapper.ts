import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedFib4CalculationEngineEntryView, type GovernedFib4CalculationEngineResult } from "./governed-fib4-calculation-engine";
function mapEntry(raw: unknown): GovernedFib4CalculationEngineEntryView | null {
...
export function mapGovernedFib4CalculationEngineEnvelope(payload: unknown): GovernedFib4CalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedFib4CalculationEngineEntryView => e !== null);
