import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedNews2CalculationEngineEntryView, type GovernedNews2CalculationEngineResult } from "./governed-news2-calculation-engine";
function mapEntry(raw: unknown): GovernedNews2CalculationEngineEntryView | null {
...
export function mapGovernedNews2CalculationEngineEnvelope(payload: unknown): GovernedNews2CalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedNews2CalculationEngineEntryView => e !== null);
