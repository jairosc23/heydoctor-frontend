import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedChronicDiseaseFollowUpAnalysisComponentKey, type GovernedChronicDiseaseFollowUpAnalysisComponentPresence, type GovernedChronicDiseaseFollowUpAnalysisResult } from "./governed-chronic-disease-follow-up-analysis";

const COMPONENT_DEFS: Array<{ key: GovernedChronicDiseaseFollowUpAnalysisComponentKey; label: string }> = [
...
export function mapGovernedChronicDiseaseFollowUpAnalysisEnvelope(payload: unknown): GovernedChronicDiseaseFollowUpAnalysisResult | null {
  ...
  const components: GovernedChronicDiseaseFollowUpAnalysisComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
    if (key === "followUp") present = items.length > 0 || surfaces.length > 0 || data.kind != null || data.drugInteractionAnalysis != null;
