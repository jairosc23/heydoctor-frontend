...
import { GOVERNED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-chronic-disease-follow-up-analysis";
import { mapGovernedChronicDiseaseFollowUpAnalysisEnvelope } from "./governed-chronic-disease-follow-up-analysis-mapper";

describe("GovernedChronicDiseaseFollowUpAnalysis mapper", () => {
  it("maps HITL intelligence surface without write flags", () => {
    const mapped = mapGovernedChronicDiseaseFollowUpAnalysisEnvelope({
    ...
    assert.equal(mapGovernedChronicDiseaseFollowUpAnalysisEnvelope(null), null);
  });
});
