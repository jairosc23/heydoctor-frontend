...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-hypertension-management-engine";
import { mapGovernedHypertensionManagementEngineEnvelope } from "./governed-hypertension-management-engine-mapper";
describe("GovernedHypertensionManagementEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedHypertensionManagementEngineEnvelope({
    ...
    assert.equal(mapGovernedHypertensionManagementEngineEnvelope(null), null);
  });
});
