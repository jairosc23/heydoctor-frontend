import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedHypertensionManagementEngineComponentKey, type GovernedHypertensionManagementEngineComponentPresence, type GovernedHypertensionManagementEngineResult } from "./governed-hypertension-management-engine";
const COMPONENT_DEFS: Array<{ key: GovernedHypertensionManagementEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedHypertensionManagementEngineEnvelope(payload: unknown): GovernedHypertensionManagementEngineResult | null {
  ...
  const components: GovernedHypertensionManagementEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
