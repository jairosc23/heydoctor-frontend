import { GOVERNED_CLINICAL_CALCULATION_SYSTEM_ENTERPRISE_UI_GOVERNANCE, type GovernedFraminghamCalculationEngineEntryView, type GovernedFraminghamCalculationEngineResult } from "./governed-framingham-calculation-engine";
function mapEntry(raw: unknown): GovernedFraminghamCalculationEngineEntryView | null {
...
export function mapGovernedFraminghamCalculationEngineEnvelope(payload: unknown): GovernedFraminghamCalculationEngineResult | null {
  ...
  const entries = entriesRaw.map(mapEntry).filter((e): e is GovernedFraminghamCalculationEngineEntryView => e !== null);
