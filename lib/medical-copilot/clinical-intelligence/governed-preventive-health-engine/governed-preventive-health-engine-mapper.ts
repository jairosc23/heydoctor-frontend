import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE, type GovernedPreventiveHealthEngineComponentKey, type GovernedPreventiveHealthEngineComponentPresence, type GovernedPreventiveHealthEngineResult } from "./governed-preventive-health-engine";
const COMPONENT_DEFS: Array<{ key: GovernedPreventiveHealthEngineComponentKey; label: string }> = [
  { key: "engine", label: "Engine" },
  { key: "specialty", label: "Specialty" },
  { key: "governance", label: "Governance" },
  { key: "hitl", label: "HITL" },
];
export function mapGovernedPreventiveHealthEngineEnvelope(payload: unknown): GovernedPreventiveHealthEngineResult | null {
  ...
  const components: GovernedPreventiveHealthEngineComponentPresence[] = COMPONENT_DEFS.map(({ key, label }) => {
    let present = false;
