...
import { GOVERNED_SPECIALIZED_CLINICAL_INTELLIGENCE_UI_GOVERNANCE } from "./governed-preventive-health-engine";
import { mapGovernedPreventiveHealthEngineEnvelope } from "./governed-preventive-health-engine-mapper";
describe("GovernedPreventiveHealthEngine mapper", () => {
  it("maps HITL specialty engine surface without write flags", () => {
    const mapped = mapGovernedPreventiveHealthEngineEnvelope({
    ...
    assert.equal(mapGovernedPreventiveHealthEngineEnvelope(null), null);
  });
});
