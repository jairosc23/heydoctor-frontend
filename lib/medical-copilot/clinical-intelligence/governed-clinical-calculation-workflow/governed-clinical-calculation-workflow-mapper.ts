import { GOVERNED_CLINICAL_WORKFLOW_ENGINE_UI_GOVERNANCE, type GovernedClinicalCalculationWorkflowRefView, type GovernedClinicalCalculationWorkflowResult, type GovernedClinicalWorkflowView } from "./governed-clinical-calculation-workflow";
function mapRef(raw: unknown): GovernedClinicalCalculationWorkflowRefView | null {
...
function mapWf(raw: unknown): GovernedClinicalWorkflowView | null {
  ...
  const refs = Array.isArray(a.surfaceRefs) ? a.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalCalculationWorkflowRefView => x !== null) : [];
  return {
...
export function mapGovernedClinicalCalculationWorkflowEnvelope(payload: unknown): GovernedClinicalCalculationWorkflowResult | null {
  ...
  const surfaceRefs = Array.isArray(data.surfaceRefs)
    ? data.surfaceRefs.map(mapRef).filter((x): x is GovernedClinicalCalculationWorkflowRefView => x !== null)
