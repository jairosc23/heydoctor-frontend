...
const SECTION = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalEvidenceSection.tsx");
const CHART = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalEncounterChart.tsx");

const FORBIDDEN = [
  "composeClinicalEvidence",
  "clinical-evidence.aggregate",
  "evaluateClinicalEvidenceGate",
  "resolveClinicalEvidenceCapability",
  "adaptClinicalEvidenceComposerInput",
  "adaptKnowledgeCitation",
  "adaptClinicalFoundationClinic",
  "clinical-evidence.composer",
...
test("clinical evidence client does not import Composer, Aggregate, Gate, adapters, Knowledge or Foundation", () => {
...
test("Encounter clinical evidence section consumes only the HTTP client", () => {
  const source = readFileSync(SECTION, "utf8");
  assert.ok(source.includes("@/lib/clinical-evidence"));
  assert.ok(source.includes("listEnabledClinicalEvidenceTypes"));
