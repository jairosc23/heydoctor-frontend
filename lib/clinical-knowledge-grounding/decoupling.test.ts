...
const SECTION = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalKnowledgeGroundingSection.tsx");
const CHART = join(DIR, "../../app/panel/consultas/[id]/_components/chart/ClinicalEncounterChart.tsx");

const FORBIDDEN = [
  "composeClinicalKnowledgeGrounding",
  "clinical-knowledge-grounding.aggregate",
  "evaluateClinicalKnowledgeGroundingGate",
  "resolveClinicalKnowledgeGroundingCapability",
  "adaptClinicalKnowledgeGroundingComposerInput",
  "adaptKnowledgeCitation",
  "adaptEvidenceCitation",
  "adaptClinicalFoundationClinic",
  "clinical-knowledge-grounding.composer",
  "clinical-knowledge-grounding.gate",
  "clinical-knowledge-grounding.capability",
...
test("Encounter scientific governance section consumes only the HTTP client", () => {
  ...
  assert.ok(source.includes("@/lib/clinical-knowledge-grounding"));
  assert.ok(source.includes("listEnabledClinicalKnowledgeGroundingTypes"));
  ...
  assertClean("ClinicalKnowledgeGroundingSection.tsx", source);
});

test("Encounter chart wires the section without engine domain imports", () => {
  const source = readFileSync(CHART, "utf8");
  assert.ok(source.includes("ClinicalKnowledgeGroundingSection"));
