...
import {
  ...
  type ClinicalKnowledgeEngineHttpView,
  type ClinicalKnowledgeEngineListItem,
...
export interface ClinicalKnowledgeEngineSectionProps {
...
function citationSummary(view: ClinicalKnowledgeEngineHttpView): string | null {
...
function stanceLabel(adviseStance: unknown): string | null {
  return cipVocabularyLabel(adviseStance, STANCE_LABEL);
}

export function ClinicalKnowledgeEngineSection({
  consultationId,
}: ClinicalKnowledgeEngineSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalKnowledgeEngineListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalKnowledgeEngineTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar el motor de conocimiento clínico."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    <ClinicalEncounterSection
      sectionNumber={43}
      title="Clinical Knowledge Engine"
      id="encounter-section-43"
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Knowledge Engine. Representa
        ClinicalKnowledgeEngineView, Gate y Capability. No persiste, no
        decide y no ejecuta.
      </p>
      ...
      {state === "loading" ? (
        <div
          ...
          data-testid="clinical-knowledge-engine-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-knowledge-engine-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-knowledge-engine-empty"
        >
          No hay consejo elegible habilitado para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-knowledge-engine-list">
          {items.map((item) => {
            ...
            const stance = view ? stanceLabel(view.adviseStance) : null;
            return (
              <li
                key={item.adviseType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-knowledge-engine-card-${item.adviseType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-knowledge-engine-view-${item.adviseType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-knowledge-engine-view-${item.adviseType}`}
                      >
                        ClinicalKnowledgeEngineView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-knowledge-engine-status-${item.adviseType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-knowledge-engine-immutable-${item.adviseType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-knowledge-engine-gate-${item.adviseType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-knowledge-engine-capability-${item.adviseType}`}
                >
