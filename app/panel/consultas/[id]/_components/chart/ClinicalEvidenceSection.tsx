...
function citationSummary(view: ClinicalEvidenceHttpView): string | null {
...
export function ClinicalEvidenceSection({
  consultationId,
}: ClinicalEvidenceSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalEvidenceListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalEvidenceTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar la evidencia clínica."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    ...
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Evidence. Representa ClinicalEvidenceView,
      ...
      {state === "loading" ? (
        <div
          ...
          data-testid="clinical-evidence-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-evidence-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-evidence-empty"
        >
          No hay evidencia clínica habilitada para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-evidence-list">
          {items.map((item) => {
            ...
            const stance = view ? stanceLabel(view.evidenceStance) : null;
            return (
              <li
                key={item.evidenceType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-evidence-card-${item.evidenceType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-evidence-view-${item.evidenceType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-evidence-view-${item.evidenceType}`}
                      >
                        ClinicalEvidenceView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-evidence-status-${item.evidenceType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-evidence-immutable-${item.evidenceType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-evidence-gate-${item.evidenceType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-evidence-capability-${item.evidenceType}`}
                >
