...
import {
  ...
  type ClinicalReasoningHttpView,
  type ClinicalReasoningListItem,
...
function citationSummary(view: ClinicalReasoningHttpView): string | null {
  const understandings = view.sourceRefs?.understandings?.length
    ? view.sourceRefs.understandings
    : Array.isArray(view.payload.understandings)
      ? view.payload.understandings
      : [];
  const ids = understandings
    .map((ref) => ref.understandingId)
...
export function ClinicalReasoningSection({
  ...
  const [items, setItems] = useState<ClinicalReasoningListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalReasoningTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar el razonamiento clínico."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    ...
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Reasoning. Representa
        ClinicalReasoningView, Gate y Capability. No persiste, no
        diagnostica, no recomienda y no modifica el razonamiento
        estructural.
      </p>
      ...
      {state === "loading" ? (
        <div
          ...
          data-testid="clinical-reasoning-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-reasoning-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-reasoning-empty"
        >
          No hay razonamientos clínicos habilitados para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-reasoning-list">
          {items.map((item) => {
              ...
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-reasoning-view-${item.reasoningType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-reasoning-view-${item.reasoningType}`}
                      >
                        ClinicalReasoningView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-reasoning-status-${item.reasoningType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-reasoning-immutable-${item.reasoningType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-reasoning-gate-${item.reasoningType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-reasoning-capability-${item.reasoningType}`}
                >
