"use client";

import { useCallback, useEffect, useState } from "react";
import { getApiErrorMessage } from "@/lib/heydoctor-api";
import {
  listEnabledClinicalArtifacts,
  type ClinicalArtifactHttpView,
  type ClinicalArtifactListItem,
} from "@/lib/clinical-artifacts";
import { ClinicalEncounterSection } from "./ClinicalEncounterSection";

export interface ClinicalArtifactsSectionProps {
  consultationId?: string | null;
}

type LoadState = "idle" | "loading" | "ready" | "error";

const STATUS_LABEL: Record<string, string> = {
  recorded: "Registrado",
};

function asRecord(value: unknown): Record<string, unknown> {
  return value && typeof value === "object"
    ? (value as Record<string, unknown>)
    : {};
}

function payloadSummary(view: ClinicalArtifactHttpView): string | null {
  const payload = asRecord(view.payload);
  const kind = payload.kind;
  if (kind === "clinical_document") {
    const documentType =
      typeof payload.documentType === "string"
        ? payload.documentType.trim()
        : "";
    const summary =
      typeof payload.summary === "string" ? payload.summary.trim() : "";
    return [documentType, summary].filter(Boolean).join(" · ") || null;
  }
  if (kind === "clinical_order") {
    const orderType =
      typeof payload.orderType === "string" ? payload.orderType.trim() : "";
    const summary =
      typeof payload.summary === "string" ? payload.summary.trim() : "";
    return [orderType, summary].filter(Boolean).join(" · ") || null;
  }
  if (kind === "clinical_decision") {
    const decisionType =
      typeof payload.decisionType === "string"
        ? payload.decisionType.trim()
        : "";
    const summary =
      typeof payload.summary === "string" ? payload.summary.trim() : "";
    return [decisionType, summary].filter(Boolean).join(" · ") || null;
  }
  if (kind === "encounter_close") {
    return typeof payload.note === "string" ? payload.note.trim() || null : null;
  }
  if (kind === "authority_event") {
    return typeof payload.eventType === "string"
      ? payload.eventType.trim() || null
      : null;
  }
  return null;
}

function statusLabel(status: string): string {
  return STATUS_LABEL[status] ?? status;
}

export function ClinicalArtifactsSection({
  consultationId,
}: ClinicalArtifactsSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalArtifactListItem[]>([]);
  const [error, setError] = useState<string | null>(null);

  const load = useCallback(async () => {
    if (!consultationId) {
      setItems([]);
      setError(null);
      setState("ready");
      return;
    }
    setState("loading");
    setError(null);
    try {
      const next = await listEnabledClinicalArtifacts(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(
          err,
          "No se pudieron cargar los artefactos clínicos.",
        ),
      );
      setState("error");
    }
  }, [consultationId]);

  useEffect(() => {
    void load();
  }, [load]);

  return (
    <ClinicalEncounterSection
      sectionNumber={26}
      title="Clinical Artifacts"
      id="encounter-section-26"
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Artifact Registry. Representa
        ClinicalArtifactView, Gate y Capability. No persiste, no busca historia y
        no navega relaciones.
      </p>

      {state === "loading" ? (
        <div
          className="space-y-hd-2"
          data-testid="clinical-artifacts-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
              key={slot}
              className="h-20 animate-pulse rounded-hd-md border border-hd-border-subtle bg-slate-100"
            />
          ))}
        </div>
      ) : null}

      {state === "error" ? (
        <div
          className="rounded-hd-md border border-red-200 bg-red-50 px-hd-3 py-hd-2 text-sm text-red-800"
          data-testid="clinical-artifacts-error"
          role="alert"
        >
          <p>{error}</p>
          <button
            type="button"
            onClick={() => void load()}
            className="mt-2 rounded-hd-md border border-red-300 bg-white px-2.5 py-1 text-xs font-semibold text-red-800 hover:bg-red-100"
          >
            Reintentar
          </button>
        </div>
      ) : null}

      {state === "ready" && items.length === 0 ? (
        <p
          className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-muted px-hd-3 py-hd-2 text-sm text-slate-600"
          data-testid="clinical-artifacts-empty"
        >
          No hay artefactos clínicos habilitados para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-artifacts-list">
          {items.map((item) => {
            const projection = item.preview.data.view;
            const view = projection.ok ? projection.view : null;
            const issues = item.preview.data.gate.ok
              ? []
              : item.preview.data.gate.issues;
            const summary = view ? payloadSummary(view) : null;
            return (
              <li
                key={item.artifactType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-artifact-card-${item.artifactType}`}
              >
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div className="min-w-0">
                    <h4 className="text-sm font-semibold text-slate-800">
                      {item.capability.title}
                    </h4>
                    {view ? (
                      <p
                        className="text-xs text-slate-500"
                        data-testid={`clinical-artifact-view-${item.artifactType}`}
                      >
                        {view.patient.name} · {view.clinic.name} ·{" "}
                        {view.countryCode}
                      </p>
                    ) : (
                      <p
                        className="text-xs text-slate-500"
                        data-testid={`clinical-artifact-view-${item.artifactType}`}
                      >
                        ClinicalArtifactView no disponible
                        {"reason" in projection ? ` · ${projection.reason}` : ""}
                      </p>
                    )}
                    {view ? (
                      <p
                        className="mt-1 text-xs text-slate-600"
                        data-testid={`clinical-artifact-status-${item.artifactType}`}
                      >
                        {statusLabel(view.status)}
                      </p>
                    ) : null}
                    {summary ? (
                      <p className="mt-1 text-sm text-slate-700">{summary}</p>
                    ) : null}
                    {view?.relatedArtifactId ? (
                      <p
                        className="mt-1 text-xs text-slate-500"
                        data-testid={`clinical-artifact-related-${item.artifactType}`}
                      >
                        Referencia: {view.relatedArtifactId}
                      </p>
                    ) : null}
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700"
                        data-testid={`clinical-artifact-immutable-${item.artifactType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                    {item.preview.data.gate.ok ? (
                      <span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-emerald-800">
                        Gate OK
                      </span>
                    ) : (
                      <span className="rounded-full border border-red-200 bg-red-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-red-800">
                        Gate
                      </span>
                    )}
                  </div>
                </div>

                {issues.length > 0 ? (
                  <ul
                    className="mt-2 space-y-1 rounded-hd-md border border-amber-200 bg-amber-50 px-hd-3 py-hd-2"
                    data-testid={`clinical-artifact-gate-${item.artifactType}`}
                  >
                    {issues.map((issue) => (
                      <li
                        key={`${issue.code}:${issue.field}`}
                        className="text-xs text-amber-900"
                      >
                        {issue.message}
                      </li>
                    ))}
                  </ul>
                ) : null}

                <div
                  className="mt-3 flex flex-wrap gap-1.5"
                  data-testid={`clinical-artifact-capability-${item.artifactType}`}
                >
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Preview {item.capability.supportsPreview ? "on" : "off"}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Historia {item.capability.supportsHistory ? "on" : "off"}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Trazabilidad{" "}
                    {item.capability.supportsTraceability ? "on" : "off"}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Relación{" "}
                    {item.capability.supportsRelationship ? "on" : "off"}
                  </span>
                </div>
              </li>
            );
          })}
        </ul>
      ) : null}
    </ClinicalEncounterSection>
  );
}
