...
function citationSummary(view: ClinicalLearningHttpView): string | null {
  const executions = view.sourceRefs?.executions?.length
    ? view.sourceRefs.executions
    : Array.isArray(view.payload.executions)
      ? view.payload.executions
      : [];
  const ids = executions
    .map((ref) => ref.executionId)
...
export function ClinicalLearningSection({
  consultationId,
}: ClinicalLearningSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalLearningListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalLearningTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar el aprendizaje clínico."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    ...
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Learning. Representa ClinicalLearningView,
        Gate y Capability. No persiste, no reescribe CCF y no reejecuta.
      </p>

      {state === "loading" ? (
        <div
          className="space-y-hd-2"
          data-testid="clinical-learning-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-learning-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-learning-empty"
        >
          No hay aprendizajes clínicos habilitados para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-learning-list">
          {items.map((item) => {
            ...
            const learningReturn = view ? returnLabel(view.learningReturn) : null;
            return (
              <li
                key={item.learningType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-learning-card-${item.learningType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-learning-view-${item.learningType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-learning-view-${item.learningType}`}
                      >
                        ClinicalLearningView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-learning-status-${item.learningType}`}
                      >
                        {statusLabel(view.status)}
                        {learningReturn ? ` · ${learningReturn}` : ""}
                      </p>
                    ) : null}
                    {summary ? (
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-learning-immutable-${item.learningType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-learning-gate-${item.learningType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-learning-capability-${item.learningType}`}
                >
