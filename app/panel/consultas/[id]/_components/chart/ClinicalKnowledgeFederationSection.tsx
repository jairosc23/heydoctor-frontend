...
import {
  ...
  type ClinicalKnowledgeFederationHttpView,
  type ClinicalKnowledgeFederationListItem,
} from "@/lib/clinical-knowledge-federation";
import { cipVocabularyLabel } from "../cip-preview-display";
import { ClinicalEncounterSection } from "./ClinicalEncounterSection";

export interface ClinicalKnowledgeFederationSectionProps {
...
function citationSummary(view: ClinicalKnowledgeFederationHttpView): string | null {
...
function stanceLabel(federationStance: unknown): string | null {
  return cipVocabularyLabel(federationStance, STANCE_LABEL);
}

export function ClinicalKnowledgeFederationSection({
  consultationId,
}: ClinicalKnowledgeFederationSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalKnowledgeFederationListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalKnowledgeFederationTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar la federación de conocimiento."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    <ClinicalEncounterSection
      sectionNumber={41}
      title="Clinical Knowledge Federation"
      id="encounter-section-41"
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Knowledge Federation. Representa
        ClinicalKnowledgeFederationView, Gate y Capability. No persiste, no
        decide y no ejecuta.
      </p>
      ...
      {state === "loading" ? (
        <div
          ...
          data-testid="clinical-knowledge-federation-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-knowledge-federation-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-knowledge-federation-empty"
        >
          No hay federación de conocimiento habilitada para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-knowledge-federation-list">
          {items.map((item) => {
            ...
            const stance = view ? stanceLabel(view.federationStance) : null;
            return (
              <li
                key={item.federationType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-knowledge-federation-card-${item.federationType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-knowledge-federation-view-${item.federationType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-knowledge-federation-view-${item.federationType}`}
                      >
                        ClinicalKnowledgeFederationView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-knowledge-federation-status-${item.federationType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-knowledge-federation-immutable-${item.federationType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-knowledge-federation-gate-${item.federationType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-knowledge-federation-capability-${item.federationType}`}
                >
