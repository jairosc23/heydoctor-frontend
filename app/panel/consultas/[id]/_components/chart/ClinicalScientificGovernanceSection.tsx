...
import {
  ...
  type ClinicalScientificGovernanceHttpView,
  type ClinicalScientificGovernanceListItem,
} from "@/lib/clinical-scientific-governance";
import { cipVocabularyLabel } from "../cip-preview-display";
import { ClinicalEncounterSection } from "./ClinicalEncounterSection";

export interface ClinicalScientificGovernanceSectionProps {
...
function citationSummary(view: ClinicalScientificGovernanceHttpView): string | null {
  ...
  const ids = citations.flatMap((ref) =>
    [ref.knowledgeId, ref.evidenceId].filter(
...
function stanceLabel(scientificStance: unknown): string | null {
  return cipVocabularyLabel(scientificStance, STANCE_LABEL);
}

export function ClinicalScientificGovernanceSection({
  consultationId,
}: ClinicalScientificGovernanceSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalScientificGovernanceListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalScientificGovernanceTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar la gobernanza científica."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    <ClinicalEncounterSection
      sectionNumber={40}
      title="Clinical Scientific Governance"
      id="encounter-section-40"
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Scientific Governance. Representa
        ClinicalScientificGovernanceView, Gate y Capability. No persiste, no
        decide y no ejecuta.
      </p>
      ...
      {state === "loading" ? (
        <div
          ...
          data-testid="clinical-scientific-governance-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-scientific-governance-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-scientific-governance-empty"
        >
          No hay gobernanza científica habilitada para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-scientific-governance-list">
          {items.map((item) => {
            ...
            const stance = view ? stanceLabel(view.scientificStance) : null;
            return (
              <li
                key={item.scientificType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-scientific-governance-card-${item.scientificType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-scientific-governance-view-${item.scientificType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-scientific-governance-view-${item.scientificType}`}
                      >
                        ClinicalScientificGovernanceView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-scientific-governance-status-${item.scientificType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-scientific-governance-immutable-${item.scientificType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-scientific-governance-gate-${item.scientificType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-scientific-governance-capability-${item.scientificType}`}
                >
