...
function citationSummary(view: ClinicalGovernanceHttpView): string | null {
  const recommendations = view.sourceRefs?.recommendations?.length
    ? view.sourceRefs.recommendations
    : Array.isArray(view.payload.recommendations)
      ? view.payload.recommendations
      : [];
  const ids = recommendations
    .map((ref) => ref.recommendationId)
...
export function ClinicalGovernanceSection({
  consultationId,
}: ClinicalGovernanceSectionProps) {
  const [state, setState] = useState<LoadState>("idle");
  const [items, setItems] = useState<ClinicalGovernanceListItem[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const next = await listEnabledClinicalGovernanceTypes(consultationId);
      setItems(next);
      setState("ready");
    } catch (err) {
      setItems([]);
      setError(
        getApiErrorMessage(err, "No se pudo cargar la gobernanza clínica."),
      );
      setState("error");
    }
  }, [consultationId]);
  ...
  return (
    ...
    >
      <p className="mb-hd-2 text-sm text-slate-600">
        Preview HTTP del Clinical Governance. Representa ClinicalGovernanceView,
        Gate y Capability. No persiste, no decide, no autoriza y no ejecuta.
      </p>

      {state === "loading" ? (
        <div
          className="space-y-hd-2"
          data-testid="clinical-governance-skeleton"
          aria-busy="true"
        >
          {[0, 1, 2].map((slot) => (
            <div
      ...
      {state === "error" ? (
        <div
          ...
          data-testid="clinical-governance-error"
          role="alert"
        >
          <p>{error}</p>
          <button
      ...
      {state === "ready" && items.length === 0 ? (
        <p
          ...
          data-testid="clinical-governance-empty"
        >
          No hay gobernanzas clínicas habilitadas para este contexto.
        </p>
      ) : null}

      {state === "ready" && items.length > 0 ? (
        <ul className="space-y-hd-2" data-testid="clinical-governance-list">
          {items.map((item) => {
            ...
            const posture = view ? postureLabel(view.posture) : null;
            return (
              <li
                key={item.governanceType}
                className="rounded-hd-md border border-hd-border-subtle bg-hd-surface-raised p-hd-3"
                data-testid={`clinical-governance-card-${item.governanceType}`}
              >
                  ...
                  <div className="min-w-0">
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-governance-view-${item.governanceType}`}
                      >
                      ...
                      <p
                        ...
                        data-testid={`clinical-governance-view-${item.governanceType}`}
                      >
                        ClinicalGovernanceView no disponible
                    ...
                    {view ? (
                      <p
                        ...
                        data-testid={`clinical-governance-status-${item.governanceType}`}
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {item.capability.immutable ? (
                      <span
                        ...
                        data-testid={`clinical-governance-immutable-${item.governanceType}`}
                      >
                        Inmutable
                      </span>
                    ) : null}
                ...
                {issues.length > 0 ? (
                  <ul
                    ...
                    data-testid={`clinical-governance-gate-${item.governanceType}`}
                  >
                    {issues.map((issue) => (
                      <li
                ...
                <div
                  ...
                  data-testid={`clinical-governance-capability-${item.governanceType}`}
                >
                  ...
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Gobernanza{" "}
                    {item.capability.supportsGovernance ? "on" : "off"}
                  </span>
                  <span className="rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-slate-700">
                    Diagnóstico{" "}
