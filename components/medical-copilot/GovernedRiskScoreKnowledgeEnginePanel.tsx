...
import { useGovernedRiskScoreKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-risk-score-knowledge-engine";
import type { GovernedRiskScoreKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-risk-score-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedRiskScoreKnowledgeEngineEntryView }) {
  return (
...
export function GovernedRiskScoreKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRiskScoreKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-risk-score-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Risk Score Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-risk-score-knowledge-engine">
