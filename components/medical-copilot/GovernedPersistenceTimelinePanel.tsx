...
import { useGovernedPersistenceTimeline } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-timeline";
import type { GovernedPersistenceTimelineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-timeline";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceTimelineComponentPresence;
}) {
  return (
...
export function GovernedPersistenceTimelinePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceTimeline({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-timeline-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Timeline Gobernado (Fase 71)">
          <p className="mb-3 text-xs text-slate-500">
            Timeline de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-timeline">
