...
import { useGovernedClinicalIntelligenceStage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-stage";
import type { GovernedClinicalIntelligenceStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedClinicalIntelligenceStageStageView }) {
  return (
...
export function GovernedClinicalIntelligenceStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalIntelligenceStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-intelligence-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Intelligence Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-intelligence-stage">
