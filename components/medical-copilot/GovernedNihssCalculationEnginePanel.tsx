...
import { useGovernedNihssCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-nihss-calculation-engine";
import type { GovernedNihssCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-nihss-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedNihssCalculationEngineEntryView }) {
  return (
...
export function GovernedNihssCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNihssCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-nihss-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"NIHSS"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-nihss-calculation-engine">
