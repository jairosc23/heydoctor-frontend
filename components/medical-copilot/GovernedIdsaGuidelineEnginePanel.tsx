...
import { useGovernedIdsaGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-idsa-guideline-engine";
import type { GovernedIdsaGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-idsa-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedIdsaGuidelineEngineEntryView }) {
  return (
...
export function GovernedIdsaGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedIdsaGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-idsa-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"IDSA Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-idsa-guideline-engine">
