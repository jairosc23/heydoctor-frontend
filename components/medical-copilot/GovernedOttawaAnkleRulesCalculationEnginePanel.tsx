...
import { useGovernedOttawaAnkleRulesCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ottawa-ankle-rules-calculation-engine";
import type { GovernedOttawaAnkleRulesCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ottawa-ankle-rules-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedOttawaAnkleRulesCalculationEngineEntryView }) {
  return (
...
export function GovernedOttawaAnkleRulesCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedOttawaAnkleRulesCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-ottawa-ankle-rules-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Ottawa Ankle Rules"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ottawa-ankle-rules-calculation-engine">
