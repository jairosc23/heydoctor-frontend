...
import { useGovernedClinicalSafetyChecks } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-safety-checks";
import type { GovernedClinicalSafetyChecksComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-safety-checks";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalSafetyChecksComponentPresence }) {
  return (
...
export function GovernedClinicalSafetyChecksPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSafetyChecks({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-safety-checks-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Safety Checks">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-safety-checks">
