...
import { useGovernedAdaGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ada-guideline-engine";
import type { GovernedAdaGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ada-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAdaGuidelineEngineEntryView }) {
  return (
...
export function GovernedAdaGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAdaGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-ada-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ADA Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ada-guideline-engine">
