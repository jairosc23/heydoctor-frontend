...
import { useGovernedDrugMonographKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-monograph-knowledge-engine";
import type { GovernedDrugMonographKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-monograph-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedDrugMonographKnowledgeEngineEntryView }) {
  return (
...
export function GovernedDrugMonographKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDrugMonographKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-drug-monograph-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Drug Monograph Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-drug-monograph-knowledge-engine">
