...
import { useGovernedGuidelineRuntimeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-runtime-engine";
import type { GovernedGuidelineRuntimeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-runtime-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedGuidelineRuntimeEngineEntryView }) {
  return (
...
export function GovernedGuidelineRuntimeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGuidelineRuntimeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-guideline-runtime-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Guideline Runtime Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-guideline-runtime-engine">
