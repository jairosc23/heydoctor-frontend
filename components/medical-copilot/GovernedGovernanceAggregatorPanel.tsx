...
import { useGovernedGovernanceAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-governance-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-governance-aggregator";
...
export function GovernedGovernanceAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGovernanceAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-governance-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governance Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-governance-aggregator">
