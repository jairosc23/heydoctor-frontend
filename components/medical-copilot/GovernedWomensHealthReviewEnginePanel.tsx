...
import { useGovernedWomensHealthReviewEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-womens-health-review-engine";
import type { GovernedWomensHealthReviewEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-womens-health-review-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedWomensHealthReviewEngineComponentPresence }) {
  return (
...
export function GovernedWomensHealthReviewEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWomensHealthReviewEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-womens-health-review-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Women's Health Review Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-womens-health-review-engine">
