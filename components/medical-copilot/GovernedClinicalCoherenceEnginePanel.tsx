...
import { useGovernedClinicalCoherenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-coherence-decision-engine";
import type { GovernedClinicalCoherenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-coherence-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalCoherenceEngineEntryView }) {
  return (
...
export function GovernedClinicalCoherenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalCoherenceEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-coherence-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Coherence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-coherence-decision-engine">
