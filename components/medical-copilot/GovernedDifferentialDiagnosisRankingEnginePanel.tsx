...
import { useGovernedDifferentialDiagnosisRankingEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-diagnosis-ranking-decision-engine";
import type { GovernedDifferentialDiagnosisRankingEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-diagnosis-ranking-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDifferentialDiagnosisRankingEngineEntryView }) {
  return (
    <div className="rounded border border-slate-200 bg-white p-3 text-sm" data-testid="clinical-decision-system-entry">
      <p className="text-xs font-medium text-slate-800">{entry.entryTitle}</p>
      <p className="mt-1 text-[11px] text-slate-500">role: {entry.decisionRole || "—"} · domain: {entry.domain}</p>
      ...
      <p className="mt-2 text-[11px] uppercase tracking-wide text-amber-700">HITL · DECISION SYSTEM · NO LLM · NO EMR</p>
    </div>
  );
}

export function GovernedDifferentialDiagnosisRankingEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDifferentialDiagnosisRankingEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-differential-diagnosis-ranking-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Differential Diagnosis Ranking Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-differential-diagnosis-ranking-decision-engine">
