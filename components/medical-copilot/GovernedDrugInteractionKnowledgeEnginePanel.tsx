...
import { useGovernedDrugInteractionKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-knowledge-engine";
import type { GovernedDrugInteractionKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedDrugInteractionKnowledgeEngineEntryView }) {
  return (
...
export function GovernedDrugInteractionKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDrugInteractionKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-drug-interaction-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Drug Interaction Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-drug-interaction-knowledge-engine">
