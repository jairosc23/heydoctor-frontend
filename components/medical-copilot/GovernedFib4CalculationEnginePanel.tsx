...
import { useGovernedFib4CalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-fib4-calculation-engine";
import type { GovernedFib4CalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-fib4-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedFib4CalculationEngineEntryView }) {
  return (
...
export function GovernedFib4CalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedFib4CalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-fib4-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"FIB-4"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-fib4-calculation-engine">
