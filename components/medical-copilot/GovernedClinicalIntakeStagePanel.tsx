...
import { useGovernedClinicalIntakeStage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intake-stage";
import type { GovernedClinicalIntakeStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intake-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedClinicalIntakeStageStageView }) {
  return (
...
export function GovernedClinicalIntakeStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalIntakeStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-intake-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Intake Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-intake-stage">
