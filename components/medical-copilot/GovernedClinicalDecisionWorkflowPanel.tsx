...
import { useGovernedClinicalDecisionWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-workflow";
...
export function GovernedClinicalDecisionWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDecisionWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-decision-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Decision Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-decision-workflow">
