...
import { useGovernedTimiCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-timi-calculation-engine";
import type { GovernedTimiCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-timi-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedTimiCalculationEngineEntryView }) {
  return (
...
export function GovernedTimiCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedTimiCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-timi-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"TIMI"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-timi-calculation-engine">
