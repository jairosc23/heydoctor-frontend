...
import { useGovernedKnowledgeAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-knowledge-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-knowledge-aggregator";
...
export function GovernedKnowledgeAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedKnowledgeAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-knowledge-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Knowledge Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-knowledge-aggregator">
