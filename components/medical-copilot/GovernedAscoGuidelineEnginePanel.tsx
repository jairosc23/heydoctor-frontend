...
import { useGovernedAscoGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-asco-guideline-engine";
import type { GovernedAscoGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-asco-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAscoGuidelineEngineEntryView }) {
  return (
...
export function GovernedAscoGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAscoGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-asco-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ASCO Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-asco-guideline-engine">
