...
import { useGovernedEvidenceSourceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-source-engine";
import type { GovernedEvidenceSourceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-source-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceSourceEngineEntryView }) {
  return (
...
export function GovernedEvidenceSourceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceSourceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-source-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Source Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-source-engine">
