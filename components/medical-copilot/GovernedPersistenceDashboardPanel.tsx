...
import { useGovernedPersistenceDashboard } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-dashboard";
import type { GovernedPersistenceDashboardComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-dashboard";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceDashboardComponentPresence;
}) {
  return (
...
export function GovernedPersistenceDashboardPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceDashboard({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-dashboard-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Dashboard Gobernado (Fase 73)">
          <p className="mb-3 text-xs text-slate-500">
            Dashboard de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-dashboard">
