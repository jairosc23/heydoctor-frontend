...
import { useGovernedApprovalQueue } from "@/lib/medical-copilot/clinical-intelligence/governed-approval-queue";
import type { GovernedApprovalQueueComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-approval-queue";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedApprovalQueueComponentPresence;
}) {
  return (
...
export function GovernedApprovalQueuePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedApprovalQueue({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-approval-queue-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Approval Queue Gobernado (Fase 54)">
          <p className="mb-3 text-xs text-slate-500">
            Cola de aprobación gobernada · Solo visualización · NO aprueba · Sin persistencia · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-approval-queue">
