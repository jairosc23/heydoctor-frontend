...
import { useGovernedRecommendationValidation } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-validation";
import type { GovernedRecommendationValidationComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-validation";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedRecommendationValidationComponentPresence }) {
  return (
...
export function GovernedRecommendationValidationPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRecommendationValidation({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-recommendation-validation-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Recommendation Validation">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-recommendation-validation">
