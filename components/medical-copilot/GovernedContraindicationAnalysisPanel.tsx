...
import { useGovernedContraindicationAnalysis } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-analysis";
import type { GovernedContraindicationAnalysisComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-analysis";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedContraindicationAnalysisComponentPresence }) {
  return (
...
export function GovernedContraindicationAnalysisPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedContraindicationAnalysis({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-contraindication-analysis-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Contraindication Analysis">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-contraindication-analysis">
