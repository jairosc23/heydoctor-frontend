...
import { useGovernedSuggestionAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-suggestion-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-suggestion-aggregator";
...
export function GovernedSuggestionAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSuggestionAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-suggestion-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Suggestion Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-suggestion-aggregator">
