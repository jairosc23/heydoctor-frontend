...
import { useGovernedGuidelineRecommendationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-recommendation-engine";
import type { GovernedGuidelineRecommendationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-recommendation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedGuidelineRecommendationEngineEntryView }) {
  return (
...
export function GovernedGuidelineRecommendationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGuidelineRecommendationEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-guideline-recommendation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Guideline Recommendation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-guideline-recommendation-engine">
