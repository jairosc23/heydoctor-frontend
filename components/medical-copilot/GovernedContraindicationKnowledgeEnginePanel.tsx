...
import { useGovernedContraindicationKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-knowledge-engine";
import type { GovernedContraindicationKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedContraindicationKnowledgeEngineEntryView }) {
  return (
...
export function GovernedContraindicationKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedContraindicationKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-contraindication-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Contraindication Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-contraindication-knowledge-engine">
