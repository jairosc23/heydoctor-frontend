...
import { useGovernedCalculationValidationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-validation-engine";
import type { GovernedCalculationValidationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-validation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCalculationValidationEngineEntryView }) {
  return (
...
export function GovernedCalculationValidationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCalculationValidationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-calculation-validation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Calculation Validation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-calculation-validation-engine">
