...
import { useGovernedCarePathwayKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-care-pathway-knowledge-engine";
import type { GovernedCarePathwayKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-care-pathway-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedCarePathwayKnowledgeEngineEntryView }) {
  return (
...
export function GovernedCarePathwayKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCarePathwayKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-care-pathway-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Care Pathway Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-care-pathway-knowledge-engine">
