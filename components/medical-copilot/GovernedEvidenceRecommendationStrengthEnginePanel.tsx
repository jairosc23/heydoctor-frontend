...
import { useGovernedEvidenceRecommendationStrengthEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-recommendation-strength-engine";
import type { GovernedEvidenceRecommendationStrengthEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-recommendation-strength-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceRecommendationStrengthEngineEntryView }) {
  return (
...
export function GovernedEvidenceRecommendationStrengthEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceRecommendationStrengthEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-recommendation-strength-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Recommendation Strength Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-recommendation-strength-engine">
