...
import { useGovernedSoapPersistenceExecution } from "@/lib/medical-copilot/clinical-intelligence/governed-soap-persistence-execution";
import type { GovernedSoapPersistenceExecutionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-soap-persistence-execution";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";
function ComponentPresenceCard({ component }: { component: GovernedSoapPersistenceExecutionComponentPresence }) {
  return (
...
export function GovernedSoapPersistenceExecutionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSoapPersistenceExecution({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-soap-persistence-execution-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed SOAP Persistence Execution">
          <p className="mb-3 text-xs text-slate-500">Persistencia gobernada · HITL · rollback · dominio soap</p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-soap-persistence-execution">
