...
import { useGovernedDiagnosticAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-aggregator";
...
export function GovernedDiagnosticAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiagnosticAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-diagnostic-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Diagnostic Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diagnostic-aggregator">
