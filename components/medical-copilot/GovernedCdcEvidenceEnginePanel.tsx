...
import { useGovernedCdcEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-cdc-evidence-engine";
import type { GovernedCdcEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-cdc-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedCdcEvidenceEngineEntryView }) {
  return (
...
export function GovernedCdcEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCdcEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-cdc-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CDC Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-cdc-evidence-engine">
