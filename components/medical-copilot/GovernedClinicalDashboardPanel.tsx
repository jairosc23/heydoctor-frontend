...
import { useGovernedClinicalDashboard } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-dashboard";
import type { GovernedClinicalDashboardComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-dashboard";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalDashboardComponentPresence;
}) {
  return (
...
export function GovernedClinicalDashboardPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDashboard({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-dashboard-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Dashboard Gobernado (Fase 35)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del dashboard clínico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-dashboard">
