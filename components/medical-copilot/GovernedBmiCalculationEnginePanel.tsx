...
import { useGovernedBmiCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-bmi-calculation-engine";
import type { GovernedBmiCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-bmi-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedBmiCalculationEngineEntryView }) {
  return (
...
export function GovernedBmiCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedBmiCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-bmi-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"BMI"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-bmi-calculation-engine">
