...
import { useGovernedRulesEvaluationStage } from "@/lib/medical-copilot/clinical-intelligence/governed-rules-evaluation-stage";
import type { GovernedRulesEvaluationStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-rules-evaluation-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedRulesEvaluationStageStageView }) {
  return (
...
export function GovernedRulesEvaluationStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRulesEvaluationStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-rules-evaluation-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Rules Evaluation Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-rules-evaluation-stage">
