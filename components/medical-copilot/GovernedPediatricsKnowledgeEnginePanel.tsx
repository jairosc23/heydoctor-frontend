...
import { useGovernedPediatricsKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-pediatrics-knowledge-engine";
import type { GovernedPediatricsKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-pediatrics-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedPediatricsKnowledgeEngineEntryView }) {
  return (
...
export function GovernedPediatricsKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPediatricsKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-pediatrics-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Pediatrics Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-pediatrics-knowledge-engine">
