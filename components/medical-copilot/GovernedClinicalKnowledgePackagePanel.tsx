...
import { useGovernedClinicalKnowledgePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-knowledge-package";
import type { GovernedClinicalKnowledgePackageEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-knowledge-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedClinicalKnowledgePackageEntryView }) {
  return (
...
export function GovernedClinicalKnowledgePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalKnowledgePackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-knowledge-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Knowledge Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-knowledge-package">
