...
import { useGovernedPhysicianWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-workspace";
import type { GovernedPhysicianWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPhysicianWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedPhysicianWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-physician-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Physician Workspace Gobernado (Fase 19)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del workspace médico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-workspace">
