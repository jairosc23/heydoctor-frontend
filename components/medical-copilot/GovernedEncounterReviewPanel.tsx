...
import { useGovernedEncounterReview } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-review";
import type { GovernedEncounterReviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-review";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedEncounterReviewComponentPresence;
}) {
  return (
...
export function GovernedEncounterReviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEncounterReview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-encounter-review-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Encounter Review Gobernado (Fase 25)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la revisión de encuentro gobernada · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-encounter-review">
