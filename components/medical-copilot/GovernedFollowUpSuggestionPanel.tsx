...
import { useGovernedFollowUpSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-follow-up-suggestion";
import type { GovernedFollowUpSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-follow-up-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedFollowUpSuggestionComponentPresence }) {
  return (
...
export function GovernedFollowUpSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedFollowUpSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-follow-up-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Follow-up Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-follow-up-suggestion">
