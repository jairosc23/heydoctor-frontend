...
import { useGovernedCha2ds2VascCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-cha2ds2-vasc-calculation-engine";
import type { GovernedCha2ds2VascCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-cha2ds2-vasc-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCha2ds2VascCalculationEngineEntryView }) {
  return (
...
export function GovernedCha2ds2VascCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCha2ds2VascCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-cha2ds2-vasc-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CHA2DS2-VASc"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-cha2ds2-vasc-calculation-engine">
