...
import { useGovernedMedicationSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-medication-suggestion";
import type { GovernedMedicationSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-medication-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedMedicationSuggestionComponentPresence }) {
  return (
...
export function GovernedMedicationSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMedicationSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-medication-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Medication Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-medication-suggestion">
