...
import { useGovernedSpecializedClinicalIntelligencePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-specialized-clinical-intelligence-package";
import type { GovernedSpecializedClinicalIntelligencePackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-specialized-clinical-intelligence-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedSpecializedClinicalIntelligencePackageComponentPresence }) {
  return (
...
export function GovernedSpecializedClinicalIntelligencePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSpecializedClinicalIntelligencePackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-specialized-clinical-intelligence-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Specialized Clinical Intelligence Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-specialized-clinical-intelligence-package">
