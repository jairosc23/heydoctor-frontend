...
import { useGovernedClinicalHypothesisEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-hypothesis-decision-engine";
import type { GovernedClinicalHypothesisEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-hypothesis-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalHypothesisEngineEntryView }) {
  return (
...
export function GovernedClinicalHypothesisEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalHypothesisEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-hypothesis-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Hypothesis Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-hypothesis-decision-engine">
