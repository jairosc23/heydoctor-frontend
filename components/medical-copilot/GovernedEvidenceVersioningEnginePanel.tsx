...
import { useGovernedEvidenceVersioningEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-versioning-engine";
import type { GovernedEvidenceVersioningEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-versioning-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceVersioningEngineEntryView }) {
  return (
...
export function GovernedEvidenceVersioningEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceVersioningEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-versioning-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Versioning Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-versioning-engine">
