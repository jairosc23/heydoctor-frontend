...
import { useGovernedFollowUpKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-follow-up-knowledge-engine";
import type { GovernedFollowUpKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-follow-up-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedFollowUpKnowledgeEngineEntryView }) {
  return (
...
export function GovernedFollowUpKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedFollowUpKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-follow-up-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Follow-up Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-follow-up-knowledge-engine">
