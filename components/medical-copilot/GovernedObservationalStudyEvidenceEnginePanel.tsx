...
import { useGovernedObservationalStudyEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-observational-study-evidence-engine";
import type { GovernedObservationalStudyEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-observational-study-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedObservationalStudyEvidenceEngineEntryView }) {
  return (
...
export function GovernedObservationalStudyEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedObservationalStudyEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-observational-study-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Observational Study Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-observational-study-evidence-engine">
