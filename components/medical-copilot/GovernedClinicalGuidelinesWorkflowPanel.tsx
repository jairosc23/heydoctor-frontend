...
import { useGovernedClinicalGuidelinesWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-workflow";
...
export function GovernedClinicalGuidelinesWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalGuidelinesWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-guidelines-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Guidelines Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-guidelines-workflow">
