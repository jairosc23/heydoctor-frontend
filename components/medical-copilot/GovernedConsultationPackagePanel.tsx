...
import { useGovernedConsultationPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-package";
import type { GovernedConsultationPackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationPackageComponentPresence;
}) {
  return (
...
export function GovernedConsultationPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationPackage({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Package Gobernado (Fase 28)">
          <p className="mb-3 text-xs text-slate-500">
            Paquete oficial de solo lectura de la consulta gobernada · Sin generación · Sin edición · Sin aprobación · Sin ejecución · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-package">
