...
import { useGovernedClinicalReasoningPipeline } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-reasoning-pipeline";
import type { GovernedClinicalReasoningPipelineStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-reasoning-pipeline";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedClinicalReasoningPipelineStageView }) {
  return (
...
export function GovernedClinicalReasoningPipelinePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalReasoningPipeline({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-reasoning-pipeline-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Reasoning Pipeline"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-reasoning-pipeline">
