...
import { useGovernedDecisionSupportStage } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-support-stage";
import type { GovernedDecisionSupportStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-support-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedDecisionSupportStageStageView }) {
  return (
...
export function GovernedDecisionSupportStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionSupportStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-decision-support-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Decision Support Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-support-stage">
