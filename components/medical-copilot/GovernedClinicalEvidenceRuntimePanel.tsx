...
import { useGovernedClinicalEvidenceRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-runtime";
import type { GovernedClinicalEvidenceRuntimeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-runtime";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalEvidenceRuntimeComponentPresence }) {
  return (
...
export function GovernedClinicalEvidenceRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalEvidenceRuntime({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-evidence-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Evidence Runtime">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-evidence-runtime">
