...
import { useGovernedMissingLaboratoryDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-laboratory-detection-decision-engine";
import type { GovernedMissingLaboratoryDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-laboratory-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedMissingLaboratoryDetectionEngineEntryView }) {
  return (
...
export function GovernedMissingLaboratoryDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMissingLaboratoryDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-missing-laboratory-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Missing Laboratory Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-missing-laboratory-detection-decision-engine">
