...
import { useGovernedSystematicReviewEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-systematic-review-evidence-engine";
import type { GovernedSystematicReviewEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-systematic-review-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedSystematicReviewEvidenceEngineEntryView }) {
  return (
...
export function GovernedSystematicReviewEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSystematicReviewEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-systematic-review-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Systematic Review Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-systematic-review-evidence-engine">
