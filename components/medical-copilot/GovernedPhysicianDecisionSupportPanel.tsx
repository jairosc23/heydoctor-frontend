...
import { useGovernedPhysicianDecisionSupport } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-decision-support";
import type { GovernedPhysicianDecisionSupportComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-decision-support";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedPhysicianDecisionSupportComponentPresence }) {
  return (
...
export function GovernedPhysicianDecisionSupportPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianDecisionSupport({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-physician-decision-support-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Physician Decision Support">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-decision-support">
