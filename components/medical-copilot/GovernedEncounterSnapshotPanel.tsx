...
import { useGovernedEncounterSnapshot } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-snapshot";
import type { GovernedEncounterSnapshotComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-snapshot";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedEncounterSnapshotComponentPresence;
}) {
  return (
...
export function GovernedEncounterSnapshotPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEncounterSnapshot({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-encounter-snapshot-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Encounter Snapshot Gobernado (Fase 26)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del snapshot de encuentro gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-encounter-snapshot">
