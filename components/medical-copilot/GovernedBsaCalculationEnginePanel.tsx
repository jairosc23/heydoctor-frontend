...
import { useGovernedBsaCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-bsa-calculation-engine";
import type { GovernedBsaCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-bsa-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedBsaCalculationEngineEntryView }) {
  return (
...
export function GovernedBsaCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedBsaCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-bsa-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"BSA"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-bsa-calculation-engine">
