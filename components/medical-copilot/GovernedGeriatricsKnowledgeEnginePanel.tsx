...
import { useGovernedGeriatricsKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-geriatrics-knowledge-engine";
import type { GovernedGeriatricsKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-geriatrics-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedGeriatricsKnowledgeEngineEntryView }) {
  return (
...
export function GovernedGeriatricsKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGeriatricsKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-geriatrics-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Geriatrics Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-geriatrics-knowledge-engine">
