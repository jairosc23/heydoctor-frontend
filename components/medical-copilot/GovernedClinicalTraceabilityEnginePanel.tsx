...
import { useGovernedClinicalTraceabilityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-traceability-decision-engine";
import type { GovernedClinicalTraceabilityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-traceability-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalTraceabilityEngineEntryView }) {
  return (
...
export function GovernedClinicalTraceabilityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalTraceabilityEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-traceability-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Traceability Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-traceability-decision-engine">
