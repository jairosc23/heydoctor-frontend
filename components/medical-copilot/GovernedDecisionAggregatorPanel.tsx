...
import { useGovernedDecisionAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-aggregator";
...
export function GovernedDecisionAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-decision-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Decision Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-aggregator">
