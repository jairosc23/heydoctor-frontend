...
import { useGovernedClinicalGuidelineEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guideline-evidence-engine";
import type { GovernedClinicalGuidelineEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guideline-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedClinicalGuidelineEvidenceEngineEntryView }) {
  return (
...
export function GovernedClinicalGuidelineEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalGuidelineEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-guideline-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Guideline Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-guideline-evidence-engine">
