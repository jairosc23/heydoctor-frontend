...
import { useGovernedClinicalSessionWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-session-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-session-workflow";
...
export function GovernedClinicalSessionWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSessionWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-session-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Session Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-session-workflow">
