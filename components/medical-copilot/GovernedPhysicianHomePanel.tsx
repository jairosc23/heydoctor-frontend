...
import { useGovernedPhysicianHome } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-home";
import type { GovernedPhysicianHomeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-home";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPhysicianHomeComponentPresence;
}) {
  return (
...
export function GovernedPhysicianHomePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianHome({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-physician-home-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Physician Home Gobernado (Fase 40)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del home médico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-home">
