...
import { useGovernedEscGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-esc-guideline-engine";
import type { GovernedEscGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-esc-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedEscGuidelineEngineEntryView }) {
  return (
...
export function GovernedEscGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEscGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-esc-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ESC Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-esc-guideline-engine">
