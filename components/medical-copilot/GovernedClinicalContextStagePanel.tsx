...
import { useGovernedClinicalContextStage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-context-stage";
import type { GovernedClinicalContextStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-context-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedClinicalContextStageStageView }) {
  return (
...
export function GovernedClinicalContextStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalContextStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-context-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Context Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-context-stage">
