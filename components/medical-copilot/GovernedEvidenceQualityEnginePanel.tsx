...
import { useGovernedEvidenceQualityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-quality-engine";
import type { GovernedEvidenceQualityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-quality-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceQualityEngineEntryView }) {
  return (
...
export function GovernedEvidenceQualityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceQualityEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-quality-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Quality Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-quality-engine">
