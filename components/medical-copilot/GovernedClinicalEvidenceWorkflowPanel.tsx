...
import { useGovernedClinicalEvidenceWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-workflow";
...
export function GovernedClinicalEvidenceWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalEvidenceWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-evidence-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Evidence Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-evidence-workflow">
