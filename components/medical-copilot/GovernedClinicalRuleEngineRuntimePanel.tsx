...
import { useGovernedClinicalRuleEngineRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-rule-engine-runtime";
import type { GovernedClinicalRuleEngineRuntimeEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-rule-engine-runtime";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedClinicalRuleEngineRuntimeEvaluationView }) {
  return (
...
export function GovernedClinicalRuleEngineRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalRuleEngineRuntime({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-rule-engine-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Rule Engine Runtime"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-rule-engine-runtime">
