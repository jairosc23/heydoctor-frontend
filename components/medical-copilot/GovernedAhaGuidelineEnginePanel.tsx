...
import { useGovernedAhaGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-aha-guideline-engine";
import type { GovernedAhaGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-aha-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAhaGuidelineEngineEntryView }) {
  return (
...
export function GovernedAhaGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAhaGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-aha-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"AHA Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-aha-guideline-engine">
