...
import { useGovernedCurb65CalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-curb65-calculation-engine";
import type { GovernedCurb65CalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-curb65-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCurb65CalculationEngineEntryView }) {
  return (
...
export function GovernedCurb65CalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCurb65CalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-curb65-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CURB-65"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-curb65-calculation-engine">
