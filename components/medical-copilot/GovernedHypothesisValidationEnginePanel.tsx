...
import { useGovernedHypothesisValidationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-hypothesis-validation-decision-engine";
import type { GovernedHypothesisValidationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-hypothesis-validation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedHypothesisValidationEngineEntryView }) {
  return (
...
export function GovernedHypothesisValidationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHypothesisValidationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-hypothesis-validation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Hypothesis Validation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-hypothesis-validation-decision-engine">
