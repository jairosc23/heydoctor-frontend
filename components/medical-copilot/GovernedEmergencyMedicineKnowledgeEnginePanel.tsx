...
import { useGovernedEmergencyMedicineKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-emergency-medicine-knowledge-engine";
import type { GovernedEmergencyMedicineKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-emergency-medicine-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedEmergencyMedicineKnowledgeEngineEntryView }) {
  return (
...
export function GovernedEmergencyMedicineKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEmergencyMedicineKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-emergency-medicine-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Emergency Medicine Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-emergency-medicine-knowledge-engine">
