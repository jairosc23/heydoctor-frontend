...
import { useGovernedDiseaseKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-disease-knowledge-engine";
import type { GovernedDiseaseKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-disease-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedDiseaseKnowledgeEngineEntryView }) {
  return (
...
export function GovernedDiseaseKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiseaseKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-disease-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Disease Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-disease-knowledge-engine">
