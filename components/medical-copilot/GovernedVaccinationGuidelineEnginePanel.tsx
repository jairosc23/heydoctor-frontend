...
import { useGovernedVaccinationGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-guideline-engine";
import type { GovernedVaccinationGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedVaccinationGuidelineEngineEntryView }) {
  return (
...
export function GovernedVaccinationGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedVaccinationGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-vaccination-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Vaccination Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-vaccination-guideline-engine">
