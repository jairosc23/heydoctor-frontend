...
import { useGovernedPopulationAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-population-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-population-aggregator";
...
export function GovernedPopulationAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPopulationAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-population-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Population Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-population-aggregator">
