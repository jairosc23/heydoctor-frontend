...
import { useGovernedEvidenceConsistencyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-consistency-engine";
import type { GovernedEvidenceConsistencyEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-consistency-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceConsistencyEngineEntryView }) {
  return (
...
export function GovernedEvidenceConsistencyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceConsistencyEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-consistency-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Consistency Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-consistency-engine">
