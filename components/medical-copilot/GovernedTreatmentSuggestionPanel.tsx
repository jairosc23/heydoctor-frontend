...
import { useGovernedTreatmentSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-treatment-suggestion";
import type { GovernedTreatmentSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-treatment-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedTreatmentSuggestionComponentPresence }) {
  return (
...
export function GovernedTreatmentSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedTreatmentSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-treatment-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Treatment Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-treatment-suggestion">
