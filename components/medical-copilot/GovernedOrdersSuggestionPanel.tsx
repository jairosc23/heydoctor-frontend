...
import { useGovernedOrdersSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-suggestion";
import type { GovernedOrdersSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedOrdersSuggestionComponentPresence }) {
  return (
...
export function GovernedOrdersSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedOrdersSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-orders-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Orders Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-orders-suggestion">
