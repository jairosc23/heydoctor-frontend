...
import { useGovernedFraminghamCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-framingham-calculation-engine";
import type { GovernedFraminghamCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-framingham-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedFraminghamCalculationEngineEntryView }) {
  return (
...
export function GovernedFraminghamCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedFraminghamCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-framingham-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Framingham"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-framingham-calculation-engine">
