...
import { useGovernedEvidenceProvenanceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-provenance-engine";
import type { GovernedEvidenceProvenanceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-provenance-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceProvenanceEngineEntryView }) {
  return (
...
export function GovernedEvidenceProvenanceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceProvenanceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-provenance-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Provenance Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-provenance-engine">
