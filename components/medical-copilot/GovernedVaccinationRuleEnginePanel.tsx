...
import { useGovernedVaccinationRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-rule-engine";
import type { GovernedVaccinationRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedVaccinationRuleEngineEvaluationView }) {
  return (
...
export function GovernedVaccinationRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedVaccinationRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-vaccination-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Vaccination Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-vaccination-rule-engine">
