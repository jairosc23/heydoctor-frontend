...
import { useGovernedCaseSeriesEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-case-series-evidence-engine";
import type { GovernedCaseSeriesEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-case-series-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedCaseSeriesEvidenceEngineEntryView }) {
  return (
...
export function GovernedCaseSeriesEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCaseSeriesEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-case-series-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Case Series Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-case-series-evidence-engine">
