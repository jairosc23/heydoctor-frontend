...
import { useGovernedClinicalPopulationWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-population-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-population-workflow";
...
export function GovernedClinicalPopulationWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalPopulationWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-population-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Population Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-population-workflow">
