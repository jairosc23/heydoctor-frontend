...
import { useGovernedPreventiveCareSuggestions } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-care-suggestions";
import type { GovernedPreventiveCareSuggestionsComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-care-suggestions";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedPreventiveCareSuggestionsComponentPresence }) {
  return (
...
export function GovernedPreventiveCareSuggestionsPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveCareSuggestions({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-care-suggestions-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Preventive Care Suggestions">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-care-suggestions">
