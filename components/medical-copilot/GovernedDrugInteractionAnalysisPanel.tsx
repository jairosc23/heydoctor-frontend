...
import { useGovernedDrugInteractionAnalysis } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-analysis";
import type { GovernedDrugInteractionAnalysisComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-analysis";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedDrugInteractionAnalysisComponentPresence }) {
  return (
...
export function GovernedDrugInteractionAnalysisPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDrugInteractionAnalysis({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-drug-interaction-analysis-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Drug Interaction Analysis">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-drug-interaction-analysis">
