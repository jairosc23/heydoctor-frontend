...
import { useGovernedGlasgowCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-glasgow-calculation-engine";
import type { GovernedGlasgowCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-glasgow-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedGlasgowCalculationEngineEntryView }) {
  return (
...
export function GovernedGlasgowCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGlasgowCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-glasgow-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Glasgow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-glasgow-calculation-engine">
