...
import { useGovernedAhaEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-aha-evidence-engine";
import type { GovernedAhaEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-aha-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedAhaEvidenceEngineEntryView }) {
  return (
...
export function GovernedAhaEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAhaEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-aha-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"AHA Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-aha-evidence-engine">
