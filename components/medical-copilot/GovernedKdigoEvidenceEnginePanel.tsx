...
import { useGovernedKdigoEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-kdigo-evidence-engine";
import type { GovernedKdigoEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-kdigo-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedKdigoEvidenceEngineEntryView }) {
  return (
...
export function GovernedKdigoEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedKdigoEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-kdigo-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"KDIGO Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-kdigo-evidence-engine">
