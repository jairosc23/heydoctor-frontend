...
import { useGovernedClinicalActionCandidateEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-action-candidate-decision-engine";
import type { GovernedClinicalActionCandidateEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-action-candidate-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalActionCandidateEngineEntryView }) {
  return (
...
export function GovernedClinicalActionCandidateEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalActionCandidateEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-action-candidate-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Action Candidate Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-action-candidate-decision-engine">
