...
import { useGovernedContraindicationRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-rule-engine";
import type { GovernedContraindicationRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-contraindication-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedContraindicationRuleEngineEvaluationView }) {
  return (
...
export function GovernedContraindicationRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedContraindicationRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-contraindication-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Contraindication Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-contraindication-rule-engine">
