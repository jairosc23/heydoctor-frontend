...
import { useGovernedGinaGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-gina-guideline-engine";
import type { GovernedGinaGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-gina-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedGinaGuidelineEngineEntryView }) {
  return (
...
export function GovernedGinaGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGinaGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-gina-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"GINA Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-gina-guideline-engine">
