...
import { useGovernedConsultationReview } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-review";
import type { GovernedConsultationReviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-review";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationReviewComponentPresence;
}) {
  return (
...
export function GovernedConsultationReviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationReview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-review-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Review Gobernado (Fase 22)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la revisión de consulta gobernada · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-review">
