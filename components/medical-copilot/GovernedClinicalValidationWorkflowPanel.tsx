...
import { useGovernedClinicalValidationWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-validation-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-validation-workflow";
...
export function GovernedClinicalValidationWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalValidationWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-validation-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Validation Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-validation-workflow">
