...
import { useGovernedCalculationAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-aggregator";
...
export function GovernedCalculationAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCalculationAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-calculation-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Calculation Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-calculation-aggregator">
