...
import { useGovernedGuidelineAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-aggregator";
...
export function GovernedGuidelineAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGuidelineAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-guideline-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Guideline Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-guideline-aggregator">
