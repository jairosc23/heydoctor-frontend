...
import { useGovernedClinicalAssessmentSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-assessment-suggestion";
import type { GovernedClinicalAssessmentSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-assessment-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalAssessmentSuggestionComponentPresence }) {
  return (
...
export function GovernedClinicalAssessmentSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAssessmentSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-assessment-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Assessment Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-assessment-suggestion">
