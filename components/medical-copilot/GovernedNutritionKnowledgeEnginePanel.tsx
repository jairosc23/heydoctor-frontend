...
import { useGovernedNutritionKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-nutrition-knowledge-engine";
import type { GovernedNutritionKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-nutrition-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedNutritionKnowledgeEngineEntryView }) {
  return (
...
export function GovernedNutritionKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNutritionKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-nutrition-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Nutrition Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-nutrition-knowledge-engine">
