...
import { useGovernedAuditAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-audit-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-audit-aggregator";
...
export function GovernedAuditAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAuditAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-audit-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Audit Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-audit-aggregator">
