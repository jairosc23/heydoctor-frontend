...
import { useGovernedClinicalSummaryStage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-summary-stage";
import type { GovernedClinicalSummaryStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-summary-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedClinicalSummaryStageStageView }) {
  return (
...
export function GovernedClinicalSummaryStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSummaryStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-summary-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Summary Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-summary-stage">
