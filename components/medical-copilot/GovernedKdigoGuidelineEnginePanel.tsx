...
import { useGovernedKdigoGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-kdigo-guideline-engine";
import type { GovernedKdigoGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-kdigo-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedKdigoGuidelineEngineEntryView }) {
  return (
...
export function GovernedKdigoGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedKdigoGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-kdigo-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"KDIGO Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-kdigo-guideline-engine">
