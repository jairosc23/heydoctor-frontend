...
import { useGovernedPendingActions } from "@/lib/medical-copilot/clinical-intelligence/governed-pending-actions";
import type { GovernedPendingActionsComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-pending-actions";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPendingActionsComponentPresence;
}) {
  return (
...
export function GovernedPendingActionsPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPendingActions({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-pending-actions-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Pending Actions Gobernado (Fase 55)">
          <p className="mb-3 text-xs text-slate-500">
            Estados temporales de acciones pendientes · Solo lectura · NO ejecuta · Sin persistencia · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-pending-actions">
