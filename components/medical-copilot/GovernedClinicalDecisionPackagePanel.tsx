...
import { useGovernedClinicalDecisionPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-package";
import type { GovernedClinicalDecisionPackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalDecisionPackageComponentPresence }) {
  return (
...
export function GovernedClinicalDecisionPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDecisionPackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-decision-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Decision Package">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-decision-package">
