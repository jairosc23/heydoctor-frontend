...
import { useGovernedEvidenceHierarchyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-hierarchy-engine";
import type { GovernedEvidenceHierarchyEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-hierarchy-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceHierarchyEngineEntryView }) {
  return (
...
export function GovernedEvidenceHierarchyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceHierarchyEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-hierarchy-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Hierarchy Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-hierarchy-engine">
