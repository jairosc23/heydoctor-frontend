...
import { useGovernedDrugInteractionRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-rule-engine";
import type { GovernedDrugInteractionRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-drug-interaction-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedDrugInteractionRuleEngineEvaluationView }) {
  return (
...
export function GovernedDrugInteractionRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDrugInteractionRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-drug-interaction-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Drug Interaction Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-drug-interaction-rule-engine">
