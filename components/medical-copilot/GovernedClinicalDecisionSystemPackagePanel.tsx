...
import { useGovernedClinicalDecisionSystemPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-system-package";
import type { GovernedClinicalDecisionSystemPackageEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-system-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalDecisionSystemPackageEntryView }) {
  return (
...
export function GovernedClinicalDecisionSystemPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDecisionSystemPackage({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-decision-system-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Decision Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-decision-system-package">
