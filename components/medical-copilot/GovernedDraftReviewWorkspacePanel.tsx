...
import { useGovernedDraftReviewWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-draft-review-workspace";
import type { GovernedDraftReviewWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-draft-review-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedDraftReviewWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedDraftReviewWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDraftReviewWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-draft-review-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Draft Review Workspace Gobernado (Fase 50)">
          <p className="mb-3 text-xs text-slate-500">
            Revisión estructural de drafts gobernados · Solo lectura · Sin generación · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-draft-review-workspace">
