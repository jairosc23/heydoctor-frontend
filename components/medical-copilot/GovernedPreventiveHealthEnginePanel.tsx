...
import { useGovernedPreventiveHealthEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-health-engine";
import type { GovernedPreventiveHealthEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-health-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedPreventiveHealthEngineComponentPresence }) {
  return (
...
export function GovernedPreventiveHealthEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveHealthEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-health-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Preventive Health Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-health-engine">
