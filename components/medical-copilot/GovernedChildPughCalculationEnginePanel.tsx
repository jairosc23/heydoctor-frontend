...
import { useGovernedChildPughCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-child-pugh-calculation-engine";
import type { GovernedChildPughCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-child-pugh-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedChildPughCalculationEngineEntryView }) {
  return (
...
export function GovernedChildPughCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedChildPughCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-child-pugh-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Child-Pugh"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-child-pugh-calculation-engine">
