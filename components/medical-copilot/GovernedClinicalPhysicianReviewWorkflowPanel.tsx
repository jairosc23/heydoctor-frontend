...
import { useGovernedClinicalPhysicianReviewWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-physician-review-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-physician-review-workflow";
...
export function GovernedClinicalPhysicianReviewWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalPhysicianReviewWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-physician-review-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Physician Review Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-physician-review-workflow">
