...
import { useGovernedMedicationKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-medication-knowledge-engine";
import type { GovernedMedicationKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-medication-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedMedicationKnowledgeEngineEntryView }) {
  return (
...
export function GovernedMedicationKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMedicationKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-medication-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Medication Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-medication-knowledge-engine">
