...
import { useGovernedAllergyCrossCheck } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-cross-check";
import type { GovernedAllergyCrossCheckComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-cross-check";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedAllergyCrossCheckComponentPresence }) {
  return (
...
export function GovernedAllergyCrossCheckPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAllergyCrossCheck({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-allergy-cross-check-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Allergy Cross Check">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-allergy-cross-check">
