...
import { useGovernedPersistenceNavigation } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-navigation";
import type { GovernedPersistenceNavigationComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-navigation";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceNavigationComponentPresence;
}) {
  return (
...
export function GovernedPersistenceNavigationPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceNavigation({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-navigation-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Navigation Gobernado (Fase 72)">
          <p className="mb-3 text-xs text-slate-500">
            Navegación de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-navigation">
