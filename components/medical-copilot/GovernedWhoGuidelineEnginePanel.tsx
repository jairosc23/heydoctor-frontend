...
import { useGovernedWhoGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-who-guideline-engine";
import type { GovernedWhoGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-who-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedWhoGuidelineEngineEntryView }) {
  return (
...
export function GovernedWhoGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWhoGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-who-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"WHO Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-who-guideline-engine">
