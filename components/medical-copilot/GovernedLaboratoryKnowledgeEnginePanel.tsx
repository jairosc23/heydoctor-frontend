...
import { useGovernedLaboratoryKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-laboratory-knowledge-engine";
import type { GovernedLaboratoryKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-laboratory-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedLaboratoryKnowledgeEngineEntryView }) {
  return (
...
export function GovernedLaboratoryKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedLaboratoryKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-laboratory-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Laboratory Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-laboratory-knowledge-engine">
