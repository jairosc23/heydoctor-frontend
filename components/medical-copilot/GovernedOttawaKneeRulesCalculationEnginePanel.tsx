...
import { useGovernedOttawaKneeRulesCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ottawa-knee-rules-calculation-engine";
import type { GovernedOttawaKneeRulesCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ottawa-knee-rules-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedOttawaKneeRulesCalculationEngineEntryView }) {
  return (
...
export function GovernedOttawaKneeRulesCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedOttawaKneeRulesCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-ottawa-knee-rules-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Ottawa Knee Rules"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ottawa-knee-rules-calculation-engine">
