...
import { useGovernedDiabetesCareEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-diabetes-care-engine";
import type { GovernedDiabetesCareEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-diabetes-care-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedDiabetesCareEngineComponentPresence }) {
  return (
...
export function GovernedDiabetesCareEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiabetesCareEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-diabetes-care-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Diabetes Care Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diabetes-care-engine">
