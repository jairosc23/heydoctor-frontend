...
import { useGovernedClinicalAlertCenter } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-alert-center";
import type { GovernedClinicalAlertCenterComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-alert-center";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalAlertCenterComponentPresence }) {
  return (
...
export function GovernedClinicalAlertCenterPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAlertCenter({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-alert-center-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Alert Center">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-alert-center">
