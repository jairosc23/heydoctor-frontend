...
import { useGovernedProcedureKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-procedure-knowledge-engine";
import type { GovernedProcedureKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-procedure-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedProcedureKnowledgeEngineEntryView }) {
  return (
...
export function GovernedProcedureKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedProcedureKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-procedure-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Procedure Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-procedure-knowledge-engine">
