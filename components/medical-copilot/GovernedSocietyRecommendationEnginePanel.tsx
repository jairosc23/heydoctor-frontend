...
import { useGovernedSocietyRecommendationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-society-recommendation-engine";
import type { GovernedSocietyRecommendationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-society-recommendation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedSocietyRecommendationEngineEntryView }) {
  return (
...
export function GovernedSocietyRecommendationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSocietyRecommendationEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-society-recommendation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Society Recommendation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-society-recommendation-engine">
