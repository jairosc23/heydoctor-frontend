...
import { useGovernedAscvdCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ascvd-calculation-engine";
import type { GovernedAscvdCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ascvd-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedAscvdCalculationEngineEntryView }) {
  return (
...
export function GovernedAscvdCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAscvdCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-ascvd-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ASCVD"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ascvd-calculation-engine">
