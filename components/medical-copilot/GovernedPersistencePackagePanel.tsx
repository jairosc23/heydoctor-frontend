...
import { useGovernedPersistencePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-package";
import type { GovernedPersistencePackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistencePackageComponentPresence;
}) {
  return (
...
export function GovernedPersistencePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistencePackage({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Package Gobernado (Fase 78)">
          <p className="mb-3 text-xs text-slate-500">
            Contrato oficial previo a la persistencia futura · Solo lectura · NO persiste · NO aprueba · NO ejecuta · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-package">
