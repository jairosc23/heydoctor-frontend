...
import { useGovernedChronicDiseaseRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-chronic-disease-rule-engine";
import type { GovernedChronicDiseaseRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-chronic-disease-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedChronicDiseaseRuleEngineEvaluationView }) {
  return (
...
export function GovernedChronicDiseaseRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedChronicDiseaseRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-chronic-disease-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Chronic Disease Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-chronic-disease-rule-engine">
