...
import { useGovernedPrescriptionPersistenceExecution } from "@/lib/medical-copilot/clinical-intelligence/governed-prescription-persistence-execution";
import type { GovernedPrescriptionPersistenceExecutionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-prescription-persistence-execution";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";
function ComponentPresenceCard({ component }: { component: GovernedPrescriptionPersistenceExecutionComponentPresence }) {
  return (
...
export function GovernedPrescriptionPersistenceExecutionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPrescriptionPersistenceExecution({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-prescription-persistence-execution-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Prescription Persistence Execution">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-prescription-persistence-execution">
