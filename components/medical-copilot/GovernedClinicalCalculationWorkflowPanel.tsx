...
import { useGovernedClinicalCalculationWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-calculation-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-calculation-workflow";
...
export function GovernedClinicalCalculationWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalCalculationWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-calculation-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Calculation Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-calculation-workflow">
