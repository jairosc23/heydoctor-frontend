...
import { useGovernedReasoningAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-reasoning-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-reasoning-aggregator";
...
export function GovernedReasoningAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedReasoningAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-reasoning-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Reasoning Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-reasoning-aggregator">
