...
import { useGovernedClinicalAlertRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-alert-rule-engine";
import type { GovernedClinicalAlertRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-alert-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedClinicalAlertRuleEngineEvaluationView }) {
  return (
...
export function GovernedClinicalAlertRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAlertRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-alert-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Clinical Alert Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-alert-rule-engine">
