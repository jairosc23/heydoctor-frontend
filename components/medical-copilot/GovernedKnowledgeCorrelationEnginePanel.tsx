...
import { useGovernedKnowledgeCorrelationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-knowledge-correlation-decision-engine";
import type { GovernedKnowledgeCorrelationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-knowledge-correlation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedKnowledgeCorrelationEngineEntryView }) {
  return (
...
export function GovernedKnowledgeCorrelationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedKnowledgeCorrelationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-knowledge-correlation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Knowledge Correlation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-knowledge-correlation-decision-engine">
