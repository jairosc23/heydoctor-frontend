...
import { useGovernedCdcGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-cdc-guideline-engine";
import type { GovernedCdcGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-cdc-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedCdcGuidelineEngineEntryView }) {
  return (
...
export function GovernedCdcGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCdcGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-cdc-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CDC Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-cdc-guideline-engine">
