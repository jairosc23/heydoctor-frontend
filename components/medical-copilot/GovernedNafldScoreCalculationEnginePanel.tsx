...
import { useGovernedNafldScoreCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-nafld-score-calculation-engine";
import type { GovernedNafldScoreCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-nafld-score-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedNafldScoreCalculationEngineEntryView }) {
  return (
...
export function GovernedNafldScoreCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNafldScoreCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-nafld-score-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"NAFLD Score"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-nafld-score-calculation-engine">
