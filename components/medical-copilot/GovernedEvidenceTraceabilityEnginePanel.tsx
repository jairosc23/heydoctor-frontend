...
import { useGovernedEvidenceTraceabilityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-traceability-engine";
import type { GovernedEvidenceTraceabilityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-traceability-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceTraceabilityEngineEntryView }) {
  return (
...
export function GovernedEvidenceTraceabilityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceTraceabilityEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-traceability-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Traceability Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-traceability-engine">
