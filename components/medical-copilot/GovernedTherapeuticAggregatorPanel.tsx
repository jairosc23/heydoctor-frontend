...
import { useGovernedTherapeuticAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-therapeutic-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-therapeutic-aggregator";
...
export function GovernedTherapeuticAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedTherapeuticAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-therapeutic-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Therapeutic Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-therapeutic-aggregator">
