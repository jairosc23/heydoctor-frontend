...
import { useGovernedRedFlagKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-red-flag-knowledge-engine";
import type { GovernedRedFlagKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-red-flag-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedRedFlagKnowledgeEngineEntryView }) {
  return (
...
export function GovernedRedFlagKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRedFlagKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-red-flag-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Red Flag Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-red-flag-knowledge-engine">
