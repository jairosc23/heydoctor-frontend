...
import { useGovernedAllergyKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-knowledge-engine";
import type { GovernedAllergyKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedAllergyKnowledgeEngineEntryView }) {
  return (
...
export function GovernedAllergyKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAllergyKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-allergy-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Allergy Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-allergy-knowledge-engine">
