...
import { useGovernedDiagnosticCriteriaKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-criteria-knowledge-engine";
import type { GovernedDiagnosticCriteriaKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-criteria-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedDiagnosticCriteriaKnowledgeEngineEntryView }) {
  return (
...
export function GovernedDiagnosticCriteriaKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiagnosticCriteriaKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-diagnostic-criteria-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Diagnostic Criteria Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diagnostic-criteria-knowledge-engine">
