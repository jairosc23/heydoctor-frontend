...
import { useGovernedDecisionConfidenceAggregationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-confidence-aggregation-engine";
import type { GovernedDecisionConfidenceAggregationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-confidence-aggregation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDecisionConfidenceAggregationEngineEntryView }) {
  return (
...
export function GovernedDecisionConfidenceAggregationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionConfidenceAggregationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-decision-confidence-aggregation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Decision Confidence Aggregation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-confidence-aggregation-engine">
