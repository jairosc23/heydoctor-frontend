...
import { useGovernedSurvivingSepsisGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-surviving-sepsis-guideline-engine";
import type { GovernedSurvivingSepsisGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-surviving-sepsis-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedSurvivingSepsisGuidelineEngineEntryView }) {
  return (
...
export function GovernedSurvivingSepsisGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSurvivingSepsisGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-surviving-sepsis-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Surviving Sepsis Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-surviving-sepsis-guideline-engine">
