...
import { useGovernedEncounterTimeline } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-timeline";
import type { GovernedEncounterTimelineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-timeline";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedEncounterTimelineComponentPresence;
}) {
  return (
...
export function GovernedEncounterTimelinePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEncounterTimeline({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-encounter-timeline-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Encounter Timeline Gobernado (Fase 43)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del timeline de encuentro gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-encounter-timeline">
