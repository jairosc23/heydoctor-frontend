...
import { useGovernedClinicalPersistenceWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-persistence-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-persistence-workflow";
...
export function GovernedClinicalPersistenceWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalPersistenceWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-persistence-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Persistence Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-persistence-workflow">
