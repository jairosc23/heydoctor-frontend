...
import { useGovernedCopdGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-copd-guideline-engine";
import type { GovernedCopdGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-copd-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedCopdGuidelineEngineEntryView }) {
  return (
...
export function GovernedCopdGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCopdGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-copd-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"COPD Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-copd-guideline-engine">
