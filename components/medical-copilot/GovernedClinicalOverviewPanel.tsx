...
import { useGovernedClinicalOverview } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-overview";
import type { GovernedClinicalOverviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-overview";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalOverviewComponentPresence;
}) {
  return (
...
export function GovernedClinicalOverviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalOverview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-overview-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Overview Gobernado (Fase 37)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del overview clínico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-overview">
