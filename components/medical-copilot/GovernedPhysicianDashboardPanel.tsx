...
import { useGovernedPhysicianDashboard } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-dashboard";
import type { GovernedPhysicianDashboardComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-dashboard";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPhysicianDashboardComponentPresence;
}) {
  return (
...
export function GovernedPhysicianDashboardPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianDashboard({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-physician-dashboard-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Physician Dashboard Gobernado (Fase 34)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del dashboard médico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-dashboard">
