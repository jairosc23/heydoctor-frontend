...
import { useGovernedApgarCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-apgar-calculation-engine";
import type { GovernedApgarCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-apgar-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedApgarCalculationEngineEntryView }) {
  return (
...
export function GovernedApgarCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedApgarCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-apgar-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"APGAR"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-apgar-calculation-engine">
