...
import { useGovernedClinicalReviewPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-review-package";
import type { GovernedClinicalReviewPackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-review-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalReviewPackageComponentPresence;
}) {
  return (
...
export function GovernedClinicalReviewPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalReviewPackage({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-review-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Review Package Gobernado (Fase 56)">
          <p className="mb-3 text-xs text-slate-500">
            Paquete de revisión clínica gobernada · Solo lectura · Sin persistencia · Sin ejecución · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-review-package">
