...
import { useGovernedWellsDvtCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-wells-dvt-calculation-engine";
import type { GovernedWellsDvtCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-wells-dvt-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedWellsDvtCalculationEngineEntryView }) {
  return (
...
export function GovernedWellsDvtCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWellsDvtCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-wells-dvt-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Wells DVT"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-wells-dvt-calculation-engine">
