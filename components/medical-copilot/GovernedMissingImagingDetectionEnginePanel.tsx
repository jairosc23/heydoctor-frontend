...
import { useGovernedMissingImagingDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-imaging-detection-decision-engine";
import type { GovernedMissingImagingDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-imaging-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedMissingImagingDetectionEngineEntryView }) {
  return (
...
export function GovernedMissingImagingDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMissingImagingDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-missing-imaging-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Missing Imaging Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-missing-imaging-detection-decision-engine">
