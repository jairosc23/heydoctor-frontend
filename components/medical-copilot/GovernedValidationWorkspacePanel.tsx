...
import { useGovernedValidationWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-validation-workspace";
import type { GovernedValidationWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-validation-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedValidationWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedValidationWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedValidationWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-validation-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Validation Workspace Gobernado (Fase 52)">
          <p className="mb-3 text-xs text-slate-500">
            Workspace de validación gobernada · Solo lectura · Sin persistencia · Sin ejecución · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-validation-workspace">
