...
import { useGovernedEvidenceMapping } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-mapping";
import type { GovernedEvidenceMappingComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-mapping";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedEvidenceMappingComponentPresence }) {
  return (
...
export function GovernedEvidenceMappingPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceMapping({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-mapping-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Evidence Mapping">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-mapping">
