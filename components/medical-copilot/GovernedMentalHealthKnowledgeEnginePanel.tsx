...
import { useGovernedMentalHealthKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-mental-health-knowledge-engine";
import type { GovernedMentalHealthKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-mental-health-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedMentalHealthKnowledgeEngineEntryView }) {
  return (
...
export function GovernedMentalHealthKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMentalHealthKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-mental-health-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Mental Health Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-mental-health-knowledge-engine">
