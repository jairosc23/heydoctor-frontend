...
import { useGovernedGuidelineTraceabilityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-traceability-engine";
import type { GovernedGuidelineTraceabilityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-traceability-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedGuidelineTraceabilityEngineEntryView }) {
  return (
...
export function GovernedGuidelineTraceabilityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGuidelineTraceabilityEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-guideline-traceability-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Guideline Traceability Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-guideline-traceability-engine">
