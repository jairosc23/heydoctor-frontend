...
import { useGovernedDifferentialDiagnosisSuggestion } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-diagnosis-suggestion";
import type { GovernedDifferentialDiagnosisSuggestionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-diagnosis-suggestion";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedDifferentialDiagnosisSuggestionComponentPresence }) {
  return (
...
export function GovernedDifferentialDiagnosisSuggestionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDifferentialDiagnosisSuggestion({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-differential-diagnosis-suggestion-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Differential Diagnosis Suggestion">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-differential-diagnosis-suggestion">
