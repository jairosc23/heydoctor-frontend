...
import { useGovernedAcogGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-acog-guideline-engine";
import type { GovernedAcogGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-acog-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAcogGuidelineEngineEntryView }) {
  return (
...
export function GovernedAcogGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAcogGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-acog-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ACOG Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-acog-guideline-engine">
