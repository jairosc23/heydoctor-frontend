...
import { useGovernedHypertensionManagementEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-hypertension-management-engine";
import type { GovernedHypertensionManagementEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-hypertension-management-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedHypertensionManagementEngineComponentPresence }) {
  return (
...
export function GovernedHypertensionManagementEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHypertensionManagementEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-hypertension-management-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Hypertension Management Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-hypertension-management-engine">
