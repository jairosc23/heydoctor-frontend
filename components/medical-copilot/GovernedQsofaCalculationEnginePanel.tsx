...
import { useGovernedQsofaCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-qsofa-calculation-engine";
import type { GovernedQsofaCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-qsofa-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedQsofaCalculationEngineEntryView }) {
  return (
...
export function GovernedQsofaCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedQsofaCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-qsofa-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"qSOFA"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-qsofa-calculation-engine">
