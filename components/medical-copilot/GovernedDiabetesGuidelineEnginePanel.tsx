...
import { useGovernedDiabetesGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-diabetes-guideline-engine";
import type { GovernedDiabetesGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-diabetes-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedDiabetesGuidelineEngineEntryView }) {
  return (
...
export function GovernedDiabetesGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiabetesGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-diabetes-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Diabetes Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diabetes-guideline-engine">
