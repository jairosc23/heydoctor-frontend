...
import { useGovernedConsultationWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-workspace";
import type { GovernedConsultationWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedConsultationWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Workspace Gobernado (Fase 23)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del workspace de consulta gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-workspace">
