...
import { useGovernedHasBledCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-has-bled-calculation-engine";
import type { GovernedHasBledCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-has-bled-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedHasBledCalculationEngineEntryView }) {
  return (
...
export function GovernedHasBledCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHasBledCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-has-bled-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"HAS-BLED"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-has-bled-calculation-engine">
