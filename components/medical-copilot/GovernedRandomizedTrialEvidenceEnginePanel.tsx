...
import { useGovernedRandomizedTrialEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-randomized-trial-evidence-engine";
import type { GovernedRandomizedTrialEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-randomized-trial-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedRandomizedTrialEvidenceEngineEntryView }) {
  return (
...
export function GovernedRandomizedTrialEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRandomizedTrialEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-randomized-trial-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Randomized Trial Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-randomized-trial-evidence-engine">
