...
import { useGovernedPersistenceSession } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-session";
import type { GovernedPersistenceSessionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-session";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceSessionComponentPresence;
}) {
  return (
...
export function GovernedPersistenceSessionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceSession({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-session-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Session Gobernado (Fase 74)">
          <p className="mb-3 text-xs text-slate-500">
            Sesión de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-session">
