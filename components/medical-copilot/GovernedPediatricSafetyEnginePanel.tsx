...
import { useGovernedPediatricSafetyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-pediatric-safety-engine";
import type { GovernedPediatricSafetyEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-pediatric-safety-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedPediatricSafetyEngineComponentPresence }) {
  return (
...
export function GovernedPediatricSafetyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPediatricSafetyEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-pediatric-safety-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Pediatric Safety Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-pediatric-safety-engine">
