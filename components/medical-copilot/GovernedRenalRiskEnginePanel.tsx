...
import { useGovernedRenalRiskEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-renal-risk-engine";
import type { GovernedRenalRiskEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-renal-risk-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedRenalRiskEngineComponentPresence }) {
  return (
...
export function GovernedRenalRiskEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRenalRiskEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-renal-risk-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Renal Risk Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-renal-risk-engine">
