...
import { useGovernedCkdEpiCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ckd-epi-calculation-engine";
import type { GovernedCkdEpiCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ckd-epi-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCkdEpiCalculationEngineEntryView }) {
  return (
...
export function GovernedCkdEpiCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCkdEpiCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-ckd-epi-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CKD-EPI"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ckd-epi-calculation-engine">
