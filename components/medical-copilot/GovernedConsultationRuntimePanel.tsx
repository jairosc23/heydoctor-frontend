...
import { useGovernedConsultationRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-runtime";
import type { GovernedConsultationRuntimeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-runtime";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationRuntimeComponentPresence;
}) {
  return (
...
export function GovernedConsultationRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationRuntime({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Runtime Gobernado (Fase 20)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del runtime de consulta gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-runtime">
