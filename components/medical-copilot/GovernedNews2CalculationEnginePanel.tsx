...
import { useGovernedNews2CalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-news2-calculation-engine";
import type { GovernedNews2CalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-news2-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedNews2CalculationEngineEntryView }) {
  return (
...
export function GovernedNews2CalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNews2CalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-news2-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"NEWS2"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-news2-calculation-engine">
