...
import { useGovernedPublicHealthKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-public-health-knowledge-engine";
import type { GovernedPublicHealthKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-public-health-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedPublicHealthKnowledgeEngineEntryView }) {
  return (
...
export function GovernedPublicHealthKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPublicHealthKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-public-health-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Public Health Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-public-health-knowledge-engine">
