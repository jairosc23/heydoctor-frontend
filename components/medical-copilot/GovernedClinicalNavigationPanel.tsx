...
import { useGovernedClinicalNavigation } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-navigation";
import type { GovernedClinicalNavigationComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-navigation";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalNavigationComponentPresence;
}) {
  return (
...
export function GovernedClinicalNavigationPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalNavigation({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-navigation-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Navigation Gobernado (Fase 44)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la navegación clínica gobernada · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-navigation">
