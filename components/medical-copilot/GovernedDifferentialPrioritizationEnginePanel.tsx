...
import { useGovernedDifferentialPrioritizationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-prioritization-decision-engine";
import type { GovernedDifferentialPrioritizationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-differential-prioritization-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDifferentialPrioritizationEngineEntryView }) {
  return (
...
export function GovernedDifferentialPrioritizationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDifferentialPrioritizationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-differential-prioritization-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Differential Prioritization Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-differential-prioritization-decision-engine">
