...
import { useGovernedVaccineKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccine-knowledge-engine";
import type { GovernedVaccineKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccine-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedVaccineKnowledgeEngineEntryView }) {
  return (
...
export function GovernedVaccineKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedVaccineKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-vaccine-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Vaccine Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-vaccine-knowledge-engine">
