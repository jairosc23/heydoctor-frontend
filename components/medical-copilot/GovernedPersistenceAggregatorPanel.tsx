...
import { useGovernedPersistenceAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-aggregator";
...
export function GovernedPersistenceAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-persistence-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Persistence Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-aggregator">
