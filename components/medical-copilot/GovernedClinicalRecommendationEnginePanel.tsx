...
import { useGovernedClinicalRecommendationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-recommendation-decision-engine";
import type { GovernedClinicalRecommendationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-recommendation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalRecommendationEngineEntryView }) {
  return (
...
export function GovernedClinicalRecommendationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalRecommendationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-recommendation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Recommendation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-recommendation-decision-engine">
