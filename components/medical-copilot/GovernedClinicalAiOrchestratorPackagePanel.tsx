...
import { useGovernedClinicalAiOrchestratorPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-ai-orchestrator-package";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-ai-orchestrator-package";
...
export function GovernedClinicalAiOrchestratorPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAiOrchestratorPackage({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-ai-orchestrator-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical AI Orchestrator Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-ai-orchestrator-package">
