...
import { useGovernedEvidenceCorrelationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-correlation-decision-engine";
import type { GovernedEvidenceCorrelationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-correlation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedEvidenceCorrelationEngineEntryView }) {
  return (
...
export function GovernedEvidenceCorrelationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceCorrelationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-evidence-correlation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Correlation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-correlation-decision-engine">
