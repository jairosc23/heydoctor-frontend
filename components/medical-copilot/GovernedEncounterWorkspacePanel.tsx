...
import { useGovernedEncounterWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-workspace";
import type { GovernedEncounterWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-encounter-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedEncounterWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedEncounterWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEncounterWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-encounter-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Encounter Workspace Gobernado (Fase 24)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del workspace de encuentro gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-encounter-workspace">
