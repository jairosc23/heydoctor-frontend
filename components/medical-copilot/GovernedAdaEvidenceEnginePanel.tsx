...
import { useGovernedAdaEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ada-evidence-engine";
import type { GovernedAdaEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ada-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedAdaEvidenceEngineEntryView }) {
  return (
...
export function GovernedAdaEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAdaEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-ada-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ADA Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ada-evidence-engine">
