...
import { useGovernedClinicalDocumentationWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-documentation-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-documentation-workflow";
...
export function GovernedClinicalDocumentationWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDocumentationWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-documentation-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Documentation Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-documentation-workflow">
