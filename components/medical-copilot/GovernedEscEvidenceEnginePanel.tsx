...
import { useGovernedEscEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-esc-evidence-engine";
import type { GovernedEscEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-esc-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEscEvidenceEngineEntryView }) {
  return (
...
export function GovernedEscEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEscEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-esc-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ESC Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-esc-evidence-engine">
