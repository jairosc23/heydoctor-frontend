...
import { useGovernedConsultationDashboard } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-dashboard";
import type { GovernedConsultationDashboardComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-dashboard";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationDashboardComponentPresence;
}) {
  return (
...
export function GovernedConsultationDashboardPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationDashboard({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-dashboard-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Dashboard Gobernado (Fase 33)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del dashboard de consulta gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-dashboard">
