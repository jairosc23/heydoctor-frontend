...
import { useGovernedDiagnosticGapDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-gap-detection-decision-engine";
import type { GovernedDiagnosticGapDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-gap-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDiagnosticGapDetectionEngineEntryView }) {
  return (
...
export function GovernedDiagnosticGapDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiagnosticGapDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-diagnostic-gap-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Diagnostic Gap Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diagnostic-gap-detection-decision-engine">
