...
import { useGovernedClinicalContextAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-context-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-context-aggregator";
...
export function GovernedClinicalContextAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalContextAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-context-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Context Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-context-aggregator">
