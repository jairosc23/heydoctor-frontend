...
import { useGovernedRecommendationPrioritizationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-prioritization-decision-engine";
import type { GovernedRecommendationPrioritizationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-prioritization-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedRecommendationPrioritizationEngineEntryView }) {
  return (
...
export function GovernedRecommendationPrioritizationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRecommendationPrioritizationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-recommendation-prioritization-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Recommendation Prioritization Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-recommendation-prioritization-decision-engine">
