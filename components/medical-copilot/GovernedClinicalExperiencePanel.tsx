...
import { useGovernedClinicalExperience } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-experience";
import type { GovernedClinicalExperienceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-experience";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalExperienceComponentPresence;
}) {
  return (
...
export function GovernedClinicalExperiencePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalExperience({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-experience-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Experience Gobernado (Fase 45)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la experiencia clínica gobernada · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-experience">
