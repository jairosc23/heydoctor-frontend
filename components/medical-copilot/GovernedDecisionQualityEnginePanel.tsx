...
import { useGovernedDecisionQualityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-quality-engine";
import type { GovernedDecisionQualityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-quality-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDecisionQualityEngineEntryView }) {
  return (
...
export function GovernedDecisionQualityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionQualityEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-decision-quality-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Decision Quality Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-quality-engine">
