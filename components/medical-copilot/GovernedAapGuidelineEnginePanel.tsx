...
import { useGovernedAapGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-aap-guideline-engine";
import type { GovernedAapGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-aap-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAapGuidelineEngineEntryView }) {
  return (
...
export function GovernedAapGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAapGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-aap-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"AAP Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-aap-guideline-engine">
