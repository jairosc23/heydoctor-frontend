...
import { useGovernedGoldGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-gold-guideline-engine";
import type { GovernedGoldGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-gold-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedGoldGuidelineEngineEntryView }) {
  return (
...
export function GovernedGoldGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGoldGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-gold-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"GOLD Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-gold-guideline-engine">
