...
import { useGovernedEvidenceConfidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-confidence-engine";
import type { GovernedEvidenceConfidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-confidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceConfidenceEngineEntryView }) {
  return (
...
export function GovernedEvidenceConfidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceConfidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-confidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Confidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-confidence-engine">
