...
import { useGovernedDecisionGovernanceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-governance-engine";
import type { GovernedDecisionGovernanceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-governance-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDecisionGovernanceEngineEntryView }) {
  return (
...
export function GovernedDecisionGovernanceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionGovernanceEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-decision-governance-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Decision Governance Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-governance-engine">
