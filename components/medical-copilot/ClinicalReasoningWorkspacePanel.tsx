...
import { useClinicalReasoningWorkspace } from "@/lib/medical-copilot/clinical-intelligence/clinical-reasoning-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

export function ClinicalReasoningWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useClinicalReasoningWorkspace({ sessionId, enabled: Boolean(sessionId) });
  const model = result?.reasoningWorkspace;
  return (
    <div data-testid="clinical-reasoning-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Reasoning Workspace (AI-46)">
          ...
          {model ? (
            <div className="space-y-3" data-testid="clinical-reasoning-workspace">
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <span>clinicalReasoningWorkspaceId: {model.clinicalReasoningWorkspaceId}</span>
                <span>·</span>
