...
import { useGovernedPercCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-perc-calculation-engine";
import type { GovernedPercCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-perc-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedPercCalculationEngineEntryView }) {
  return (
...
export function GovernedPercCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPercCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-perc-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"PERC"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-perc-calculation-engine">
