...
import { useGovernedClinicalRiskDetection } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-risk-detection";
import type { GovernedClinicalRiskDetectionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-risk-detection";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalRiskDetectionComponentPresence }) {
  return (
...
export function GovernedClinicalRiskDetectionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalRiskDetection({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-risk-detection-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Risk Detection">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-risk-detection">
