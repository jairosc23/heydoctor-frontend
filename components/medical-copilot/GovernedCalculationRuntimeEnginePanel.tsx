...
import { useGovernedCalculationRuntimeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-runtime-engine";
import type { GovernedCalculationRuntimeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-calculation-runtime-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCalculationRuntimeEngineEntryView }) {
  return (
...
export function GovernedCalculationRuntimeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCalculationRuntimeEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-calculation-runtime-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Calculation Runtime"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-calculation-runtime-engine">
