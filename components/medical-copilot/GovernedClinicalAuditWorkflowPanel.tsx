...
import { useGovernedClinicalAuditWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-audit-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-audit-workflow";
...
export function GovernedClinicalAuditWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAuditWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-audit-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Audit Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-audit-workflow">
