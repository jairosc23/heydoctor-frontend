...
import { useGovernedDeterministicClinicalRulesPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-deterministic-clinical-rules-package";
import type { GovernedDeterministicClinicalRulesPackageEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-deterministic-clinical-rules-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedDeterministicClinicalRulesPackageEvaluationView }) {
  return (
...
export function GovernedDeterministicClinicalRulesPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDeterministicClinicalRulesPackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-deterministic-clinical-rules-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Deterministic Clinical Rules Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-deterministic-clinical-rules-package">
