...
import { useGovernedEvidenceAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-aggregator";
...
export function GovernedEvidenceAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-evidence-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-aggregator">
