...
import { useGovernedClinicalDashboardWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-dashboard-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-dashboard-workflow";
...
export function GovernedClinicalDashboardWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDashboardWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-dashboard-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Dashboard Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-dashboard-workflow">
