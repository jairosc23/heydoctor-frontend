...
import { useGovernedOrdersPersistenceBridge } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-persistence-bridge";
import type { GovernedOrdersPersistenceBridgeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-persistence-bridge";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedOrdersPersistenceBridgeComponentPresence;
}) {
  return (
...
export function GovernedOrdersPersistenceBridgePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedOrdersPersistenceBridge({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-orders-persistence-bridge-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Orders Persistence Bridge">
          <p className="mb-3 text-xs text-slate-500">
            Orders · canales Laboratory / Imaging / Procedures / Monitoring · Bloques 1–10 · pre-write · writeAttempted=false · repositoryInvoked=false
          </p>
          ...
          {result ? (
            <div
              className="space-y-3"
              data-testid="governed-orders-persistence-bridge"
            >
