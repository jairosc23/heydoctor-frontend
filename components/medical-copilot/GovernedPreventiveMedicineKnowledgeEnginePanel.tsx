...
import { useGovernedPreventiveMedicineKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-medicine-knowledge-engine";
import type { GovernedPreventiveMedicineKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-medicine-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedPreventiveMedicineKnowledgeEngineEntryView }) {
  return (
...
export function GovernedPreventiveMedicineKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveMedicineKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-medicine-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Preventive Medicine Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-medicine-knowledge-engine">
