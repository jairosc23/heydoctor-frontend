...
import { useGovernedRuleAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-rule-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-rule-aggregator";
...
export function GovernedRuleAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRuleAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-rule-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Rule Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-rule-aggregator">
