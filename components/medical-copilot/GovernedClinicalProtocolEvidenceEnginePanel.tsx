...
import { useGovernedClinicalProtocolEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-protocol-evidence-engine";
import type { GovernedClinicalProtocolEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-protocol-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedClinicalProtocolEvidenceEngineEntryView }) {
  return (
...
export function GovernedClinicalProtocolEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalProtocolEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-protocol-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Protocol Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-protocol-evidence-engine">
