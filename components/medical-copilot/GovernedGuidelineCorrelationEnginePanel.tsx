...
import { useGovernedGuidelineCorrelationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-correlation-decision-engine";
import type { GovernedGuidelineCorrelationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-guideline-correlation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedGuidelineCorrelationEngineEntryView }) {
  return (
...
export function GovernedGuidelineCorrelationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGuidelineCorrelationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-guideline-correlation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Guideline Correlation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-guideline-correlation-decision-engine">
