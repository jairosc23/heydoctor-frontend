...
import { useGovernedMissingHistoryDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-history-detection-decision-engine";
import type { GovernedMissingHistoryDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-history-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedMissingHistoryDetectionEngineEntryView }) {
  return (
...
export function GovernedMissingHistoryDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMissingHistoryDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-missing-history-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Missing History Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-missing-history-detection-decision-engine">
