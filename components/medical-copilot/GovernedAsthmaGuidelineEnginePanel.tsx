...
import { useGovernedAsthmaGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-asthma-guideline-engine";
import type { GovernedAsthmaGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-asthma-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAsthmaGuidelineEngineEntryView }) {
  return (
...
export function GovernedAsthmaGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAsthmaGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-asthma-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Asthma Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-asthma-guideline-engine">
