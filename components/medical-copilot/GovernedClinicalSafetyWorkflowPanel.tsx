...
import { useGovernedClinicalSafetyWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-safety-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-safety-workflow";
...
export function GovernedClinicalSafetyWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSafetyWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-safety-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Safety Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-safety-workflow">
