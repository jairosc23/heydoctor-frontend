...
import { useGovernedImagingKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-imaging-knowledge-engine";
import type { GovernedImagingKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-imaging-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedImagingKnowledgeEngineEntryView }) {
  return (
...
export function GovernedImagingKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedImagingKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-imaging-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Imaging Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-imaging-knowledge-engine">
