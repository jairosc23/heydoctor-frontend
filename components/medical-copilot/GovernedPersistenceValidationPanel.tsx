...
import { useGovernedPersistenceValidation } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-validation";
import type { GovernedPersistenceValidationComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-validation";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceValidationComponentPresence;
}) {
  return (
...
export function GovernedPersistenceValidationPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceValidation({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-validation-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Validation Gobernado (Fase 77)">
          <p className="mb-3 text-xs text-slate-500">
            Validación de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-validation">
