...
import { useGovernedPhysicianReviewStage } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-review-stage";
import type { GovernedPhysicianReviewStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-review-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedPhysicianReviewStageStageView }) {
  return (
...
export function GovernedPhysicianReviewStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianReviewStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-physician-review-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Physician Review Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-review-stage">
