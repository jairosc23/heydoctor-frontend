...
import { useGovernedAllergyRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-rule-engine";
import type { GovernedAllergyRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-allergy-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedAllergyRuleEngineEvaluationView }) {
  return (
...
export function GovernedAllergyRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAllergyRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-allergy-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Allergy Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-allergy-rule-engine">
