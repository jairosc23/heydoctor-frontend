...
import { useGovernedClinicalGuidelinesKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-knowledge-engine";
import type { GovernedClinicalGuidelinesKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedClinicalGuidelinesKnowledgeEngineEntryView }) {
  return (
...
export function GovernedClinicalGuidelinesKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalGuidelinesKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-guidelines-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Guidelines Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-guidelines-knowledge-engine">
