...
import { useGovernedClinicalExplainabilityEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-explainability-decision-engine";
import type { GovernedClinicalExplainabilityEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-explainability-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalExplainabilityEngineEntryView }) {
  return (
...
export function GovernedClinicalExplainabilityEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalExplainabilityEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-explainability-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Explainability Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-explainability-decision-engine">
