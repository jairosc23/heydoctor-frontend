...
import { useGovernedWellsPeCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-wells-pe-calculation-engine";
import type { GovernedWellsPeCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-wells-pe-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedWellsPeCalculationEngineEntryView }) {
  return (
...
export function GovernedWellsPeCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWellsPeCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-wells-pe-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Wells PE"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-wells-pe-calculation-engine">
