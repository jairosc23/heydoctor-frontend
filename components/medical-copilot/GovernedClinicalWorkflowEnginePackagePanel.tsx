...
import { useGovernedClinicalWorkflowEnginePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-workflow-engine-package";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-workflow-engine-package";
...
export function GovernedClinicalWorkflowEnginePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalWorkflowEnginePackage({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-workflow-engine-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Enterprise Clinical Workflow Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-workflow-engine-package">
