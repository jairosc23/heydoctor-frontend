...
import { useGovernedExpertConsensusEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-expert-consensus-evidence-engine";
import type { GovernedExpertConsensusEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-expert-consensus-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedExpertConsensusEvidenceEngineEntryView }) {
  return (
...
export function GovernedExpertConsensusEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedExpertConsensusEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-expert-consensus-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Expert Consensus Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-expert-consensus-evidence-engine">
