...
import { useGovernedAccGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-acc-guideline-engine";
import type { GovernedAccGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-acc-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedAccGuidelineEngineEntryView }) {
  return (
...
export function GovernedAccGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedAccGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-acc-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"ACC Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-acc-guideline-engine">
