...
import { useGovernedClinicalConflictDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-conflict-detection-decision-engine";
import type { GovernedClinicalConflictDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-conflict-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalConflictDetectionEngineEntryView }) {
  return (
...
export function GovernedClinicalConflictDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalConflictDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-conflict-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Conflict Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-conflict-detection-decision-engine">
