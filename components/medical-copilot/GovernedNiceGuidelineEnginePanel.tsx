...
import { useGovernedNiceGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-nice-guideline-engine";
import type { GovernedNiceGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-nice-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedNiceGuidelineEngineEntryView }) {
  return (
...
export function GovernedNiceGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNiceGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-nice-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"NICE Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-nice-guideline-engine">
