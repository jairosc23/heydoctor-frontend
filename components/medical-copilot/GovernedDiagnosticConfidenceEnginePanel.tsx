...
import { useGovernedDiagnosticConfidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-confidence-decision-engine";
import type { GovernedDiagnosticConfidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-diagnostic-confidence-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDiagnosticConfidenceEngineEntryView }) {
  return (
...
export function GovernedDiagnosticConfidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDiagnosticConfidenceEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-diagnostic-confidence-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Diagnostic Confidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-diagnostic-confidence-decision-engine">
