...
import { useGovernedCentorCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-centor-calculation-engine";
import type { GovernedCentorCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-centor-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCentorCalculationEngineEntryView }) {
  return (
...
export function GovernedCentorCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCentorCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-centor-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Centor"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-centor-calculation-engine">
