...
import { useGovernedEvidenceConfidence } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-confidence";
import type { GovernedEvidenceConfidenceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-confidence";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedEvidenceConfidenceComponentPresence }) {
  return (
...
export function GovernedEvidenceConfidencePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceConfidence({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-confidence-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Evidence Confidence">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-confidence">
