...
import { useGovernedClinicalKnowledgeWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-knowledge-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-knowledge-workflow";
...
export function GovernedClinicalKnowledgeWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalKnowledgeWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-knowledge-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Knowledge Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-knowledge-workflow">
