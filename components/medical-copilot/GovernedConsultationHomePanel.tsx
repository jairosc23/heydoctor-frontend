...
import { useGovernedConsultationHome } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-home";
import type { GovernedConsultationHomeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-home";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationHomeComponentPresence;
}) {
  return (
...
export function GovernedConsultationHomePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationHome({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-home-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Home Gobernado (Fase 41)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del home de consulta gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-home">
