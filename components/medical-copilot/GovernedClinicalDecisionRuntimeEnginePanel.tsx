...
import { useGovernedClinicalDecisionRuntimeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-runtime-engine";
import type { GovernedClinicalDecisionRuntimeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-decision-runtime-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalDecisionRuntimeEngineEntryView }) {
  return (
...
export function GovernedClinicalDecisionRuntimeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalDecisionRuntimeEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-decision-runtime-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Decision Runtime"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-decision-runtime-engine">
