...
import { useGovernedSuggestionsAggregationStage } from "@/lib/medical-copilot/clinical-intelligence/governed-suggestions-aggregation-stage";
import type { GovernedSuggestionsAggregationStageStageView } from "@/lib/medical-copilot/clinical-intelligence/governed-suggestions-aggregation-stage";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function StageCard({ stage }: { stage: GovernedSuggestionsAggregationStageStageView }) {
  return (
...
export function GovernedSuggestionsAggregationStagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSuggestionsAggregationStage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-suggestions-aggregation-stage-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Suggestions Aggregation Stage"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-suggestions-aggregation-stage">
