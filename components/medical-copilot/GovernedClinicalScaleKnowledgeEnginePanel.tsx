...
import { useGovernedClinicalScaleKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-scale-knowledge-engine";
import type { GovernedClinicalScaleKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-scale-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedClinicalScaleKnowledgeEngineEntryView }) {
  return (
...
export function GovernedClinicalScaleKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalScaleKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-scale-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Scale Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-scale-knowledge-engine">
