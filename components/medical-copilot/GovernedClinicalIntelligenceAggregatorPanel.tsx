...
import { useGovernedClinicalIntelligenceAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-aggregator";
...
export function GovernedClinicalIntelligenceAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalIntelligenceAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-intelligence-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Intelligence Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-intelligence-aggregator">
