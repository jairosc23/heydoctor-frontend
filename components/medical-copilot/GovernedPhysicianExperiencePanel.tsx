...
import { useGovernedPhysicianExperience } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-experience";
import type { GovernedPhysicianExperienceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-experience";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPhysicianExperienceComponentPresence;
}) {
  return (
...
export function GovernedPhysicianExperiencePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianExperience({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-physician-experience-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Physician Experience Gobernado (Fase 46)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la experiencia médica gobernada · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-experience">
