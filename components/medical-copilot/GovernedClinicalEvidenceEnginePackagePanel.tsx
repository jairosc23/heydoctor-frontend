...
import { useGovernedClinicalEvidenceEnginePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-engine-package";
import type { GovernedClinicalEvidenceEnginePackageEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-evidence-engine-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedClinicalEvidenceEnginePackageEntryView }) {
  return (
...
export function GovernedClinicalEvidenceEnginePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalEvidenceEnginePackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-evidence-engine-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Evidence Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-evidence-engine-package">
