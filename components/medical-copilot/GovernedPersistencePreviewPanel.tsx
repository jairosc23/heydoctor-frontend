...
import { useGovernedPersistencePreview } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-preview";
import type { GovernedPersistencePreviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-preview";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistencePreviewComponentPresence;
}) {
  return (
...
export function GovernedPersistencePreviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistencePreview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-preview-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Preview Gobernado (Fase 76)">
          <p className="mb-3 text-xs text-slate-500">
            Preview de persistencia gobernada · Solo lectura · Solo preview · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-preview">
