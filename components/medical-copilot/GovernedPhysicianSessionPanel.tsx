...
import { useGovernedPhysicianSession } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-session";
import type { GovernedPhysicianSessionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-session";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPhysicianSessionComponentPresence;
}) {
  return (
...
export function GovernedPhysicianSessionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianSession({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-physician-session-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Physician Session Gobernado (Fase 57)">
          <p className="mb-3 text-xs text-slate-500">
            Sesión médica gobernada · Solo lectura · Sin persistencia · Sin ejecución · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-session">
