...
import { useGovernedCockcroftGaultCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-cockcroft-gault-calculation-engine";
import type { GovernedCockcroftGaultCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-cockcroft-gault-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedCockcroftGaultCalculationEngineEntryView }) {
  return (
...
export function GovernedCockcroftGaultCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCockcroftGaultCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-cockcroft-gault-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Cockcroft-Gault"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-cockcroft-gault-calculation-engine">
