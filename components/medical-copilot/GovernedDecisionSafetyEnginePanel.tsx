...
import { useGovernedDecisionSafetyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-safety-engine";
import type { GovernedDecisionSafetyEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-decision-safety-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedDecisionSafetyEngineEntryView }) {
  return (
...
export function GovernedDecisionSafetyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDecisionSafetyEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-decision-safety-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Decision Safety Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-decision-safety-engine">
