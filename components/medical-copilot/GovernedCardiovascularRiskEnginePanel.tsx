...
import { useGovernedCardiovascularRiskEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-cardiovascular-risk-engine";
import type { GovernedCardiovascularRiskEngineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-cardiovascular-risk-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedCardiovascularRiskEngineComponentPresence }) {
  return (
...
export function GovernedCardiovascularRiskEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCardiovascularRiskEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-cardiovascular-risk-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Cardiovascular Risk Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-cardiovascular-risk-engine">
