...
import { useGovernedApprovalPreview } from "@/lib/medical-copilot/clinical-intelligence/governed-approval-preview";
import type { GovernedApprovalPreviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-approval-preview";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedApprovalPreviewComponentPresence;
}) {
  return (
...
export function GovernedApprovalPreviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedApprovalPreview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-approval-preview-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Approval Preview Gobernado (Fase 53)">
          <p className="mb-3 text-xs text-slate-500">
            Preview de aprobación gobernada · Solo lectura · NO aprueba · Sin persistencia · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-approval-preview">
