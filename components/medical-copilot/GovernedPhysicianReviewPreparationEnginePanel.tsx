...
import { useGovernedPhysicianReviewPreparationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-review-preparation-decision-engine";
import type { GovernedPhysicianReviewPreparationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-physician-review-preparation-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedPhysicianReviewPreparationEngineEntryView }) {
  return (
...
export function GovernedPhysicianReviewPreparationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPhysicianReviewPreparationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-physician-review-preparation-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Physician Review Preparation Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-physician-review-preparation-decision-engine">
