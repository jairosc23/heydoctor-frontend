...
import { useGovernedDraftComparisonWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-draft-comparison-workspace";
import type { GovernedDraftComparisonWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-draft-comparison-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedDraftComparisonWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedDraftComparisonWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedDraftComparisonWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-draft-comparison-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Draft Comparison Workspace Gobernado (Fase 51)">
          <p className="mb-3 text-xs text-slate-500">
            Comparación estructural de drafts · Solo lectura · Sin contenido clínico nuevo · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-draft-comparison-workspace">
