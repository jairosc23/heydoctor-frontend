...
import { useGovernedClinicalTimeline } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-timeline";
import type { GovernedClinicalTimelineComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-timeline";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalTimelineComponentPresence;
}) {
  return (
...
export function GovernedClinicalTimelinePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalTimeline({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-timeline-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Timeline Gobernado (Fase 42)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del timeline clínico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-timeline">
