...
import { useGovernedPreventiveScreeningKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-screening-knowledge-engine";
import type { GovernedPreventiveScreeningKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-screening-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedPreventiveScreeningKnowledgeEngineEntryView }) {
  return (
...
export function GovernedPreventiveScreeningKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveScreeningKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-screening-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Preventive Screening Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-screening-knowledge-engine">
