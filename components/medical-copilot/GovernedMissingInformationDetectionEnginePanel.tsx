...
import { useGovernedMissingInformationDetectionEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-information-detection-decision-engine";
import type { GovernedMissingInformationDetectionEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-missing-information-detection-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedMissingInformationDetectionEngineEntryView }) {
  return (
...
export function GovernedMissingInformationDetectionEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMissingInformationDetectionEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-missing-information-detection-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Missing Information Detection Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-missing-information-detection-decision-engine">
