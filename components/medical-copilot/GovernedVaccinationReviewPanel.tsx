...
import { useGovernedVaccinationReview } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-review";
import type { GovernedVaccinationReviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-vaccination-review";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedVaccinationReviewComponentPresence }) {
  return (
...
export function GovernedVaccinationReviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedVaccinationReview({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-vaccination-review-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Vaccination Review">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-vaccination-review">
