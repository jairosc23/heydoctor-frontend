...
import { useGovernedHypertensionGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-hypertension-guideline-engine";
import type { GovernedHypertensionGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-hypertension-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedHypertensionGuidelineEngineEntryView }) {
  return (
...
export function GovernedHypertensionGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHypertensionGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-hypertension-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Hypertension Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-hypertension-guideline-engine">
