...
import { useGovernedClinicalGuidelinesEnginePackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-engine-package";
import type { GovernedClinicalGuidelinesEnginePackageEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-guidelines-engine-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedClinicalGuidelinesEnginePackageEntryView }) {
  return (
...
export function GovernedClinicalGuidelinesEnginePackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalGuidelinesEnginePackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-guidelines-engine-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Guidelines Package"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-guidelines-engine-package">
