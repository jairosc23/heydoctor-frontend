...
import { useGovernedRecommendationRankingEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-ranking-decision-engine";
import type { GovernedRecommendationRankingEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-recommendation-ranking-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedRecommendationRankingEngineEntryView }) {
  return (
...
export function GovernedRecommendationRankingEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedRecommendationRankingEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-recommendation-ranking-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Recommendation Ranking Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-recommendation-ranking-decision-engine">
