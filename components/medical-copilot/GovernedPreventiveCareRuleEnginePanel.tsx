...
import { useGovernedPreventiveCareRuleEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-care-rule-engine";
import type { GovernedPreventiveCareRuleEngineEvaluationView } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-care-rule-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function RuleEvaluationCard({ evaluation }: { evaluation: GovernedPreventiveCareRuleEngineEvaluationView }) {
  return (
...
export function GovernedPreventiveCareRuleEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveCareRuleEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-care-rule-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Governed Preventive Care Rule Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-care-rule-engine">
