...
import { useGovernedOrdersPersistenceExecution } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-persistence-execution";
import type { GovernedOrdersPersistenceExecutionComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-orders-persistence-execution";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";
function ComponentPresenceCard({ component }: { component: GovernedOrdersPersistenceExecutionComponentPresence }) {
  return (
...
export function GovernedOrdersPersistenceExecutionPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedOrdersPersistenceExecution({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-orders-persistence-execution-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Orders Persistence Execution">
          <p className="mb-3 text-xs text-slate-500">Persistencia gobernada · HITL · rollback · dominio orders</p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-orders-persistence-execution">
