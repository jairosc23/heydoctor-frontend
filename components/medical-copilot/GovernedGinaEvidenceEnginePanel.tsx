...
import { useGovernedGinaEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-gina-evidence-engine";
import type { GovernedGinaEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-gina-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedGinaEvidenceEngineEntryView }) {
  return (
...
export function GovernedGinaEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedGinaEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-gina-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"GINA Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-gina-evidence-engine">
