...
import { useGovernedMeldCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-meld-calculation-engine";
import type { GovernedMeldCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-meld-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedMeldCalculationEngineEntryView }) {
  return (
...
export function GovernedMeldCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMeldCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-meld-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"MELD"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-meld-calculation-engine">
