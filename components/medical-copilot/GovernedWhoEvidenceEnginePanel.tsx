...
import { useGovernedWhoEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-who-evidence-engine";
import type { GovernedWhoEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-who-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedWhoEvidenceEngineEntryView }) {
  return (
...
export function GovernedWhoEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWhoEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-who-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"WHO Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-who-evidence-engine">
