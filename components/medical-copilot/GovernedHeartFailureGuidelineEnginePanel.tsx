...
import { useGovernedHeartFailureGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-heart-failure-guideline-engine";
import type { GovernedHeartFailureGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-heart-failure-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedHeartFailureGuidelineEngineEntryView }) {
  return (
...
export function GovernedHeartFailureGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHeartFailureGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-heart-failure-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Heart Failure Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-heart-failure-guideline-engine">
