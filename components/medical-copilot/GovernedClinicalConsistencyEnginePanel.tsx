...
import { useGovernedClinicalConsistencyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-consistency-decision-engine";
import type { GovernedClinicalConsistencyEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-consistency-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalConsistencyEngineEntryView }) {
  return (
...
export function GovernedClinicalConsistencyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalConsistencyEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-consistency-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Consistency Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-consistency-decision-engine">
