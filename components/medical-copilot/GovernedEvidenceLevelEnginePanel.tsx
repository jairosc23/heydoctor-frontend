...
import { useGovernedEvidenceLevelEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-level-engine";
import type { GovernedEvidenceLevelEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-level-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedEvidenceLevelEngineEntryView }) {
  return (
...
export function GovernedEvidenceLevelEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceLevelEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-level-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Evidence Level Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-level-engine">
