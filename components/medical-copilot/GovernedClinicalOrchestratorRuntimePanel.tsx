...
import { useGovernedClinicalOrchestratorRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-orchestrator-runtime";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-orchestrator-runtime";
...
export function GovernedClinicalOrchestratorRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalOrchestratorRuntime({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-orchestrator-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Orchestrator Runtime"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-orchestrator-runtime">
