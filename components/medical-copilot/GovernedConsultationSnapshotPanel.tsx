...
import { useGovernedConsultationSnapshot } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-snapshot";
import type { GovernedConsultationSnapshotComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-consultation-snapshot";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedConsultationSnapshotComponentPresence;
}) {
  return (
...
export function GovernedConsultationSnapshotPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedConsultationSnapshot({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-consultation-snapshot-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Consultation Snapshot Gobernado (Fase 21)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del snapshot de consulta gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-consultation-snapshot">
