...
import { useGovernedPersistenceRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-runtime";
import type { GovernedPersistenceRuntimeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-runtime";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceRuntimeComponentPresence;
}) {
  return (
...
export function GovernedPersistenceRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceRuntime({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Runtime Gobernado (Fase 75)">
          <p className="mb-3 text-xs text-slate-500">
            Runtime de preparación de persistencia · Solo lectura · NO persiste · Sin ejecución · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-runtime">
