...
import { useGovernedClinicalAnalyticsWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-analytics-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-analytics-workflow";
...
export function GovernedClinicalAnalyticsWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalAnalyticsWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-analytics-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Analytics Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-analytics-workflow">
