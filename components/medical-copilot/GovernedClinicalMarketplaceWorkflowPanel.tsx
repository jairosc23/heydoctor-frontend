...
import { useGovernedClinicalMarketplaceWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-marketplace-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-marketplace-workflow";
...
export function GovernedClinicalMarketplaceWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalMarketplaceWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-marketplace-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Marketplace Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-marketplace-workflow">
