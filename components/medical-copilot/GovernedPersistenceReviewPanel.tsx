...
import { useGovernedPersistenceReview } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-review";
import type { GovernedPersistenceReviewComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-persistence-review";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedPersistenceReviewComponentPresence;
}) {
  return (
...
export function GovernedPersistenceReviewPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPersistenceReview({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-persistence-review-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Persistence Review Gobernado (Fase 70)">
          <p className="mb-3 text-xs text-slate-500">
            Revisión de preparación de persistencia · Solo lectura · NO persiste · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-persistence-review">
