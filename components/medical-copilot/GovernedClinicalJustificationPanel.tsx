...
import { useGovernedClinicalJustification } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-justification";
import type { GovernedClinicalJustificationComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-justification";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalJustificationComponentPresence }) {
  return (
...
export function GovernedClinicalJustificationPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalJustification({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-justification-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Justification">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-justification">
