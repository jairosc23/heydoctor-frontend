...
import { useGovernedNiceEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-nice-evidence-engine";
import type { GovernedNiceEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-nice-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedNiceEvidenceEngineEntryView }) {
  return (
...
export function GovernedNiceEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedNiceEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-nice-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"NICE Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-nice-evidence-engine">
