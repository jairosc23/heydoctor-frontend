...
import { useGovernedClinicalSuggestionPackage } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-suggestion-package";
import type { GovernedClinicalSuggestionPackageComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-suggestion-package";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalSuggestionPackageComponentPresence }) {
  return (
...
export function GovernedClinicalRecommendationPackagePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSuggestionPackage({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-suggestion-package-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Recommendation Package">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-suggestion-package">
