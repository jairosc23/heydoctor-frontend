...
import { useGovernedMetaAnalysisEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-meta-analysis-evidence-engine";
import type { GovernedMetaAnalysisEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-meta-analysis-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedMetaAnalysisEvidenceEngineEntryView }) {
  return (
...
export function GovernedMetaAnalysisEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedMetaAnalysisEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-meta-analysis-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Meta-analysis Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-meta-analysis-evidence-engine">
