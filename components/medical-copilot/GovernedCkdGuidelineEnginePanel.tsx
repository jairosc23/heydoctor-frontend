...
import { useGovernedCkdGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-ckd-guideline-engine";
import type { GovernedCkdGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-ckd-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedCkdGuidelineEngineEntryView }) {
  return (
...
export function GovernedCkdGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedCkdGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-ckd-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"CKD Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-ckd-guideline-engine">
