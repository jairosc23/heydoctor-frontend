...
import { useGovernedClinicalTransparencyEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-transparency-decision-engine";
import type { GovernedClinicalTransparencyEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-transparency-decision-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedClinicalTransparencyEngineEntryView }) {
  return (
...
export function GovernedClinicalTransparencyEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalTransparencyEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-transparency-decision-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Transparency Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-transparency-decision-engine">
