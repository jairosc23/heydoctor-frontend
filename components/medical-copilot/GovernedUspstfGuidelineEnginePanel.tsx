...
import { useGovernedUspstfGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-uspstf-guideline-engine";
import type { GovernedUspstfGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-uspstf-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedUspstfGuidelineEngineEntryView }) {
  return (
...
export function GovernedUspstfGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedUspstfGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-uspstf-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"USPSTF Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-uspstf-guideline-engine">
