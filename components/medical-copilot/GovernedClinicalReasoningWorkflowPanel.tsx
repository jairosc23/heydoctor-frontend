...
import { useGovernedClinicalReasoningWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-reasoning-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-reasoning-workflow";
...
export function GovernedClinicalReasoningWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalReasoningWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-reasoning-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Reasoning Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-reasoning-workflow">
