...
import { useGovernedEvidenceTrace } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-trace";
import type { GovernedEvidenceTraceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-evidence-trace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedEvidenceTraceComponentPresence }) {
  return (
...
export function GovernedEvidenceTracePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEvidenceTrace({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-evidence-trace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Evidence Trace">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-evidence-trace">
