...
import { useGovernedClinicalSuggestionRuntime } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-suggestion-runtime";
import type { GovernedClinicalSuggestionRuntimeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-suggestion-runtime";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({ component }: { component: GovernedClinicalSuggestionRuntimeComponentPresence }) {
  return (
...
export function GovernedClinicalSuggestionRuntimePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalSuggestionRuntime({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-clinical-suggestion-runtime-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Governed Clinical Suggestion Runtime">
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-suggestion-runtime">
