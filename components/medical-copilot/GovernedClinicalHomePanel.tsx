...
import { useGovernedClinicalHome } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-home";
import type { GovernedClinicalHomeComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-home";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalHomeComponentPresence;
}) {
  return (
...
export function GovernedClinicalHomePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalHome({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-home-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Home Gobernado (Fase 39)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del home clínico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-home">
