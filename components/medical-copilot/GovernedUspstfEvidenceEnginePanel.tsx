...
import { useGovernedUspstfEvidenceEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-uspstf-evidence-engine";
import type { GovernedUspstfEvidenceEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-uspstf-evidence-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EvidenceEntryCard({ entry }: { entry: GovernedUspstfEvidenceEngineEntryView }) {
  return (
...
export function GovernedUspstfEvidenceEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedUspstfEvidenceEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-uspstf-evidence-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"USPSTF Evidence Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-uspstf-evidence-engine">
