...
export function PhysicianReviewDashboardPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = usePhysicianReviewDashboard({ sessionId, enabled: Boolean(sessionId) });
  const model = result?.reviewDashboard;
  return (
    <div data-testid="physician-review-dashboard-panel">
          ...
          {model ? (
            <div className="space-y-3" data-testid="physician-review-dashboard">
              <div className="flex flex-wrap items-center gap-2 text-xs text-slate-500">
                <span>reviewDashboardId: {model.reviewDashboardId}</span>
                <span>·</span>
