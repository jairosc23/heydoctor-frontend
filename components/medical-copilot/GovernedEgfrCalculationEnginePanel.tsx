...
import { useGovernedEgfrCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-egfr-calculation-engine";
import type { GovernedEgfrCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-egfr-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedEgfrCalculationEngineEntryView }) {
  return (
...
export function GovernedEgfrCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedEgfrCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-egfr-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"eGFR"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-egfr-calculation-engine">
