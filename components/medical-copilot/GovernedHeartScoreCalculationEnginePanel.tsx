...
import { useGovernedHeartScoreCalculationEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-heart-score-calculation-engine";
import type { GovernedHeartScoreCalculationEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-heart-score-calculation-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function EntryCard({ entry }: { entry: GovernedHeartScoreCalculationEngineEntryView }) {
  return (
...
export function GovernedHeartScoreCalculationEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedHeartScoreCalculationEngine({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-heart-score-calculation-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"HEART Score"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-heart-score-calculation-engine">
