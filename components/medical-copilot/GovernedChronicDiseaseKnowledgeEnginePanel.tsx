...
import { useGovernedChronicDiseaseKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-chronic-disease-knowledge-engine";
import type { GovernedChronicDiseaseKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-chronic-disease-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedChronicDiseaseKnowledgeEngineEntryView }) {
  return (
...
export function GovernedChronicDiseaseKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedChronicDiseaseKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-chronic-disease-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Chronic Disease Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-chronic-disease-knowledge-engine">
