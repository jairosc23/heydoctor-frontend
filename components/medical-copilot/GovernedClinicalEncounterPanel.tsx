...
import { useGovernedClinicalEncounter } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-encounter";
import type { GovernedClinicalEncounterComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-encounter";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalEncounterComponentPresence;
}) {
  return (
...
export function GovernedClinicalEncounterPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalEncounter({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-encounter-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Encounter Gobernado (Fase 18)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura de la sesión clínica completa · Sin
            generación · Sin edición · Sin aprobación · HITL obligatorio · No
          ...
          {result ? (
            <div
              className="space-y-3"
              data-testid="governed-clinical-encounter"
            >
