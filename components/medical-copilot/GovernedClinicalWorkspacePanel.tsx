...
import { useGovernedClinicalWorkspace } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-workspace";
import type { GovernedClinicalWorkspaceComponentPresence } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-workspace";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function ComponentPresenceCard({
  component,
}: {
  component: GovernedClinicalWorkspaceComponentPresence;
}) {
  return (
...
export function GovernedClinicalWorkspacePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalWorkspace({
    sessionId,
    enabled: Boolean(sessionId),
  });

  return (
    <div data-testid="governed-clinical-workspace-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title="Clinical Workspace Gobernado (Fase 29)">
          <p className="mb-3 text-xs text-slate-500">
            Composición de solo lectura del workspace clínico gobernado · Sin generación · Sin edición · Sin aprobación · HITL obligatorio · No modifica EMR/Workflow.
          </p>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-workspace">
