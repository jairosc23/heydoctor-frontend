...
import { useGovernedPreventiveGuidelineEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-guideline-engine";
import type { GovernedPreventiveGuidelineEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-preventive-guideline-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function GuidelineEntryCard({ entry }: { entry: GovernedPreventiveGuidelineEngineEntryView }) {
  return (
...
export function GovernedPreventiveGuidelineEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedPreventiveGuidelineEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-preventive-guideline-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Preventive Guideline Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-preventive-guideline-engine">
