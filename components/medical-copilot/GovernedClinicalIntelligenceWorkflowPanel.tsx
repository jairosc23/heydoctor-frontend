...
import { useGovernedClinicalIntelligenceWorkflow } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-workflow";
import type { GovernedClinicalWorkflowView } from "@/lib/medical-copilot/clinical-intelligence/governed-clinical-intelligence-workflow";
...
export function GovernedClinicalIntelligenceWorkflowPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedClinicalIntelligenceWorkflow({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-clinical-intelligence-workflow-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Clinical Intelligence Workflow"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-clinical-intelligence-workflow">
