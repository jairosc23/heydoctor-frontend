...
import { useGovernedLifestyleMedicineKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-lifestyle-medicine-knowledge-engine";
import type { GovernedLifestyleMedicineKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-lifestyle-medicine-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedLifestyleMedicineKnowledgeEngineEntryView }) {
  return (
...
export function GovernedLifestyleMedicineKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedLifestyleMedicineKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-lifestyle-medicine-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Lifestyle Medicine Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-lifestyle-medicine-knowledge-engine">
