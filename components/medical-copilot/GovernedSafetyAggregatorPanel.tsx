...
import { useGovernedSafetyAggregator } from "@/lib/medical-copilot/clinical-intelligence/governed-safety-aggregator";
import type { GovernedOrchestratorAggregatorView } from "@/lib/medical-copilot/clinical-intelligence/governed-safety-aggregator";
...
export function GovernedSafetyAggregatorPanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedSafetyAggregator({ sessionId, enabled: Boolean(sessionId) });
  return (
    <div data-testid="governed-safety-aggregator-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Safety Aggregator"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-safety-aggregator">
