...
import { useGovernedWomensHealthKnowledgeEngine } from "@/lib/medical-copilot/clinical-intelligence/governed-womens-health-knowledge-engine";
import type { GovernedWomensHealthKnowledgeEngineEntryView } from "@/lib/medical-copilot/clinical-intelligence/governed-womens-health-knowledge-engine";
import { useMedicalCopilot } from "@/context/MedicalCopilotContext";

function KnowledgeEntryCard({ entry }: { entry: GovernedWomensHealthKnowledgeEngineEntryView }) {
  return (
...
export function GovernedWomensHealthKnowledgeEnginePanel() {
  const { session } = useMedicalCopilot();
  const sessionId = session?.sessionId ?? null;
  const { loading, error, result, refresh } = useGovernedWomensHealthKnowledgeEngine({ sessionId, enabled: Boolean(sessionId) });

  return (
    <div data-testid="governed-womens-health-knowledge-engine-panel">
      <ClinicalPanel depth={2}>
        <ClinicalSection title={"Women's Health Knowledge Engine"}>
          ...
          {result ? (
            <div className="space-y-3" data-testid="governed-womens-health-knowledge-engine">
