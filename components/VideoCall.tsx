...
import {
  humanizeCallError,
  useTelemedicineCall,
} from "@/hooks/useTelemedicineCall";
...
export const VideoCall = forwardRef<VideoCallRecordingHandle, VideoCallProps>(
  function VideoCall(
    {
      consultationId,
      onEndCall,
    ...
    useLayoutEffect(() => {
      ...
      if (!el) return;
      return attachVideoPlaybackDiagnostics(
        el,
        remoteStream,
        "remote",
        logVideo,
    ...
    useEffect(() => {
      return () => {
        ...
        const nodes = [mobileShellRef.current].filter(
          Boolean,
        ) as HTMLDivElement[];
    ...
    if (!guestCall && loading) {
      return (
        <div
          ...
          aria-busy="true"
        >
          <p className="m-0">Preparando sesión…</p>
        </div>
      );
    }
    if (!guestCall && !user) {
      return (
        <div className="flex min-h-0 flex-1 flex-col justify-center p-6 text-red-200">
          <p>Inicia sesión para usar la videollamada.</p>
    ...
    const chatPanelEl = chatPanelOpen ? (
      ...
      >
        <p
          style={{ margin: 0, fontSize: 14, color: "#e2e8f0", lineHeight: 1.5 }}
        >
...
const chatSidePanelStyle: React.CSSProperties = {
  ...
  zIndex: 240,
  padding:
    "max(env(safe-area-inset-top), 20px) 20px max(env(safe-area-inset-bottom), 20px)",
