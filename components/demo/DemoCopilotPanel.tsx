...
export function DemoCopilotPanel({
  ...
  if (isLoading) {
    return (
      <div className={`rounded-lg border border-slate-200 bg-white p-4 ${className}`}>
        <div className="mb-3">
          <div className="h-3 w-24 animate-pulse rounded bg-indigo-100" />
  ...
  return (
    <div className={`overflow-hidden rounded-lg border border-indigo-100 bg-white ${className}`}>
      <div className="border-b border-indigo-100 bg-indigo-50 px-3 py-2">
        <p className="text-sm font-semibold text-indigo-900">Asistencia del Copilot</p>
        <p className="mt-0.5 text-xs text-indigo-700">Sugerencias clínicas para revisión médica.</p>
      </div>
      <div className="space-y-3 p-3 text-sm">
        ...
        {diagnoses.length > 0 && (
          <section className="rounded-md border border-indigo-100 bg-indigo-50/60 p-3">
            <h4 className="mb-1 font-medium text-indigo-900">Diagnóstico diferencial</h4>
              ...
              {(data.suggested_diagnoses ?? data.possible_diagnoses ?? []).map((d, i) => {
                ...
                return (
                  <li key={i} className="pl-1">
                    <span>{desc}</span>
                    {conf != null && (
                      <span className="ml-1 text-indigo-700">({Math.round(conf * 100)}% confianza)</span>
                    )}
                    {expl && (
              ...
              {(data.suggested_treatments ?? []).map((t, i) => {
                ...
                return (
                  <li key={i} className="pl-1">
                    <span>{name}</span>
                    {conf != null && (
                      <span className="ml-1 text-indigo-600">({Math.round(conf * 100)}%)</span>
                    )}
                    {expl && (
                      <p className="ml-4 mt-0.5 text-xs text-slate-500">Justificación: {expl}</p>
                    )}
